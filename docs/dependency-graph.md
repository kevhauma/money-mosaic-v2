...
subgraph DB["ui"]
subgraph DC["alert"]
DD["alert.component.ts"]
end
subgraph DE["badge"]
DF["badge.component.ts"]
end
subgraph DG["button"]
DH["button.component.ts"]
end
subgraph DI["collapse"]
DJ["collapse.component.ts"]
end
subgraph DK["confirm-dialog"]
DL["confirm-dialog.component.ts"]
end
subgraph DM["cycle-picker"]
DN["cycle-picker.component.ts"]
end
subgraph DO["date-range-input"]
DP["date-range-input.component.ts"]
end
subgraph DQ["divider"]
DR["divider.component.ts"]
end
subgraph DS["dropdown"]
DT["dropdown.component.ts"]
end
subgraph DU["empty-state"]
DV["empty-state.component.ts"]
end
subgraph DW["fieldset"]
DX["fieldset.component.ts"]
end
subgraph DY["flex"]
DZ["flex.component.ts"]
end
subgraph E0["granularity-picker"]
E1["granularity-picker.component.ts"]
end
E2["index.ts"]
subgraph E3["input"]
E4["input.component.ts"]
end
subgraph E5["label"]
E6["label.component.ts"]
end
subgraph E7["loading-skeleton"]
E8["loading-skeleton.component.ts"]
end
subgraph E9["modal"]
EA["mm-modal.component.ts"]
end
subgraph EB["page-header"]
EC["page-header.component.ts"]
end
subgraph ED["paginator"]
EE["paginator.component.ts"]
end
subgraph EF["paper"]
EG["paper.component.ts"]
end
subgraph EH["privacy-blur"]
EI["privacy-blur.component.ts"]
end
...
subgraph EP["stat-card"]
EQ["stat-card.component.ts"]
end
subgraph ER["table"]
ES["table.component.ts"]
end
subgraph ET["tabs"]
EU["tabs.component.ts"]
end
subgraph EV["typography"]
EW["typography.component.ts"]
end
end
subgraph EX["utils"]
EY["calendar-cycles.ts"]
EZ["confidence-color.ts"]
F0["confirm-state.ts"]
F1["currency-format.ts"]
F2["currency-symbol-presets.ts"]
F3["daisy-classes.ts"]
F4["date-buckets.ts"]
F5["date-format.pipe.ts"]
F6["date-format.ts"]
F7["debounced-text.ts"]
F8["download-json.ts"]
F9["fingerprint.ts"]
FA["format-settings.testing.ts"]
FB["format-settings.ts"]
FC["hidden-amount.ts"]
FD["iban.ts"]
FE["index.ts"]
FF["link-control-to-setting.ts"]
FG["locale-presets.ts"]
FH["number-format.ts"]
FI["pagination.ts"]
FJ["percentage.ts"]
