# Dependency graph reference

Full import graph of `src/app`, generated with `dependency-cruiser`, for Claude to consult
when tracing what imports what without re-grepping the tree by hand.

## Scope

Unlike the human-facing overview in [`reports/dependency-graph.html`](../reports/dependency-graph.html)
(which collapses each `feature-*` folder to a single node and stops one folder into
`core`/`shared`), this graph goes three folder levels deeper on both tiers — up to 4 folders
below `src/app` — which is enough to reach individual files in most feature/core/shared
subtrees.

<!-- GENERATED:STATS:START -->
- 425 leaf nodes, 1315 edges.
<!-- GENERATED:STATS:END -->

- `*.spec.ts` and `node_modules` are excluded.
- Node IDs are dependency-cruiser's internal short hashes (`4`, `2K`, `6N`, ...) — read them
  via the `subgraph`/node label text, not the ID itself.
- An edge collapsed at a folder boundary (rare here, since most subtrees fit within 4 levels)
  means _something_ inside that folder imports the target, not necessarily its `index.ts`.

## Regenerate

```bash
depcruise src/app \
  --config .dependency-cruiser.cjs \
  --collapse '^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+|^src/app/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+|^src/app/[^/]+' \
  --output-type mermaid \
  --output-to reports/dependency-graph-deep.mmd
```

## Graph

```mermaid
flowchart LR

subgraph 0["src"]
subgraph 1["app"]
subgraph 2["core"]
subgraph 3["accounts"]
4["account-deletion.service.ts"]
5["index.ts"]
6["joint-owner-lookup.ts"]
end
subgraph 7["categorisation"]
8["category-applicability.ts"]
9["co-owner-contribution.service.ts"]
A["co-owner-contribution.ts"]
B["index.ts"]
C["rule-matching.ts"]
D["rules-engine.service.ts"]
end
subgraph E["data-access"]
F["accounts.repository.ts"]
G["app-db.ts"]
H["app-settings.repository.ts"]
I["categories.repository.ts"]
J["category-comparison-settings.repository.ts"]
K["category-model.repository.ts"]
L["dashboard-layout-settings.repository.ts"]
M["data-management.repository.ts"]
N["forecast-settings.repository.ts"]
O["goals.repository.ts"]
P["import-batches.repository.ts"]
Q["index.ts"]
R["loans.repository.ts"]
S["mapping-profiles.repository.ts"]
T["recurring-overrides.repository.ts"]
U["rules.repository.ts"]
V["salary-metadata.repository.ts"]
W["transactions.repository.ts"]
X["transfer-settings.repository.ts"]
Y["transfers.repository.ts"]
end
subgraph Z["import"]
10["account-detection.ts"]
11["csv-import.service.ts"]
12["csv-parse.ts"]
13["csv-parse.worker.ts"]
14["csv-row-mapper.ts"]
15["csv-worker.types.ts"]
16["delimiter-guess.ts"]
17["import.service.ts"]
18["index.ts"]
end
subgraph 19["layout"]
subgraph 1A["app-shell"]
1B["app-shell.component.ts"]
end
1C["index.ts"]
end
subgraph 1D["links"]
1E["external-links.ts"]
1F["index.ts"]
end
subgraph 1G["loans"]
1H["amortization.ts"]
1I["index.ts"]
1J["loan-progress.ts"]
1K["what-if.ts"]
end
subgraph 1L["ml"]
1M["category-model.worker.ts"]
1N["category-model.worker.types.ts"]
1O["feature-hashing.ts"]
1P["index.ts"]
1Q["model-config.ts"]
1R["rule-proposal-mining.ts"]
1S["training-window.ts"]
end
subgraph 1T["onboarding"]
1U["home-redirect.guard.ts"]
1V["index.ts"]
1W["mark-visited.guard.ts"]
1X["visited.service.ts"]
end
subgraph 1Y["state"]
1Z["accounts.store.ts"]
20["app-settings.store.ts"]
21["categories.store.ts"]
22["chart-options-control.ts"]
23["chart-options.store.ts"]
24["forecast-settings.store.ts"]
25["goals.store.ts"]
26["import-batches.store.ts"]
27["index.ts"]
28["page-range-control.ts"]
29["range-state.store.ts"]
2A["transactions.store.ts"]
2B["transfer-settings.store.ts"]
2C["transfers.store.ts"]
end
subgraph 2D["stats"]
2E["account-balance-history.ts"]
2F["account-balance-trend.ts"]
2G["annual-lump-sum-smoothing.ts"]
2H["category-breakdown.ts"]
2I["category-composition-trend.ts"]
2J["category-cycle-heatmap.ts"]
2K["category-expense-transactions.ts"]
2L["category-kind-contribution.ts"]
2M["category-period-comparison.ts"]
2N["chart-zoom-window.ts"]
2O["classify-for-stats.ts"]
2P["classify-joint-leg.ts"]
2Q["day-transactions.ts"]
2R["embedded-bonus-smoothing.ts"]
2S["full-history-range.ts"]
2T["goal-affordability.ts"]
2U["granularity-for-span.ts"]
2V["gross-net-growth.ts"]
2W["gross-net-ratio.ts"]
2X["income-category-series.ts"]
2Y["income-events.ts"]
2Z["income-gap-detection.ts"]
30["income-growth.ts"]
31["income-step-change-detection.ts"]
32["index.ts"]
33["joint-account-stake.ts"]
34["joint-contributor-breakdown.ts"]
35["money-flow-graph.ts"]
36["multi-year-income-comparison.ts"]
37["net-margin.ts"]
38["net-worth-projection.ts"]
39["period-stats.ts"]
3A["period-window.ts"]
3B["periodized-rate.ts"]
3C["recurring-payments.ts"]
3D["recurring-projection.ts"]
3E["required-saving-rate.ts"]
3F["saving-velocity.ts"]
3G["spending-mosaic.ts"]
3H["top-transactions.ts"]
3I["wage-change-detection.ts"]
3J["weekday-weekend-split.ts"]
3K["year-over-year.ts"]
3L["yearly-income-summary.ts"]
end
subgraph 3M["storage"]
3N["index.ts"]
3O["storage-status.service.ts"]
end
subgraph 3P["theme"]
3Q["accent-colors.ts"]
3R["index.ts"]
3S["theme-styles.ts"]
3T["theme.service.ts"]
end
subgraph 3U["transactions"]
3V["attribution-override.ts"]
3W["index.ts"]
3X["nullify-transaction.ts"]
3Y["transaction-deletion.service.ts"]
end
subgraph 3Z["transfers"]
40["index.ts"]
41["transfer-cleanup.service.ts"]
42["transfer-linking.service.ts"]
43["transfer-matching.service.ts"]
44["transfer-matching.ts"]
end
end
subgraph 45["feature-accounts"]
46["account-card-vm.ts"]
47["account-icons.ts"]
48["account-list-order.ts"]
49["account-types.ts"]
4A["accounts.routes.ts"]
4B["balance-day-tooltip.ts"]
4C["balance-trend-signals.ts"]
subgraph 4D["components"]
subgraph 4E["account-balance-block"]
4F["account-balance-block.component.ts"]
end
subgraph 4G["account-balance-chart"]
4H["account-balance-chart.component.ts"]
end
subgraph 4I["account-balance-history-chart"]
4J["account-balance-history-chart.component.ts"]
end
subgraph 4K["account-card"]
4L["account-card.component.ts"]
end
subgraph 4M["account-form"]
4N["account-form.component.ts"]
end
subgraph 4O["accounts-detail"]
4P["accounts-detail.component.ts"]
end
subgraph 4Q["accounts-overview"]
4R["accounts-overview.component.ts"]
end
4S["index.ts"]
end
4T["index.ts"]
4U["last-import-status.ts"]
end
subgraph 4V["feature-categories"]
4W["categories.routes.ts"]
4X["category-icons.ts"]
4Y["category-model.service.ts"]
4Z["category-model.store.ts"]
50["category-row-vm.ts"]
subgraph 51["components"]
subgraph 52["categories-overview"]
53["categories-overview.component.ts"]
end
subgraph 54["category-form"]
55["category-form.component.ts"]
end
56["index.ts"]
subgraph 57["rule-condition-row"]
58["rule-condition-row.component.ts"]
end
subgraph 59["rule-filters"]
5A["rule-filters.component.ts"]
end
subgraph 5B["rule-form"]
5C["rule-form.component.ts"]
end
subgraph 5D["rule-share-bar"]
5E["rule-share-bar.component.ts"]
end
subgraph 5F["rules-overview"]
5G["rules-overview.component.ts"]
end
end
5H["index.ts"]
5I["rule-condition-editor.ts"]
5J["rule-filters.ts"]
5K["rule-labels.ts"]
5L["rule-share.ts"]
5M["rule-summary.ts"]
5N["rules.store.ts"]
end
subgraph 5O["feature-changelog"]
5P["changelog.routes.ts"]
subgraph 5Q["components"]
subgraph 5R["changelog-entry-row"]
5S["changelog-entry-row.component.ts"]
end
subgraph 5T["changelog-page"]
5U["changelog-page.component.ts"]
end
5V["index.ts"]
end
subgraph 5W["data"]
5X["changelog-entries.ts"]
5Y["roadmap-entries.ts"]
end
5Z["group-changelog-entries.ts"]
60["group-roadmap-entries.ts"]
61["index.ts"]
end
subgraph 62["feature-dashboard"]
63["category-comparison-settings.store.ts"]
64["category-comparison-vm.ts"]
subgraph 65["components"]
subgraph 66["account-balance-strip"]
67["account-balance-strip.component.ts"]
end
subgraph 68["action-queue-panel"]
69["action-queue-panel.component.ts"]
end
subgraph 6A["category-breakdown-panel"]
6B["category-breakdown-panel.component.ts"]
end
subgraph 6C["category-comparison-panel"]
6D["category-comparison-panel.component.ts"]
end
subgraph 6E["category-exclusion-dropdown"]
6F["category-exclusion-dropdown.component.ts"]
end
subgraph 6G["comparison-category-card"]
6H["comparison-category-card.component.ts"]
end
subgraph 6I["dashboard-customize-panel"]
6J["dashboard-customize-panel.component.ts"]
end
subgraph 6K["dashboard-overview"]
6L["dashboard-overview.component.ts"]
end
6M["index.ts"]
subgraph 6N["spending-heatmap-panel"]
6O["spending-heatmap-panel.component.ts"]
end
subgraph 6P["top-transactions-panel"]
6Q["top-transactions-panel.component.ts"]
end
subgraph 6R["trend-chart-panel"]
6S["trend-chart-panel.component.ts"]
end
subgraph 6T["weekday-weekend-split-panel"]
6U["weekday-weekend-split-panel.component.ts"]
end
end
6V["dashboard-layout-settings.store.ts"]
6W["dashboard-row-order.ts"]
6X["dashboard.routes.ts"]
6Y["index.ts"]
6Z["stats.store.ts"]
end
subgraph 70["feature-data-management"]
subgraph 71["components"]
subgraph 72["data-management-overview"]
73["data-management-overview.component.ts"]
end
74["index.ts"]
end
75["index.ts"]
end
subgraph 76["feature-explore"]
subgraph 77["components"]
subgraph 78["explore-overview"]
79["explore-overview.component.ts"]
end
7A["index.ts"]
subgraph 7B["money-flow-panel"]
7C["money-flow-panel.component.ts"]
end
subgraph 7D["spending-mosaic-panel"]
7E["spending-mosaic-panel.component.ts"]
end
end
7F["explore.routes.ts"]
7G["index.ts"]
end
subgraph 7H["feature-future"]
subgraph 7I["components"]
subgraph 7J["forecast-controls"]
7K["forecast-controls.component.ts"]
end
subgraph 7L["forecast-notice"]
7M["forecast-notice.component.ts"]
end
subgraph 7N["future-overview"]
7O["future-overview.component.ts"]
end
subgraph 7P["goal-form"]
7Q["goal-form.component.ts"]
end
subgraph 7R["goal-row"]
7S["goal-row.component.ts"]
end
subgraph 7T["goals-panel"]
7U["goals-panel.component.ts"]
end
7V["index.ts"]
subgraph 7W["net-worth-projection-chart"]
7X["net-worth-projection-chart.component.ts"]
end
subgraph 7Y["projection-figure-table"]
7Z["projection-figure-table.component.ts"]
end
end
80["forecast-chart-copy.ts"]
81["forecast-controls-vm.ts"]
82["forecast-notices.ts"]
83["forecast.store.ts"]
84["future.routes.ts"]
85["goal-row-vm.ts"]
86["index.ts"]
87["net-worth-projection-chart-option.ts"]
88["projection-accessible-row.ts"]
end
subgraph 89["feature-help"]
subgraph 8A["components"]
subgraph 8B["faq-page"]
8C["faq-page.component.ts"]
end
subgraph 8D["guide-detail"]
8E["guide-detail.component.ts"]
end
subgraph 8F["guide-steps"]
8G["guide-steps.component.ts"]
end
subgraph 8H["guides-index"]
8I["guides-index.component.ts"]
end
8J["index.ts"]
end
subgraph 8K["data"]
8L["faq.ts"]
8M["guides.ts"]
end
8N["help.routes.ts"]
8O["index.ts"]
end
subgraph 8P["feature-home"]
subgraph 8Q["components"]
subgraph 8R["home-landing"]
8S["home-landing.component.ts"]
end
8T["index.ts"]
end
8U["home.routes.ts"]
8V["index.ts"]
end
subgraph 8W["feature-import"]
8X["column-mapping.ts"]
subgraph 8Y["components"]
subgraph 8Z["account-draft-editor"]
90["account-draft-editor.component.ts"]
end
subgraph 91["batch-wait-card"]
92["batch-wait-card.component.ts"]
end
subgraph 93["column-map-amount-field"]
94["column-map-amount-field.component.ts"]
end
subgraph 95["column-map-counterparty-field"]
96["column-map-counterparty-field.component.ts"]
end
subgraph 97["column-map-sample-caption"]
98["column-map-sample-caption.component.ts"]
end
subgraph 99["column-map-simple-field"]
9A["column-map-simple-field.component.ts"]
end
subgraph 9B["column-map-stepper"]
9C["column-map-stepper.component.ts"]
end
subgraph 9D["column-map-summary-step"]
9E["column-map-summary-step.component.ts"]
end
subgraph 9F["import-map-step"]
9G["import-map-step.component.ts"]
end
subgraph 9H["import-preview-step"]
9I["import-preview-step.component.ts"]
end
subgraph 9J["import-select-step"]
9K["import-select-step.component.ts"]
end
subgraph 9L["import-summary-step"]
9M["import-summary-step.component.ts"]
end
subgraph 9N["import-wizard"]
9O["import-wizard.component.ts"]
end
9P["index.ts"]
subgraph 9Q["queued-file-row"]
9R["queued-file-row.component.ts"]
end
end
9S["duplicate-scan.ts"]
9T["import-queue.ts"]
9U["import-wizard-session.ts"]
9V["import.routes.ts"]
9W["index.ts"]
9X["mapper-steps.ts"]
9Y["mapping-profiles.store.ts"]
end
subgraph 9Z["feature-income"]
A0["career-start-date.ts"]
subgraph A1["components"]
subgraph A2["income-career-start"]
A3["income-career-start.component.ts"]
end
subgraph A4["income-category-checklist"]
A5["income-category-checklist.component.ts"]
end
subgraph A6["income-chart-cell"]
A7["income-chart-cell.component.ts"]
end
subgraph A8["income-events-sidebar"]
A9["income-events-sidebar.component.ts"]
end
subgraph AA["income-gross-color"]
AB["income-gross-color.component.ts"]
end
subgraph AC["income-gross-net-section"]
AD["income-gross-net-section.component.ts"]
end
subgraph AE["income-growth-panel"]
AF["income-growth-panel.component.ts"]
end
subgraph AG["income-inference-note"]
AH["income-inference-note.component.ts"]
end
subgraph AI["income-lump-sum-checklist"]
AJ["income-lump-sum-checklist.component.ts"]
end
subgraph AK["income-main-category"]
AL["income-main-category.component.ts"]
end
subgraph AM["income-overview"]
AN["income-overview.component.ts"]
end
subgraph AO["income-settings-page"]
AP["income-settings-page.component.ts"]
end
subgraph AQ["income-yearly-panel"]
AR["income-yearly-panel.component.ts"]
end
AS["index.ts"]
subgraph AT["salary-details-page"]
AU["salary-details-page.component.ts"]
end
subgraph AV["salary-metadata-table"]
AW["salary-metadata-table.component.ts"]
end
subgraph AX["salary-month-modal"]
AY["salary-month-modal.component.ts"]
end
end
AZ["gross-net-chart-options.ts"]
B0["income-category-vm.ts"]
B1["income-event-vm.ts"]
B2["income-granularity.ts"]
B3["income.routes.ts"]
B4["income.store.ts"]
B5["index.ts"]
B6["salary-metadata-edit.ts"]
B7["salary-metadata-rows.ts"]
end
subgraph B8["feature-learning"]
subgraph B9["components"]
BA["index.ts"]
subgraph BB["learning-overview"]
BC["learning-overview.component.ts"]
end
subgraph BD["model-status-badge"]
BE["model-status-badge.component.ts"]
end
subgraph BF["model-status"]
BG["model-status.component.ts"]
end
subgraph BH["rule-proposals"]
BI["rule-proposals.component.ts"]
end
subgraph BJ["suggestions-table"]
BK["suggestions-table.component.ts"]
end
end
BL["index.ts"]
BM["learning.routes.ts"]
BN["model-status-display.ts"]
end
subgraph BO["feature-loans"]
subgraph BP["components"]
BQ["index.ts"]
subgraph BR["loan-amortization-table"]
BS["loan-amortization-table.component.ts"]
end
subgraph BT["loan-balance-chart"]
BU["loan-balance-chart.component.ts"]
end
subgraph BV["loan-card"]
BW["loan-card.component.ts"]
end
subgraph BX["loan-composition-chart"]
BY["loan-composition-chart.component.ts"]
end
subgraph BZ["loan-detail"]
C0["loan-detail.component.ts"]
end
subgraph C1["loan-form"]
C2["loan-form.component.ts"]
end
subgraph C3["loan-payments-list"]
C4["loan-payments-list.component.ts"]
end
subgraph C5["loan-what-if"]
C6["loan-what-if.component.ts"]
end
subgraph C7["loans-overview"]
C8["loans-overview.component.ts"]
end
end
C9["index.ts"]
CA["loan-card-vm.ts"]
CB["loan-schedule-status.ts"]
CC["loan-types.ts"]
CD["loan-what-if-vm.ts"]
CE["loans.routes.ts"]
CF["loans.store.ts"]
end
subgraph CG["feature-recurring"]
CH["bills-calendar-vm.ts"]
subgraph CI["components"]
subgraph CJ["bills-calendar"]
CK["bills-calendar.component.ts"]
end
subgraph CL["bills-day-list"]
CM["bills-day-list.component.ts"]
end
subgraph CN["bills-month-grid"]
CO["bills-month-grid.component.ts"]
end
CP["index.ts"]
subgraph CQ["recurring-dismissed-list"]
CR["recurring-dismissed-list.component.ts"]
end
subgraph CS["recurring-overview"]
CT["recurring-overview.component.ts"]
end
subgraph CU["recurring-payments-panel"]
CV["recurring-payments-panel.component.ts"]
end
end
CW["index.ts"]
CX["recurring-overrides.ts"]
CY["recurring-payments-row-vm.ts"]
CZ["recurring-series.store.ts"]
D0["recurring.routes.ts"]
end
subgraph D1["feature-settings"]
subgraph D2["components"]
D3["index.ts"]
subgraph D4["settings-about-section"]
D5["settings-about-section.component.ts"]
end
subgraph D6["settings-currency-locale-section"]
D7["settings-currency-locale-section.component.ts"]
end
subgraph D8["settings-data-section"]
D9["settings-data-section.component.ts"]
end
subgraph DA["settings-overview"]
DB["settings-overview.component.ts"]
end
subgraph DC["settings-privacy-section"]
DD["settings-privacy-section.component.ts"]
end
subgraph DE["settings-reporting-section"]
DF["settings-reporting-section.component.ts"]
end
subgraph DG["settings-theme-section"]
DH["settings-theme-section.component.ts"]
end
end
DI["index.ts"]
DJ["settings.routes.ts"]
end
subgraph DK["feature-transactions"]
DL["category-picker.ts"]
subgraph DM["components"]
subgraph DN["attribution-override-fieldset"]
DO["attribution-override-fieldset.component.ts"]
end
subgraph DP["category-select-cell"]
DQ["category-select-cell.component.ts"]
end
DR["index.ts"]
subgraph DS["transaction-bulk-bar"]
DT["transaction-bulk-bar.component.ts"]
end
subgraph DU["transaction-card"]
DV["transaction-card.component.ts"]
end
subgraph DW["transaction-edit-form"]
DX["transaction-edit-form.component.ts"]
end
subgraph DY["transaction-filters"]
DZ["transaction-filters.component.ts"]
end
subgraph E0["transaction-row"]
E1["transaction-row.component.ts"]
end
subgraph E2["transactions-overview"]
E3["transactions-overview.component.ts"]
end
subgraph E4["transfer-review"]
E5["transfer-review.component.ts"]
end
end
E6["index.ts"]
E7["transaction-filters.ts"]
E8["transaction-row-vm.ts"]
E9["transactions.routes.ts"]
EA["transfer-label.ts"]
end
subgraph EB["shared"]
subgraph EC["echarts"]
ED["bucketed-axis-option.ts"]
EE["chart-theme.ts"]
EF["echarts-jsdom.testing.ts"]
EG["echarts-setup.ts"]
EH["index.ts"]
EI["legend-option.ts"]
EJ["tooltip-formatter.ts"]
end
subgraph EK["ui"]
subgraph EL["absolute-range-panel"]
EM["absolute-range-panel.component.ts"]
end
subgraph EN["alert"]
EO["alert.component.ts"]
end
subgraph EP["badge"]
EQ["badge.component.ts"]
end
subgraph ER["button"]
ES["button.component.ts"]
end
subgraph ET["collapse"]
EU["collapse.component.ts"]
end
subgraph EV["confirm-dialog"]
EW["confirm-dialog.component.ts"]
end
subgraph EX["cycle-picker"]
EY["cycle-picker.component.ts"]
end
subgraph EZ["date-range-input"]
F0["date-range-input.component.ts"]
end
subgraph F1["divider"]
F2["divider.component.ts"]
end
subgraph F3["dropdown"]
F4["dropdown.component.ts"]
end
subgraph F5["empty-state"]
F6["empty-state.component.ts"]
end
subgraph F7["fieldset"]
F8["fieldset.component.ts"]
end
subgraph F9["flex"]
FA["flex.component.ts"]
end
subgraph FB["granularity-picker"]
FC["granularity-picker.component.ts"]
end
FD["index.ts"]
subgraph FE["input"]
FF["input.component.ts"]
end
subgraph FG["label"]
FH["label.component.ts"]
end
subgraph FI["loading-skeleton"]
FJ["loading-skeleton.component.ts"]
end
subgraph FK["modal"]
FL["mm-modal.component.ts"]
end
subgraph FM["page-header"]
FN["page-header.component.ts"]
end
subgraph FO["paginator"]
FP["paginator.component.ts"]
end
subgraph FQ["paper"]
FR["paper.component.ts"]
end
subgraph FS["privacy-blur"]
FT["privacy-blur.component.ts"]
end
subgraph FU["privacy-toggle"]
FV["privacy-toggle.component.ts"]
end
subgraph FW["range-picker"]
FX["range-picker.component.ts"]
end
subgraph FY["select"]
FZ["select.component.ts"]
end
subgraph G0["stat-card"]
G1["stat-card.component.ts"]
end
subgraph G2["table"]
G3["table.component.ts"]
end
subgraph G4["tabs"]
G5["tabs.component.ts"]
end
subgraph G6["typography"]
G7["typography.component.ts"]
end
end
subgraph G8["utils"]
G9["calendar-cycles.ts"]
GA["compact-viewport.ts"]
GB["confidence-color.ts"]
GC["confirm-state.ts"]
GD["currency-format.ts"]
GE["currency-symbol-presets.ts"]
GF["daisy-classes.ts"]
GG["date-buckets.ts"]
GH["date-format.pipe.ts"]
GI["date-format.ts"]
GJ["debounced-text.ts"]
GK["download-json.ts"]
GL["fingerprint.ts"]
GM["format-settings.testing.ts"]
GN["format-settings.ts"]
GO["hidden-amount.ts"]
GP["iban.ts"]
GQ["index.ts"]
GR["link-control-to-setting.ts"]
GS["locale-presets.ts"]
GT["money-color.ts"]
GU["number-format.ts"]
GV["pagination.ts"]
GW["percentage.ts"]
GX["quick-ranges.ts"]
GY["range-expression.ts"]
GZ["search-params.ts"]
H0["selection-model.ts"]
H1["signed-amount.pipe.ts"]
H2["sortable.ts"]
H3["structural-filters.ts"]
H4["theme-hooks.ts"]
H5["unrendered-markup.testing.ts"]
subgraph H6["validators"]
H7["iban.validator.ts"]
H8["percentage.validator.ts"]
end
H9["with-archivable.ts"]
HA["with-persisted-crud.ts"]
end
end
end
end
4-->Q
4-->40
5-->4
5-->6
6-->Q
6-->GQ
8-->Q
9-->A
9-->Q
A-->5
A-->Q
B-->8
B-->A
B-->9
B-->C
B-->D
C-->Q
D-->C
D-->Q
F-->G
G-->1Q
G-->3F
G-->3Q
G-->GL
G-->GN
H-->G
H-->3R
H-->GQ
I-->G
J-->G
K-->G
L-->G
M-->G
N-->G
N-->32
O-->G
P-->G
Q-->F
Q-->G
Q-->H
Q-->I
Q-->J
Q-->K
Q-->L
Q-->M
Q-->N
Q-->O
Q-->P
Q-->R
Q-->S
Q-->T
Q-->U
Q-->V
Q-->W
Q-->X
Q-->Y
R-->G
S-->G
T-->G
U-->G
V-->G
W-->G
X-->G
Y-->G
10-->Q
10-->GQ
11-->15
11-->Q
12-->14
12-->15
13-->12
13-->15
14-->Q
15-->14
15-->Q
17-->14
17-->Q
17-->40
17-->GQ
18-->10
18-->11
18-->12
18-->14
18-->15
18-->16
18-->17
1B-->27
1B-->ES
1B-->G7
1C-->1B
1F-->1E
1H-->GQ
1I-->1H
1I-->1J
1I-->1K
1J-->1H
1J-->Q
1J-->GQ
1K-->1H
1K-->1J
1K-->Q
1M-->1N
1M-->1O
1M-->1Q
1N-->1Q
1O-->1Q
1P-->1N
1P-->1O
1P-->1Q
1P-->1R
1P-->1S
1R-->B
1R-->Q
1U-->1X
1V-->1U
1V-->1W
1V-->1X
1W-->1X
1Z-->21
1Z-->2A
1Z-->2C
1Z-->5
1Z-->Q
1Z-->32
1Z-->40
1Z-->GQ
20-->Q
20-->3R
20-->GQ
21-->2A
21-->Q
21-->GQ
22-->23
22-->32
22-->EH
22-->GQ
23-->32
23-->EH
23-->GQ
24-->Q
24-->32
25-->Q
25-->GQ
26-->2A
26-->2C
26-->B
26-->Q
26-->18
27-->1Z
27-->20
27-->21
27-->22
27-->23
27-->24
27-->25
27-->26
27-->28
27-->29
27-->2A
27-->2B
27-->2C
28-->1Z
28-->20
28-->29
28-->2A
28-->Q
28-->32
28-->GQ
29-->20
29-->GQ
2A-->Q
2A-->3W
2A-->40
2B-->Q
2C-->2A
2C-->2B
2C-->Q
2C-->40
2E-->Q
2E-->GQ
2F-->2E
2F-->Q
2F-->GQ
2G-->2I
2G-->2X
2G-->GQ
2H-->2O
2H-->Q
2I-->2H
2I-->Q
2I-->GQ
2J-->2H
2J-->2O
2J-->Q
2J-->GQ
2K-->2O
2K-->Q
2L-->Q
2M-->2H
2M-->3A
2M-->Q
2N-->GQ
2O-->2L
2O-->2P
2O-->Q
2O-->40
2P-->5
2P-->Q
2Q-->Q
2R-->2I
2R-->2X
2R-->Q
2R-->GQ
2S-->Q
2T-->Q
2T-->GQ
2U-->GQ
2V-->2W
2W-->2X
2W-->Q
2X-->2I
2X-->Q
2X-->GQ
2Y-->2Z
2Y-->31
2Y-->3I
2Y-->Q
2Z-->2I
2Z-->2X
2Z-->GQ
30-->2X
30-->3K
30-->GQ
31-->2I
31-->2R
31-->2X
31-->GQ
32-->2E
32-->2F
32-->2G
32-->2H
32-->2I
32-->2J
32-->2K
32-->2L
32-->2M
32-->2N
32-->2O
32-->2P
32-->2Q
32-->2R
32-->2S
32-->2T
32-->2U
32-->2V
32-->2W
32-->2X
32-->2Y
32-->2Z
32-->30
32-->31
32-->33
32-->34
32-->35
32-->36
32-->37
32-->38
32-->39
32-->3A
32-->3B
32-->3C
32-->3D
32-->3E
32-->3F
32-->3G
32-->3H
32-->3I
32-->3J
32-->3K
32-->3L
33-->2P
33-->Q
34-->2P
34-->5
34-->Q
35-->Q
35-->GQ
36-->3L
38-->GQ
39-->2O
39-->Q
3A-->GQ
3B-->GQ
3C-->2O
3C-->B
3C-->Q
3C-->GQ
3D-->3C
3D-->GQ
3E-->Q
3E-->GQ
3F-->39
3F-->Q
3F-->GQ
3G-->2H
3G-->2K
3G-->Q
3H-->Q
3H-->40
3I-->2W
3J-->2O
3J-->Q
3J-->GQ
3K-->39
3K-->Q
3L-->2I
3L-->Q
3L-->GQ
3N-->3O
3Q-->3S
3R-->3Q
3R-->3S
3R-->3T
3T-->3S
3V-->Q
3W-->3V
3W-->3X
3W-->3Y
3X-->Q
3Y-->Q
3Y-->40
40-->41
40-->42
40-->44
40-->43
41-->Q
42-->44
42-->Q
43-->42
43-->44
43-->Q
44-->6
44-->Q
44-->GQ
46-->4U
46-->Q
47-->Q
48-->Q
49-->Q
4A-->4P
4A-->4R
4A-->EH
4B-->32
4B-->GQ
4C-->Q
4C-->27
4C-->32
4C-->GQ
4F-->FD
4F-->GQ
4H-->4B
4H-->4C
4H-->Q
4H-->32
4H-->EH
4H-->FD
4H-->GQ
4J-->4B
4J-->4C
4J-->Q
4J-->27
4J-->32
4J-->EH
4J-->FD
4L-->46
4L-->47
4L-->4F
4L-->FD
4N-->47
4N-->49
4N-->Q
4N-->FD
4N-->GQ
4N-->H7
4N-->H8
4P-->4U
4P-->4F
4P-->4H
4P-->4N
4P-->27
4P-->FD
4P-->GQ
4R-->46
4R-->47
4R-->48
4R-->4U
4R-->4J
4R-->4L
4R-->4N
4R-->Q
4R-->27
4R-->FD
4R-->GQ
4S-->4F
4S-->4H
4S-->4J
4S-->4L
4S-->4N
4S-->4P
4S-->4R
4T-->47
4T-->49
4T-->4A
4T-->4S
4U-->GQ
4W-->53
4W-->5G
4Y-->1P
4Z-->4Y
4Z-->5N
4Z-->Q
4Z-->1P
4Z-->27
50-->Q
50-->FD
53-->4X
53-->50
53-->55
53-->B
53-->Q
53-->27
53-->FD
53-->GQ
55-->4X
55-->Q
55-->FD
56-->53
56-->55
56-->5A
56-->5C
56-->5E
56-->5G
58-->5I
58-->5K
58-->B
58-->Q
58-->27
58-->FD
5A-->5J
5A-->27
5A-->FD
5A-->GQ
5C-->5I
5C-->58
5C-->B
5C-->Q
5C-->27
5C-->FD
5E-->5L
5E-->5N
5E-->FD
5E-->GQ
5G-->5J
5G-->5M
5G-->5N
5G-->5A
5G-->5C
5G-->5E
5G-->Q
5G-->27
5G-->FD
5G-->GQ
5H-->4W
5H-->4X
5H-->4Y
5H-->4Z
5H-->56
5H-->5J
5H-->5M
5H-->5N
5I-->5K
5I-->Q
5J-->5M
5J-->Q
5K-->Q
5L-->Q
5M-->5K
5M-->Q
5N-->5L
5N-->B
5N-->Q
5N-->27
5N-->GQ
5P-->5U
5S-->FD
5U-->5X
5U-->5Y
5U-->5Z
5U-->60
5U-->5S
5U-->FD
5V-->5U
5Z-->5X
60-->5Y
61-->5P
61-->5V
63-->Q
67-->27
67-->FD
67-->GQ
69-->27
69-->40
69-->FD
69-->GQ
6B-->6Z
6B-->27
6B-->32
6B-->EH
6B-->FD
6B-->GQ
6D-->63
6D-->64
6D-->6Z
6D-->6F
6D-->6H
6D-->27
6D-->FD
6D-->GQ
6F-->27
6F-->FD
6H-->64
6H-->FD
6J-->6V
6J-->6W
6J-->Q
6J-->FD
6L-->6V
6L-->6W
6L-->6Z
6L-->67
6L-->69
6L-->6B
6L-->6D
6L-->6J
6L-->6O
6L-->6Q
6L-->6S
6L-->6U
6L-->27
6L-->32
6L-->FD
6L-->GQ
6M-->67
6M-->69
6M-->6B
6M-->6D
6M-->6F
6M-->6J
6M-->6L
6M-->6O
6M-->6Q
6M-->6S
6M-->6U
6O-->6F
6O-->27
6O-->32
6O-->40
6O-->EH
6O-->FD
6O-->GQ
6Q-->6Z
6Q-->27
6Q-->FD
6Q-->GQ
6S-->27
6S-->32
6S-->40
6S-->EH
6S-->FD
6S-->GQ
6U-->6Z
6U-->27
6U-->FD
6U-->GQ
6V-->6W
6V-->Q
6W-->Q
6X-->6L
6X-->EH
6Y-->6M
6Y-->6X
6Z-->63
6Z-->27
6Z-->32
6Z-->40
73-->Q
73-->3N
73-->FD
73-->GQ
74-->73
75-->74
79-->7C
79-->7E
79-->27
79-->FD
7A-->79
7A-->7C
7A-->7E
7C-->27
7C-->32
7C-->EH
7C-->FD
7C-->GQ
7E-->Q
7E-->27
7E-->32
7E-->40
7E-->EH
7E-->FD
7E-->GQ
7F-->79
7F-->EH
7G-->7A
7G-->7F
7K-->81
7K-->83
7K-->Q
7K-->27
7K-->32
7K-->FD
7K-->GQ
7M-->82
7M-->FD
7O-->7K
7O-->7U
7O-->7X
7O-->FD
7Q-->Q
7Q-->FD
7S-->85
7S-->FD
7U-->82
7U-->83
7U-->85
7U-->7M
7U-->7Q
7U-->7S
7U-->Q
7U-->27
7U-->FD
7U-->GQ
7V-->7K
7V-->7M
7V-->7O
7V-->7Q
7V-->7S
7V-->7U
7V-->7X
7V-->7Z
7X-->80
7X-->83
7X-->87
7X-->88
7X-->7Z
7X-->27
7X-->FD
7X-->GQ
7Z-->88
80-->Q
81-->Q
81-->32
81-->FD
81-->GQ
82-->80
82-->Q
82-->32
82-->FD
82-->GQ
83-->Q
83-->27
83-->32
83-->40
84-->7O
84-->EH
85-->Q
85-->32
85-->FD
85-->GQ
86-->7V
86-->80
86-->81
86-->82
86-->83
86-->84
86-->85
86-->87
86-->88
87-->32
87-->EH
87-->GQ
8C-->8L
8C-->FD
8E-->8M
8E-->8G
8E-->FD
8G-->8M
8G-->FD
8I-->8M
8I-->FD
8J-->8C
8J-->8E
8J-->8G
8J-->8I
8N-->8C
8N-->8E
8N-->8I
8O-->8J
8O-->8M
8O-->8N
8S-->1F
8S-->FD
8T-->8S
8U-->8S
8V-->8T
8V-->8U
8X-->Q
90-->9T
90-->4T
90-->FD
92-->FD
94-->98
94-->FD
96-->98
96-->FD
98-->FD
9A-->8X
9A-->98
9A-->FD
9C-->9X
9C-->FD
9E-->8X
9E-->FD
9G-->8X
9G-->9S
9G-->9X
9G-->9Y
9G-->94
9G-->96
9G-->9A
9G-->9C
9G-->9E
9G-->9I
9G-->Q
9G-->18
9G-->FD
9I-->9S
9I-->18
9I-->FD
9I-->GQ
9K-->9T
9K-->9Y
9K-->9R
9K-->Q
9K-->18
9K-->FD
9M-->Q
9M-->FD
9O-->9U
9O-->92
9O-->9G
9O-->9K
9O-->9M
9O-->27
9O-->FD
9P-->9G
9P-->9I
9P-->9K
9P-->9M
9P-->9O
9R-->9T
9R-->90
9R-->Q
9R-->FD
9S-->18
9S-->FD
9T-->Q
9U-->8X
9U-->9S
9U-->9T
9U-->9Y
9U-->Q
9U-->18
9U-->27
9U-->4T
9V-->9O
9W-->8X
9W-->9P
9W-->9T
9W-->9V
9W-->9X
9W-->9Y
9X-->8X
9Y-->Q
A0-->32
A3-->A0
A3-->B4
A3-->FD
A3-->GQ
A5-->B0
A5-->FD
A7-->FD
A9-->B1
A9-->B2
A9-->B4
A9-->27
A9-->32
A9-->FD
AB-->B4
AB-->3R
AB-->EH
AB-->FD
AD-->AZ
AD-->B4
AD-->A7
AD-->27
AD-->32
AD-->EH
AD-->FD
AD-->GQ
AF-->B2
AF-->B4
AF-->A3
AF-->AH
AF-->27
AF-->32
AF-->FD
AF-->GQ
AH-->FD
AJ-->B0
AJ-->B4
AJ-->A5
AL-->B0
AL-->B4
AL-->FD
AN-->B4
AN-->B7
AN-->A9
AN-->AD
AN-->AF
AN-->AH
AN-->AJ
AN-->AL
AN-->AR
AN-->AY
AN-->27
AN-->32
AN-->EH
AN-->FD
AN-->GQ
AP-->B0
AP-->B4
AP-->A3
AP-->A5
AP-->AB
AP-->AJ
AP-->AL
AP-->FD
AR-->B4
AR-->27
AR-->32
AR-->40
AR-->EH
AR-->FD
AR-->GQ
AS-->A3
AS-->A5
AS-->A7
AS-->A9
AS-->AD
AS-->AF
AS-->AL
AS-->AN
AS-->AP
AS-->AR
AS-->AU
AS-->AW
AS-->AY
AU-->AW
AU-->FD
AW-->B2
AW-->B4
AW-->B6
AW-->B7
AW-->FD
AW-->GQ
AY-->B4
AY-->B6
AY-->B7
AY-->AW
AY-->Q
AY-->FD
AZ-->32
AZ-->EH
AZ-->GQ
B0-->Q
B1-->B2
B1-->Q
B1-->32
B1-->FD
B1-->GQ
B2-->GQ
B3-->AN
B3-->AP
B3-->AU
B3-->EH
B4-->A0
B4-->B2
B4-->Q
B4-->27
B4-->32
B4-->3R
B4-->40
B5-->AS
B5-->B0
B5-->B2
B5-->B3
B5-->B4
B5-->B6
B5-->B7
B6-->Q
B7-->Q
BA-->BC
BA-->BG
BA-->BI
BA-->BK
BC-->BE
BC-->BG
BC-->BI
BC-->BK
BC-->FD
BE-->BN
BE-->5H
BE-->FD
BG-->BN
BG-->1P
BG-->27
BG-->5H
BG-->FD
BI-->Q
BI-->1P
BI-->27
BI-->5H
BI-->FD
BI-->GQ
BK-->Q
BK-->27
BK-->5H
BK-->FD
BK-->GQ
BL-->BA
BL-->BM
BM-->BC
BN-->5H
BN-->FD
BQ-->BS
BQ-->BU
BQ-->BW
BQ-->BY
BQ-->C0
BQ-->C2
BQ-->C4
BQ-->C6
BQ-->C8
BS-->BY
BS-->Q
BS-->1I
BS-->FD
BS-->GQ
BU-->Q
BU-->1I
BU-->EH
BU-->FD
BW-->CA
BW-->FD
BY-->Q
BY-->1I
BY-->EH
BY-->FD
BY-->GQ
C0-->CB
C0-->CF
C0-->BS
C0-->BU
C0-->C2
C0-->C4
C0-->C6
C0-->1I
C0-->27
C0-->FD
C2-->CC
C2-->Q
C2-->27
C2-->FD
C2-->H8
C4-->Q
C4-->27
C4-->FD
C4-->GQ
C6-->CD
C6-->Q
C6-->1I
C6-->27
C6-->EH
C6-->FD
C8-->CA
C8-->CF
C8-->BW
C8-->C2
C8-->Q
C8-->FD
C9-->BQ
C9-->CA
C9-->CB
C9-->CC
C9-->CD
C9-->CE
C9-->CF
CA-->CB
CA-->CC
CA-->Q
CA-->1I
CA-->FD
CA-->GQ
CB-->1I
CB-->FD
CB-->GQ
CC-->Q
CD-->1I
CD-->GQ
CE-->C0
CE-->C8
CE-->EH
CF-->Q
CF-->1I
CF-->27
CF-->GQ
CH-->FD
CK-->CH
CK-->CZ
CK-->CM
CK-->CO
CK-->27
CK-->32
CK-->FD
CK-->GQ
CM-->CH
CM-->FD
CO-->CH
CO-->FD
CP-->CK
CP-->CM
CP-->CO
CP-->CT
CP-->CV
CR-->FD
CT-->CK
CT-->CV
CT-->27
CT-->FD
CV-->CY
CV-->CZ
CV-->CR
CV-->27
CV-->32
CV-->EH
CV-->FD
CV-->GQ
CW-->CP
CW-->D0
CX-->Q
CX-->32
CY-->FD
CZ-->CX
CZ-->Q
CZ-->27
CZ-->32
CZ-->40
D0-->CT
D3-->DB
D5-->1F
D5-->FD
D7-->27
D7-->FD
D7-->GQ
D9-->75
D9-->FD
DB-->D5
DB-->D7
DB-->D9
DB-->DD
DB-->DF
DB-->DH
DB-->FD
DD-->27
DD-->FD
DD-->GQ
DF-->27
DF-->FD
DF-->GQ
DH-->27
DH-->3R
DH-->FD
DI-->D3
DI-->DJ
DJ-->DB
DL-->Q
DO-->Q
DO-->27
DO-->3W
DO-->FD
DO-->GQ
DQ-->DL
DR-->DT
DR-->DX
DR-->DZ
DR-->E3
DR-->E5
DT-->DL
DT-->B
DT-->27
DT-->FD
DV-->DL
DV-->E8
DV-->DQ
DV-->FD
DV-->GQ
DX-->DL
DX-->DO
DX-->B
DX-->Q
DX-->27
DX-->3W
DX-->5H
DX-->FD
DX-->GQ
DZ-->DL
DZ-->E7
DZ-->B
DZ-->27
DZ-->FD
DZ-->GQ
E1-->DL
E1-->E8
E1-->DQ
E1-->FD
E1-->GQ
E3-->DL
E3-->E7
E3-->E8
E3-->EA
E3-->DT
E3-->DV
E3-->DX
E3-->DZ
E3-->E1
E3-->E5
E3-->B
E3-->Q
E3-->27
E3-->40
E3-->5H
E3-->FD
E3-->GQ
E5-->27
E5-->40
E5-->FD
E5-->GQ
E6-->E9
E7-->Q
E7-->40
E8-->Q
E8-->GQ
E9-->E3
EA-->Q
EH-->ED
EH-->EE
EH-->EG
EH-->EI
EH-->EJ
EJ-->GQ
EM-->ES
EM-->F6
EM-->F8
EM-->FF
EM-->G7
EM-->Q
EM-->GQ
EO-->GQ
EQ-->GQ
ES-->GQ
EU-->GQ
EW-->ES
EW-->FH
EW-->FL
EW-->G7
EY-->GQ
F0-->ES
F0-->F4
F0-->GQ
F2-->GQ
F4-->GQ
F6-->FA
F6-->G7
F8-->GQ
FA-->GQ
FD-->EM
FD-->EO
FD-->EQ
FD-->ES
FD-->EU
FD-->EW
FD-->EY
FD-->F0
FD-->F2
FD-->F4
FD-->F6
FD-->F8
FD-->FA
FD-->FC
FD-->FF
FD-->FH
FD-->FJ
FD-->FL
FD-->FN
FD-->FP
FD-->FR
FD-->FT
FD-->FV
FD-->FX
FD-->FZ
FD-->G1
FD-->G3
FD-->G5
FD-->G7
FF-->GQ
FH-->GQ
FJ-->FA
FL-->GQ
FN-->FA
FN-->G7
FP-->ES
FP-->FA
FP-->G7
FP-->GQ
FR-->GQ
FT-->GQ
FV-->ES
FV-->27
FX-->EM
FX-->ES
FX-->FA
FX-->FR
FX-->G7
FX-->Q
FX-->GQ
FZ-->GQ
G1-->FT
G1-->G7
G1-->GQ
G3-->GQ
G5-->GQ
G7-->GQ
G9-->GI
GD-->GN
GG-->GN
GH-->GI
GI-->GN
GM-->GN
GQ-->G9
GQ-->GA
GQ-->GB
GQ-->GC
GQ-->GD
GQ-->GE
GQ-->GF
GQ-->GG
GQ-->GI
GQ-->GH
GQ-->GJ
GQ-->GK
GQ-->GL
GQ-->GN
GQ-->GO
GQ-->GP
GQ-->GR
GQ-->GS
GQ-->GT
GQ-->GU
GQ-->GV
GQ-->GW
GQ-->GX
GQ-->GY
GQ-->GZ
GQ-->H0
GQ-->H1
GQ-->H2
GQ-->H3
GQ-->H4
GQ-->H9
GQ-->HA
GU-->GN
GX-->GG
GX-->GY
GY-->GG
H1-->GD
```
