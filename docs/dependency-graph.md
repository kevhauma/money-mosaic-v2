...
subgraph BV["loans-overview"]
BW["loans-overview.component.ts"]
end
end
BX["index.ts"]
BY["loan-card-vm.ts"]
...
subgraph C2["feature-recurring"]
C3["bills-calendar-vm.ts"]
subgraph C4["components"]
subgraph C5["bills-calendar"]
C6["bills-calendar.component.ts"]
end
subgraph C7["bills-day-list"]
C8["bills-day-list.component.ts"]
end
subgraph C9["bills-month-grid"]
CA["bills-month-grid.component.ts"]
end
CB["index.ts"]
subgraph CC["recurring-overview"]
CD["recurring-overview.component.ts"]
end
subgraph CE["recurring-payments-panel"]
CF["recurring-payments-panel.component.ts"]
end
end
CG["index.ts"]
CH["recurring-payments-row-vm.ts"]
CI["recurring-series.store.ts"]
CJ["recurring.routes.ts"]
end
subgraph CK["feature-settings"]
subgraph CL["components"]
CM["index.ts"]
subgraph CN["settings-about-section"]
CO["settings-about-section.component.ts"]
end
subgraph CP["settings-currency-locale-section"]
CQ["settings-currency-locale-section.component.ts"]
end
subgraph CR["settings-data-section"]
CS["settings-data-section.component.ts"]
end
subgraph CT["settings-overview"]
CU["settings-overview.component.ts"]
end
subgraph CV["settings-privacy-section"]
CW["settings-privacy-section.component.ts"]
end
subgraph CX["settings-reporting-section"]
CY["settings-reporting-section.component.ts"]
end
subgraph CZ["settings-theme-section"]
D0["settings-theme-section.component.ts"]
end
end
D1["index.ts"]
D2["settings.routes.ts"]
end
subgraph D3["feature-transactions"]
D4["category-picker.ts"]
subgraph D5["components"]
subgraph D6["attribution-override-fieldset"]
D7["attribution-override-fieldset.component.ts"]
end
subgraph D8["category-select-cell"]
D9["category-select-cell.component.ts"]
end
DA["index.ts"]
subgraph DB["transaction-bulk-bar"]
DC["transaction-bulk-bar.component.ts"]
end
subgraph DD["transaction-edit-form"]
DE["transaction-edit-form.component.ts"]
end
subgraph DF["transaction-filters"]
DG["transaction-filters.component.ts"]
end
subgraph DH["transaction-row"]
DI["transaction-row.component.ts"]
end
subgraph DJ["transactions-overview"]
DK["transactions-overview.component.ts"]
end
subgraph DL["transfer-review"]
DM["transfer-review.component.ts"]
end
end
DN["index.ts"]
DO["transaction-filters.ts"]
DP["transaction-row-vm.ts"]
DQ["transactions.routes.ts"]
end
subgraph DR["shared"]
subgraph DS["echarts"]
DT["bucketed-axis-option.ts"]
DU["chart-theme.ts"]
DV["echarts-jsdom.testing.ts"]
DW["echarts-setup.ts"]
DX["index.ts"]
DY["legend-option.ts"]
DZ["tooltip-formatter.ts"]
end
subgraph E0["ui"]
subgraph E1["absolute-range-panel"]
E2["absolute-range-panel.component.ts"]
end
subgraph E3["alert"]
E4["alert.component.ts"]
end
subgraph E5["badge"]
E6["badge.component.ts"]
end
subgraph E7["button"]
E8["button.component.ts"]
end
subgraph E9["collapse"]
EA["collapse.component.ts"]
end
subgraph EB["confirm-dialog"]
EC["confirm-dialog.component.ts"]
end
subgraph ED["cycle-picker"]
EE["cycle-picker.component.ts"]
end
subgraph EF["date-range-input"]
EG["date-range-input.component.ts"]
end
subgraph EH["divider"]
EI["divider.component.ts"]
end
subgraph EJ["dropdown"]
EK["dropdown.component.ts"]
end
subgraph EL["empty-state"]
EM["empty-state.component.ts"]
end
subgraph EN["fieldset"]
EO["fieldset.component.ts"]
end
subgraph EP["flex"]
EQ["flex.component.ts"]
end
subgraph ER["granularity-picker"]
ES["granularity-picker.component.ts"]
end
ET["index.ts"]
subgraph EU["input"]
EV["input.component.ts"]
end
subgraph EW["label"]
EX["label.component.ts"]
end
subgraph EY["loading-skeleton"]
EZ["loading-skeleton.component.ts"]
end
subgraph F0["modal"]
F1["mm-modal.component.ts"]
end
subgraph F2["page-header"]
F3["page-header.component.ts"]
end
subgraph F4["paginator"]
F5["paginator.component.ts"]
end
subgraph F6["paper"]
F7["paper.component.ts"]
end
subgraph F8["privacy-blur"]
F9["privacy-blur.component.ts"]
end
subgraph FA["privacy-toggle"]
FB["privacy-toggle.component.ts"]
end
subgraph FC["range-picker"]
FD["range-picker.component.ts"]
end
subgraph FE["select"]
FF["select.component.ts"]
end
subgraph FG["stat-card"]
FH["stat-card.component.ts"]
end
subgraph FI["table"]
FJ["table.component.ts"]
end
subgraph FK["tabs"]
FL["tabs.component.ts"]
end
subgraph FM["typography"]
FN["typography.component.ts"]
end
end
subgraph FO["utils"]
FP["calendar-cycles.ts"]
FQ["confidence-color.ts"]
FR["confirm-state.ts"]
FS["currency-format.ts"]
FT["currency-symbol-presets.ts"]
FU["daisy-classes.ts"]
FV["date-buckets.ts"]
FW["date-format.pipe.ts"]
FX["date-format.ts"]
FY["debounced-text.ts"]
FZ["download-json.ts"]
G0["fingerprint.ts"]
G1["format-settings.testing.ts"]
G2["format-settings.ts"]
G3["hidden-amount.ts"]
G4["iban.ts"]
G5["index.ts"]
G6["link-control-to-setting.ts"]
G7["locale-presets.ts"]
G8["number-format.ts"]
G9["pagination.ts"]
GA["percentage.ts"]
GB["quick-ranges.ts"]
GC["range-expression.ts"]
GD["search-params.ts"]
GE["selection-model.ts"]
GF["signed-amount.pipe.ts"]
GG["sortable.ts"]
GH["structural-filters.ts"]
GI["theme-hooks.ts"]
subgraph GJ["validators"]
GK["iban.validator.ts"]
GL["percentage.validator.ts"]
end
GM["with-archivable.ts"]
GN["with-persisted-crud.ts"]
end
end
end
end
