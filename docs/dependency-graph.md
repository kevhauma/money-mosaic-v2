# Dependency graph reference

Full import graph of `src/app`, generated with `dependency-cruiser`, for Claude to consult
when tracing what imports what without re-grepping the tree by hand.

## Scope

Unlike the human-facing overview in [`reports/dependency-graph.html`](../reports/dependency-graph.html)
(which collapses each `feature-*` folder to a single node and stops one folder into
`core`/`shared`), this graph goes three folder levels deeper on both tiers — up to 4 folders
below `src/app` — which is enough to reach individual files in most feature/core/shared
subtrees.

<!-- GENERATED:STATS:START -->
- 244 leaf nodes, 639 edges.
<!-- GENERATED:STATS:END -->

- `*.spec.ts` and `node_modules` are excluded.
- Node IDs are dependency-cruiser's internal short hashes (`4`, `2K`, `6N`, ...) — read them
  via the `subgraph`/node label text, not the ID itself.
- An edge collapsed at a folder boundary (rare here, since most subtrees fit within 4 levels)
  means _something_ inside that folder imports the target, not necessarily its `index.ts`.

## Regenerate

```bash
depcruise src/app \
  --config .dependency-cruiser.cjs \
  --collapse '^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+|^src/app/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+|^src/app/[^/]+' \
  --output-type mermaid \
  --output-to reports/dependency-graph-deep.mmd
```

## Graph

```mermaid
flowchart LR

subgraph 0["src"]
subgraph 1["app"]
subgraph 2["core"]
subgraph 3["accounts"]
4["account-deletion.service.ts"]
5["index.ts"]
6["joint-owner-lookup.ts"]
end
subgraph 7["categorisation"]
8["co-owner-contribution.service.ts"]
9["co-owner-contribution.ts"]
A["index.ts"]
B["rule-matching.ts"]
C["rules-engine.service.ts"]
end
subgraph D["data-access"]
E["accounts.repository.ts"]
F["app-db.ts"]
G["categories.repository.ts"]
H["category-comparison-settings.repository.ts"]
I["category-model.repository.ts"]
J["dashboard-layout-settings.repository.ts"]
K["data-management.repository.ts"]
L["import-batches.repository.ts"]
M["index.ts"]
N["mapping-profiles.repository.ts"]
O["rules.repository.ts"]
P["transactions.repository.ts"]
Q["transfer-settings.repository.ts"]
R["transfers.repository.ts"]
end
subgraph S["import"]
T["account-detection.ts"]
U["csv-import.service.ts"]
V["csv-parse.ts"]
W["csv-parse.worker.ts"]
X["csv-row-mapper.ts"]
Y["csv-worker.types.ts"]
Z["delimiter-guess.ts"]
10["import.service.ts"]
11["index.ts"]
end
subgraph 12["layout"]
subgraph 13["app-shell"]
14["app-shell.component.ts"]
end
15["index.ts"]
end
subgraph 16["links"]
17["external-links.ts"]
18["index.ts"]
end
subgraph 19["ml"]
1A["category-model.worker.ts"]
1B["category-model.worker.types.ts"]
1C["feature-hashing.ts"]
1D["index.ts"]
1E["model-config.ts"]
1F["rule-proposal-mining.ts"]
1G["training-window.ts"]
end
subgraph 1H["onboarding"]
1I["home-redirect.guard.ts"]
1J["index.ts"]
1K["mark-visited.guard.ts"]
1L["visited.service.ts"]
end
subgraph 1M["state"]
1N["accounts.store.ts"]
1O["categories.store.ts"]
1P["index.ts"]
1Q["transactions.store.ts"]
1R["transfer-settings.store.ts"]
1S["transfers.store.ts"]
end
subgraph 1T["stats"]
1U["account-balance-trend.ts"]
1V["category-breakdown.ts"]
1W["category-composition-trend.ts"]
1X["category-kind-contribution.ts"]
1Y["category-period-comparison.ts"]
1Z["chart-zoom-window.ts"]
20["classify-for-stats.ts"]
21["classify-joint-leg.ts"]
22["full-history-range.ts"]
23["granularity-for-span.ts"]
24["index.ts"]
25["joint-account-stake.ts"]
26["joint-contributor-breakdown.ts"]
27["net-margin.ts"]
28["net-worth-trend.ts"]
29["period-stats.ts"]
2A["period-window.ts"]
2B["periodized-rate.ts"]
2C["range-state.store.ts"]
2D["top-transactions.ts"]
2E["weekday-weekend-split.ts"]
2F["year-over-year.ts"]
end
subgraph 2G["storage"]
2H["index.ts"]
2I["storage-status.service.ts"]
end
subgraph 2J["theme"]
2K["index.ts"]
2L["theme-styles.ts"]
2M["theme.service.ts"]
end
subgraph 2N["transactions"]
2O["attribution-override.ts"]
2P["index.ts"]
2Q["nullify-transaction.ts"]
2R["transaction-deletion.service.ts"]
end
subgraph 2S["transfers"]
2T["index.ts"]
2U["transfer-cleanup.service.ts"]
2V["transfer-linking.service.ts"]
2W["transfer-matching.service.ts"]
2X["transfer-matching.ts"]
end
end
subgraph 2Y["feature-accounts"]
2Z["account-icons.ts"]
30["accounts.routes.ts"]
31["balance-trend-signals.ts"]
subgraph 32["components"]
subgraph 33["account-balance-chart"]
34["account-balance-chart.component.ts"]
end
subgraph 35["account-form"]
36["account-form.component.ts"]
end
subgraph 37["accounts-detail"]
38["accounts-detail.component.ts"]
end
subgraph 39["accounts-overview"]
3A["accounts-overview.component.ts"]
end
3B["index.ts"]
subgraph 3C["net-worth-history-chart"]
3D["net-worth-history-chart.component.ts"]
end
end
3E["index.ts"]
end
subgraph 3F["feature-categories"]
3G["categories.routes.ts"]
3H["category-icons.ts"]
3I["category-model.service.ts"]
3J["category-model.store.ts"]
subgraph 3K["components"]
subgraph 3L["categories-overview"]
3M["categories-overview.component.ts"]
end
subgraph 3N["category-form"]
3O["category-form.component.ts"]
end
3P["index.ts"]
subgraph 3Q["rule-filters"]
3R["rule-filters.component.ts"]
end
subgraph 3S["rule-form"]
3T["rule-form.component.ts"]
end
subgraph 3U["rule-share-bar"]
3V["rule-share-bar.component.ts"]
end
subgraph 3W["rules-overview"]
3X["rules-overview.component.ts"]
end
end
3Y["index.ts"]
3Z["rule-filters.ts"]
40["rule-labels.ts"]
41["rule-share.ts"]
42["rule-summary.ts"]
43["rules.store.ts"]
end
subgraph 44["feature-changelog"]
45["changelog.routes.ts"]
subgraph 46["components"]
subgraph 47["changelog-page"]
48["changelog-page.component.ts"]
end
49["index.ts"]
end
subgraph 4A["data"]
4B["changelog-entries.ts"]
4C["roadmap-entries.ts"]
end
4D["group-changelog-entries.ts"]
4E["group-roadmap-entries.ts"]
4F["index.ts"]
end
subgraph 4G["feature-dashboard"]
4H["category-comparison-settings.store.ts"]
subgraph 4I["components"]
subgraph 4J["account-balance-strip"]
4K["account-balance-strip.component.ts"]
end
subgraph 4L["action-queue-panel"]
4M["action-queue-panel.component.ts"]
end
subgraph 4N["category-breakdown-panel"]
4O["category-breakdown-panel.component.ts"]
end
subgraph 4P["category-comparison-panel"]
4Q["category-comparison-panel.component.ts"]
end
subgraph 4R["dashboard-customize-panel"]
4S["dashboard-customize-panel.component.ts"]
end
subgraph 4T["dashboard-overview"]
4U["dashboard-overview.component.ts"]
end
4V["index.ts"]
subgraph 4W["net-worth-header"]
4X["net-worth-header.component.ts"]
end
subgraph 4Y["top-transactions-panel"]
4Z["top-transactions-panel.component.ts"]
end
subgraph 50["trend-chart-panel"]
51["trend-chart-panel.component.ts"]
end
subgraph 52["weekday-weekend-split-panel"]
53["weekday-weekend-split-panel.component.ts"]
end
end
54["dashboard-layout-settings.store.ts"]
55["dashboard-row-order.ts"]
56["dashboard.routes.ts"]
57["index.ts"]
58["stats.store.ts"]
end
subgraph 59["feature-data-management"]
subgraph 5A["components"]
subgraph 5B["data-management-overview"]
5C["data-management-overview.component.ts"]
end
5D["index.ts"]
end
5E["data-management.routes.ts"]
5F["index.ts"]
end
subgraph 5G["feature-help"]
subgraph 5H["components"]
subgraph 5I["faq-page"]
5J["faq-page.component.ts"]
end
subgraph 5K["guide-detail"]
5L["guide-detail.component.ts"]
end
subgraph 5M["guides-index"]
5N["guides-index.component.ts"]
end
5O["index.ts"]
end
subgraph 5P["data"]
5Q["faq.ts"]
5R["guides.ts"]
end
5S["help.routes.ts"]
5T["index.ts"]
end
subgraph 5U["feature-home"]
subgraph 5V["components"]
subgraph 5W["home-landing"]
5X["home-landing.component.ts"]
end
5Y["index.ts"]
end
5Z["home.routes.ts"]
60["index.ts"]
end
subgraph 61["feature-import"]
subgraph 62["components"]
subgraph 63["column-map-active-field"]
64["column-map-active-field.component.ts"]
end
subgraph 65["column-map-collapsed-field"]
66["column-map-collapsed-field.component.ts"]
end
subgraph 67["column-map-field"]
68["column-map-field.component.ts"]
end
subgraph 69["import-map-step"]
6A["import-map-step.component.ts"]
end
subgraph 6B["import-preview-step"]
6C["import-preview-step.component.ts"]
end
subgraph 6D["import-select-step"]
6E["import-select-step.component.ts"]
end
subgraph 6F["import-summary-step"]
6G["import-summary-step.component.ts"]
end
subgraph 6H["import-wizard"]
6I["import-wizard.component.ts"]
end
6J["index.ts"]
end
6K["import-batches.store.ts"]
6L["import.routes.ts"]
6M["index.ts"]
6N["mapping-profiles.store.ts"]
end
subgraph 6O["feature-learning"]
subgraph 6P["components"]
6Q["index.ts"]
subgraph 6R["learning-overview"]
6S["learning-overview.component.ts"]
end
subgraph 6T["model-status"]
6U["model-status.component.ts"]
end
subgraph 6V["rule-proposals"]
6W["rule-proposals.component.ts"]
end
subgraph 6X["suggestions-table"]
6Y["suggestions-table.component.ts"]
end
end
6Z["index.ts"]
70["learning.routes.ts"]
end
subgraph 71["feature-settings"]
subgraph 72["components"]
73["index.ts"]
subgraph 74["settings-overview"]
75["settings-overview.component.ts"]
end
end
76["index.ts"]
77["settings.routes.ts"]
end
subgraph 78["feature-transactions"]
subgraph 79["components"]
subgraph 7A["attribution-override-fieldset"]
7B["attribution-override-fieldset.component.ts"]
end
7C["index.ts"]
subgraph 7D["transaction-bulk-bar"]
7E["transaction-bulk-bar.component.ts"]
end
subgraph 7F["transaction-edit-form"]
7G["transaction-edit-form.component.ts"]
end
subgraph 7H["transaction-filters"]
7I["transaction-filters.component.ts"]
end
subgraph 7J["transactions-overview"]
7K["transactions-overview.component.ts"]
end
subgraph 7L["transfer-review"]
7M["transfer-review.component.ts"]
end
end
7N["index.ts"]
7O["transaction-filters.ts"]
7P["transactions.routes.ts"]
end
subgraph 7Q["shared"]
subgraph 7R["echarts"]
7S["chart-theme.ts"]
7T["echarts-setup.ts"]
7U["index.ts"]
7V["tooltip-formatter.ts"]
end
subgraph 7W["ui"]
subgraph 7X["alert"]
7Y["alert.component.ts"]
end
subgraph 7Z["badge"]
80["badge.component.ts"]
end
subgraph 81["bento-grid"]
82["bento-grid.component.ts"]
83["bento-item.component.ts"]
end
subgraph 84["button"]
85["button.component.ts"]
end
subgraph 86["collapse"]
87["collapse.component.ts"]
end
subgraph 88["confirm-dialog"]
89["confirm-dialog.component.ts"]
end
subgraph 8A["date-range-input"]
8B["date-range-input.component.ts"]
end
subgraph 8C["divider"]
8D["divider.component.ts"]
end
subgraph 8E["dropdown"]
8F["dropdown.component.ts"]
end
subgraph 8G["empty-state"]
8H["empty-state.component.ts"]
end
subgraph 8I["fieldset"]
8J["fieldset.component.ts"]
end
subgraph 8K["flex"]
8L["flex.component.ts"]
end
subgraph 8M["granularity-picker"]
8N["granularity-picker.component.ts"]
end
8O["index.ts"]
subgraph 8P["input"]
8Q["input.component.ts"]
end
subgraph 8R["label"]
8S["label.component.ts"]
end
subgraph 8T["loading-skeleton"]
8U["loading-skeleton.component.ts"]
end
subgraph 8V["modal"]
8W["mm-modal.component.ts"]
end
subgraph 8X["page-header"]
8Y["page-header.component.ts"]
end
subgraph 8Z["paginator"]
90["paginator.component.ts"]
end
subgraph 91["paper"]
92["paper.component.ts"]
end
subgraph 93["range-grouping-switcher"]
94["range-grouping-switcher.component.ts"]
end
subgraph 95["select"]
96["select.component.ts"]
end
subgraph 97["stat-card"]
98["stat-card.component.ts"]
end
subgraph 99["table"]
9A["table.component.ts"]
end
subgraph 9B["tabs"]
9C["tabs.component.ts"]
end
subgraph 9D["typography"]
9E["typography.component.ts"]
end
end
subgraph 9F["utils"]
9G["confidence-color.ts"]
9H["confirm-state.ts"]
9I["currency-format.ts"]
9J["daisy-classes.ts"]
9K["date-buckets.ts"]
9L["debounced-text.ts"]
9M["download-json.ts"]
9N["fingerprint.ts"]
9O["iban.ts"]
9P["index.ts"]
9Q["pagination.ts"]
9R["percentage.ts"]
9S["search-params.ts"]
9T["selection-model.ts"]
9U["signed-amount.pipe.ts"]
9V["sortable.ts"]
9W["structural-filters.ts"]
9X["theme-hooks.ts"]
subgraph 9Y["validators"]
9Z["iban.validator.ts"]
A0["percentage.validator.ts"]
end
A1["with-archivable.ts"]
A2["with-persisted-crud.ts"]
end
end
end
end
4-->M
4-->2T
5-->4
5-->6
6-->M
6-->9P
8-->9
8-->M
9-->5
9-->M
A-->9
A-->8
A-->B
A-->C
B-->M
C-->B
C-->M
E-->F
F-->1E
F-->9N
G-->F
H-->F
I-->F
J-->F
K-->F
L-->F
M-->E
M-->F
M-->G
M-->H
M-->I
M-->J
M-->K
M-->L
M-->N
M-->O
M-->P
M-->Q
M-->R
N-->F
O-->F
P-->F
Q-->F
R-->F
T-->M
T-->9P
U-->Y
U-->M
V-->X
V-->Y
W-->V
W-->Y
X-->M
Y-->X
Y-->M
10-->X
10-->M
10-->2T
10-->9P
11-->T
11-->U
11-->V
11-->X
11-->Y
11-->Z
11-->10
14-->1P
14-->24
14-->85
14-->94
14-->9E
14-->9P
15-->14
18-->17
1A-->1B
1A-->1C
1A-->1E
1B-->1E
1C-->1E
1D-->1B
1D-->1C
1D-->1E
1D-->1F
1D-->1G
1F-->A
1F-->M
1I-->1L
1J-->1I
1J-->1K
1J-->1L
1K-->1L
1N-->1O
1N-->1Q
1N-->1S
1N-->5
1N-->M
1N-->24
1N-->2T
1N-->9P
1O-->1Q
1O-->M
1O-->9P
1P-->1N
1P-->1O
1P-->1Q
1P-->1R
1P-->1S
1Q-->M
1Q-->2P
1Q-->2T
1R-->M
1S-->1Q
1S-->1R
1S-->M
1S-->2T
1U-->21
1U-->28
1U-->M
1U-->9P
1V-->20
1V-->M
1W-->1V
1W-->M
1W-->9P
1X-->M
1Y-->1V
1Y-->2A
1Y-->M
1Z-->9P
20-->1X
20-->21
20-->M
20-->2T
21-->5
21-->M
22-->M
23-->9P
24-->1U
24-->1V
24-->1W
24-->1X
24-->1Y
24-->1Z
24-->20
24-->21
24-->22
24-->23
24-->25
24-->26
24-->27
24-->28
24-->29
24-->2A
24-->2B
24-->2C
24-->2D
24-->2E
24-->2F
25-->21
25-->M
26-->21
26-->5
26-->M
28-->21
28-->M
28-->9P
29-->20
29-->M
2A-->9P
2B-->9P
2C-->9P
2D-->M
2D-->2T
2E-->20
2E-->M
2E-->9P
2F-->29
2F-->M
2H-->2I
2K-->2L
2K-->2M
2M-->2L
2O-->M
2P-->2O
2P-->2Q
2P-->2R
2Q-->M
2R-->M
2R-->2T
2T-->2U
2T-->2V
2T-->2X
2T-->2W
2U-->M
2V-->2X
2V-->M
2W-->2V
2W-->2X
2W-->M
2X-->6
2X-->M
2X-->9P
2Z-->M
30-->38
30-->3A
30-->7U
31-->M
31-->1P
31-->24
31-->9P
34-->31
34-->M
34-->24
34-->7U
34-->8O
34-->9P
36-->2Z
36-->M
36-->8O
36-->9P
36-->9Z
36-->A0
38-->34
38-->36
38-->1P
38-->8O
38-->9P
3A-->2Z
3A-->36
3A-->3D
3A-->M
3A-->1P
3A-->8O
3A-->9P
3B-->34
3B-->36
3B-->38
3B-->3A
3B-->3D
3D-->31
3D-->M
3D-->1P
3D-->24
3D-->7U
3D-->8O
3E-->30
3E-->3B
3G-->3M
3G-->3X
3I-->1D
3J-->3I
3J-->43
3J-->M
3J-->1D
3J-->1P
3M-->3H
3M-->3O
3M-->M
3M-->1P
3M-->8O
3M-->9P
3O-->3H
3O-->M
3O-->8O
3P-->3M
3P-->3O
3P-->3R
3P-->3T
3P-->3V
3P-->3X
3R-->3Z
3R-->1P
3R-->8O
3R-->9P
3T-->40
3T-->A
3T-->M
3T-->1P
3T-->8O
3V-->41
3V-->43
3V-->8O
3V-->9P
3X-->3Z
3X-->42
3X-->43
3X-->3R
3X-->3T
3X-->3V
3X-->M
3X-->1P
3X-->8O
3X-->9P
3Y-->3G
3Y-->3H
3Y-->3I
3Y-->3J
3Y-->3P
3Y-->3Z
3Y-->42
3Y-->43
3Z-->42
3Z-->M
40-->M
41-->M
42-->40
42-->M
43-->41
43-->A
43-->M
43-->1P
43-->9P
45-->48
48-->4B
48-->4C
48-->4D
48-->4E
48-->8O
49-->48
4D-->4B
4E-->4C
4F-->45
4F-->49
4H-->M
4K-->1P
4K-->8O
4K-->9P
4M-->1P
4M-->2T
4M-->8O
4M-->9P
4O-->58
4O-->1P
4O-->24
4O-->7U
4O-->8O
4O-->9P
4Q-->4H
4Q-->58
4Q-->1P
4Q-->8O
4Q-->9P
4S-->54
4S-->55
4S-->M
4S-->8O
4U-->54
4U-->55
4U-->58
4U-->4K
4U-->4M
4U-->4O
4U-->4Q
4U-->4S
4U-->4X
4U-->4Z
4U-->51
4U-->53
4U-->1P
4U-->24
4U-->8O
4U-->9P
4V-->4K
4V-->4M
4V-->4O
4V-->4Q
4V-->4S
4V-->4U
4V-->4X
4V-->4Z
4V-->51
4V-->53
4X-->1P
4X-->8O
4X-->9P
4Z-->58
4Z-->1P
4Z-->24
4Z-->8O
4Z-->9P
51-->1P
51-->24
51-->2T
51-->7U
51-->8O
51-->9P
53-->58
53-->24
53-->8O
53-->9P
54-->55
54-->M
55-->M
56-->4U
56-->7U
57-->4V
57-->56
58-->4H
58-->1P
58-->24
58-->2T
5C-->M
5C-->2H
5C-->8O
5C-->9P
5D-->5C
5E-->5C
5F-->5D
5F-->5E
5J-->5Q
5J-->8O
5L-->5R
5L-->8O
5N-->5R
5N-->8O
5O-->5J
5O-->5L
5O-->5N
5S-->5J
5S-->5L
5S-->5N
5T-->5O
5T-->5S
5X-->18
5X-->8O
5Y-->5X
5Z-->5X
60-->5Y
60-->5Z
64-->6A
64-->8O
66-->6A
66-->8O
68-->64
68-->66
68-->6A
6A-->6N
6A-->68
6A-->M
6A-->11
6A-->8O
6C-->11
6C-->8O
6C-->9P
6E-->6N
6E-->M
6E-->11
6E-->8O
6G-->M
6G-->8O
6I-->6K
6I-->6N
6I-->6A
6I-->6C
6I-->6E
6I-->6G
6I-->M
6I-->11
6I-->1P
6I-->8O
6J-->6A
6J-->6C
6J-->6E
6J-->6G
6J-->6I
6K-->A
6K-->M
6K-->11
6K-->1P
6L-->6I
6M-->6J
6M-->6K
6M-->6L
6M-->6N
6N-->M
6Q-->6S
6Q-->6U
6Q-->6W
6Q-->6Y
6S-->6U
6S-->6W
6S-->6Y
6S-->8O
6U-->1D
6U-->1P
6U-->3Y
6U-->8O
6W-->M
6W-->1D
6W-->1P
6W-->3Y
6W-->8O
6W-->9P
6Y-->M
6Y-->1P
6Y-->3Y
6Y-->8O
6Y-->9P
6Z-->6Q
6Z-->70
70-->6S
73-->75
75-->18
75-->2K
75-->5F
75-->8O
76-->73
76-->77
77-->75
7B-->M
7B-->1P
7B-->2P
7B-->8O
7B-->9P
7C-->7E
7C-->7G
7C-->7I
7C-->7K
7C-->7M
7E-->1P
7E-->8O
7G-->7B
7G-->M
7G-->1P
7G-->2P
7G-->3Y
7G-->8O
7I-->7O
7I-->1P
7I-->8O
7I-->9P
7K-->7O
7K-->7E
7K-->7G
7K-->7I
7K-->7M
7K-->M
7K-->1P
7K-->2T
7K-->3Y
7K-->8O
7K-->9P
7M-->1P
7M-->2T
7M-->8O
7M-->9P
7N-->7P
7O-->M
7O-->2T
7P-->7K
7U-->7S
7U-->7T
7U-->7V
7V-->9P
7Y-->9P
80-->9P
82-->9P
83-->9P
85-->9P
87-->9P
89-->85
89-->8S
89-->8W
89-->9E
8B-->8F
8B-->9P
8D-->9P
8F-->9P
8H-->8L
8H-->9E
8J-->9P
8L-->9P
8O-->7Y
8O-->80
8O-->82
8O-->83
8O-->85
8O-->87
8O-->89
8O-->8B
8O-->8D
8O-->8F
8O-->8H
8O-->8J
8O-->8L
8O-->8N
8O-->8Q
8O-->8S
8O-->8U
8O-->8W
8O-->8Y
8O-->90
8O-->92
8O-->94
8O-->96
8O-->98
8O-->9A
8O-->9C
8O-->9E
8Q-->9P
8S-->9P
8U-->8L
8W-->9P
8Y-->8L
8Y-->9E
90-->85
90-->8L
90-->9E
90-->9P
92-->9P
94-->85
94-->8B
94-->8L
96-->9P
98-->9E
98-->9P
9A-->9P
9C-->9P
9E-->9P
9P-->9G
9P-->9H
9P-->9I
9P-->9J
9P-->9K
9P-->9L
9P-->9M
9P-->9N
9P-->9O
9P-->9Q
9P-->9R
9P-->9S
9P-->9T
9P-->9U
9P-->9V
9P-->9W
9P-->9X
9P-->A1
9P-->A2
9U-->9I
```
