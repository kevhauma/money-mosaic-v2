...
7X["index.ts"]
subgraph 7Y["settings-about-section"]
7Z["settings-about-section.component.ts"]
end
subgraph 80["settings-currency-locale-section"]
81["settings-currency-locale-section.component.ts"]
end
subgraph 82["settings-data-section"]
83["settings-data-section.component.ts"]
end
subgraph 84["settings-overview"]
85["settings-overview.component.ts"]
end
subgraph 86["settings-theme-section"]
87["settings-theme-section.component.ts"]
end
end
88["index.ts"]
89["settings.routes.ts"]
end
subgraph 8A["feature-transactions"]
subgraph 8B["components"]
subgraph 8C["attribution-override-fieldset"]
8D["attribution-override-fieldset.component.ts"]
end
subgraph 8E["category-select-cell"]
8F["category-select-cell.component.ts"]
end
8G["index.ts"]
subgraph 8H["transaction-bulk-bar"]
8I["transaction-bulk-bar.component.ts"]
end
subgraph 8J["transaction-edit-form"]
8K["transaction-edit-form.component.ts"]
end
subgraph 8L["transaction-filters"]
8M["transaction-filters.component.ts"]
end
subgraph 8N["transaction-row"]
8O["transaction-row.component.ts"]
end
subgraph 8P["transactions-overview"]
8Q["transactions-overview.component.ts"]
end
subgraph 8R["transfer-review"]
8S["transfer-review.component.ts"]
end
end
8T["index.ts"]
8U["transaction-filters.ts"]
8V["transaction-row-vm.ts"]
8W["transactions.routes.ts"]
end
subgraph 8X["shared"]
subgraph 8Y["echarts"]
8Z["chart-theme.ts"]
90["echarts-setup.ts"]
91["index.ts"]
92["tooltip-formatter.ts"]
end
subgraph 93["ui"]
subgraph 94["alert"]
95["alert.component.ts"]
end
subgraph 96["badge"]
97["badge.component.ts"]
end
subgraph 98["button"]
99["button.component.ts"]
end
subgraph 9A["collapse"]
9B["collapse.component.ts"]
end
subgraph 9C["confirm-dialog"]
9D["confirm-dialog.component.ts"]
end
subgraph 9E["date-range-input"]
9F["date-range-input.component.ts"]
end
subgraph 9G["divider"]
9H["divider.component.ts"]
end
subgraph 9I["dropdown"]
9J["dropdown.component.ts"]
end
subgraph 9K["empty-state"]
9L["empty-state.component.ts"]
end
subgraph 9M["fieldset"]
9N["fieldset.component.ts"]
end
subgraph 9O["flex"]
9P["flex.component.ts"]
end
subgraph 9Q["granularity-picker"]
9R["granularity-picker.component.ts"]
end
9S["index.ts"]
subgraph 9T["input"]
9U["input.component.ts"]
end
subgraph 9V["label"]
9W["label.component.ts"]
end
subgraph 9X["loading-skeleton"]
9Y["loading-skeleton.component.ts"]
end
subgraph 9Z["modal"]
A0["mm-modal.component.ts"]
end
subgraph A1["page-header"]
A2["page-header.component.ts"]
end
subgraph A3["paginator"]
A4["paginator.component.ts"]
end
subgraph A5["paper"]
A6["paper.component.ts"]
end
subgraph A7["range-grouping-switcher"]
A8["range-grouping-switcher.component.ts"]
end
subgraph A9["select"]
AA["select.component.ts"]
end
subgraph AB["stat-card"]
AC["stat-card.component.ts"]
end
subgraph AD["table"]
AE["table.component.ts"]
end
subgraph AF["tabs"]
AG["tabs.component.ts"]
end
subgraph AH["typography"]
AI["typography.component.ts"]
end
end
subgraph AJ["utils"]
AK["confidence-color.ts"]
AL["confirm-state.ts"]
AM["currency-format.ts"]
AN["currency-symbol-presets.ts"]
AO["daisy-classes.ts"]
AP["date-buckets.ts"]
AQ["date-format.pipe.ts"]
AR["date-format.ts"]
AS["debounced-text.ts"]
AT["download-json.ts"]
AU["fingerprint.ts"]
AV["format-settings.ts"]
AW["iban.ts"]
AX["index.ts"]
AY["link-control-to-setting.ts"]
AZ["locale-presets.ts"]
B0["number-format.ts"]
B1["pagination.ts"]
B2["percentage.ts"]
B3["search-params.ts"]
B4["selection-model.ts"]
B5["signed-amount.pipe.ts"]
B6["sortable.ts"]
B7["structural-filters.ts"]
B8["theme-hooks.ts"]
subgraph B9["validators"]
BA["iban.validator.ts"]
BB["percentage.validator.ts"]
end
BC["with-archivable.ts"]
BD["with-persisted-crud.ts"]
end
end
end
end
