# Dependency graph reference

Full import graph of `src/app`, generated with `dependency-cruiser`, for Claude to consult
when tracing what imports what without re-grepping the tree by hand.

## Scope

Unlike the human-facing overview in [`reports/dependency-graph.html`](../reports/dependency-graph.html)
(which collapses each `feature-*` folder to a single node and stops one folder into
`core`/`shared`), this graph goes three folder levels deeper on both tiers — up to 4 folders
below `src/app` — which is enough to reach individual files in most feature/core/shared
subtrees.

<!-- GENERATED:STATS:START -->
- 353 leaf nodes, 1032 edges.
<!-- GENERATED:STATS:END -->

- `*.spec.ts` and `node_modules` are excluded.
- Node IDs are dependency-cruiser's internal short hashes (`4`, `2K`, `6N`, ...) — read them
  via the `subgraph`/node label text, not the ID itself.
- An edge collapsed at a folder boundary (rare here, since most subtrees fit within 4 levels)
  means _something_ inside that folder imports the target, not necessarily its `index.ts`.

## Regenerate

```bash
depcruise src/app \
  --config .dependency-cruiser.cjs \
  --collapse '^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+|^src/app/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+|^src/app/[^/]+' \
  --output-type mermaid \
  --output-to reports/dependency-graph-deep.mmd
```

## Graph

```mermaid
flowchart LR

subgraph 0["src"]
subgraph 1["app"]
subgraph 2["core"]
subgraph 3["accounts"]
4["account-deletion.service.ts"]
5["index.ts"]
6["joint-owner-lookup.ts"]
end
subgraph 7["categorisation"]
8["category-applicability.ts"]
9["co-owner-contribution.service.ts"]
A["co-owner-contribution.ts"]
B["index.ts"]
C["rule-matching.ts"]
D["rules-engine.service.ts"]
end
subgraph E["data-access"]
F["accounts.repository.ts"]
G["app-db.ts"]
H["app-settings.repository.ts"]
I["categories.repository.ts"]
J["category-comparison-settings.repository.ts"]
K["category-model.repository.ts"]
L["dashboard-layout-settings.repository.ts"]
M["data-management.repository.ts"]
N["import-batches.repository.ts"]
O["index.ts"]
P["mapping-profiles.repository.ts"]
Q["rules.repository.ts"]
R["salary-metadata.repository.ts"]
S["transactions.repository.ts"]
T["transfer-settings.repository.ts"]
U["transfers.repository.ts"]
end
subgraph V["import"]
W["account-detection.ts"]
X["csv-import.service.ts"]
Y["csv-parse.ts"]
Z["csv-parse.worker.ts"]
10["csv-row-mapper.ts"]
11["csv-worker.types.ts"]
12["delimiter-guess.ts"]
13["import.service.ts"]
14["index.ts"]
end
subgraph 15["layout"]
subgraph 16["app-shell"]
17["app-shell.component.ts"]
end
18["index.ts"]
end
subgraph 19["links"]
1A["external-links.ts"]
1B["index.ts"]
end
subgraph 1C["ml"]
1D["category-model.worker.ts"]
1E["category-model.worker.types.ts"]
1F["feature-hashing.ts"]
1G["index.ts"]
1H["model-config.ts"]
1I["rule-proposal-mining.ts"]
1J["training-window.ts"]
end
subgraph 1K["onboarding"]
1L["home-redirect.guard.ts"]
1M["index.ts"]
1N["mark-visited.guard.ts"]
1O["visited.service.ts"]
end
subgraph 1P["state"]
1Q["accounts.store.ts"]
1R["app-settings.store.ts"]
1S["categories.store.ts"]
1T["chart-options-control.ts"]
1U["chart-options.store.ts"]
1V["index.ts"]
1W["page-range-control.ts"]
1X["range-state.store.ts"]
1Y["transactions.store.ts"]
1Z["transfer-settings.store.ts"]
20["transfers.store.ts"]
end
subgraph 21["stats"]
22["account-balance-history.ts"]
23["account-balance-trend.ts"]
24["annual-lump-sum-smoothing.ts"]
25["category-breakdown.ts"]
26["category-composition-trend.ts"]
27["category-cycle-heatmap.ts"]
28["category-kind-contribution.ts"]
29["category-period-comparison.ts"]
2A["chart-zoom-window.ts"]
2B["classify-for-stats.ts"]
2C["classify-joint-leg.ts"]
2D["day-transactions.ts"]
2E["embedded-bonus-smoothing.ts"]
2F["full-history-range.ts"]
2G["granularity-for-span.ts"]
2H["gross-net-growth.ts"]
2I["gross-net-ratio.ts"]
2J["income-category-series.ts"]
2K["income-events.ts"]
2L["income-gap-detection.ts"]
2M["income-growth.ts"]
2N["income-step-change-detection.ts"]
2O["index.ts"]
2P["joint-account-stake.ts"]
2Q["joint-contributor-breakdown.ts"]
2R["money-flow-graph.ts"]
2S["multi-year-income-comparison.ts"]
2T["net-margin.ts"]
2U["period-stats.ts"]
2V["period-window.ts"]
2W["periodized-rate.ts"]
2X["recurring-payments.ts"]
2Y["recurring-projection.ts"]
2Z["top-transactions.ts"]
30["wage-change-detection.ts"]
31["weekday-weekend-split.ts"]
32["year-over-year.ts"]
33["yearly-income-summary.ts"]
end
subgraph 34["storage"]
35["index.ts"]
36["storage-status.service.ts"]
end
subgraph 37["theme"]
38["accent-colors.ts"]
39["index.ts"]
3A["theme-styles.ts"]
3B["theme.service.ts"]
end
subgraph 3C["transactions"]
3D["attribution-override.ts"]
3E["index.ts"]
3F["nullify-transaction.ts"]
3G["transaction-deletion.service.ts"]
end
subgraph 3H["transfers"]
3I["index.ts"]
3J["transfer-cleanup.service.ts"]
3K["transfer-linking.service.ts"]
3L["transfer-matching.service.ts"]
3M["transfer-matching.ts"]
end
end
subgraph 3N["feature-accounts"]
3O["account-card-vm.ts"]
3P["account-icons.ts"]
3Q["account-list-order.ts"]
3R["account-types.ts"]
3S["accounts.routes.ts"]
3T["balance-day-tooltip.ts"]
3U["balance-trend-signals.ts"]
subgraph 3V["components"]
subgraph 3W["account-balance-block"]
3X["account-balance-block.component.ts"]
end
subgraph 3Y["account-balance-chart"]
3Z["account-balance-chart.component.ts"]
end
subgraph 40["account-balance-history-chart"]
41["account-balance-history-chart.component.ts"]
end
subgraph 42["account-card"]
43["account-card.component.ts"]
end
subgraph 44["account-form"]
45["account-form.component.ts"]
end
subgraph 46["accounts-detail"]
47["accounts-detail.component.ts"]
end
subgraph 48["accounts-overview"]
49["accounts-overview.component.ts"]
end
4A["index.ts"]
end
4B["index.ts"]
end
subgraph 4C["feature-categories"]
4D["categories.routes.ts"]
4E["category-icons.ts"]
4F["category-model.service.ts"]
4G["category-model.store.ts"]
4H["category-row-vm.ts"]
subgraph 4I["components"]
subgraph 4J["categories-overview"]
4K["categories-overview.component.ts"]
end
subgraph 4L["category-form"]
4M["category-form.component.ts"]
end
4N["index.ts"]
subgraph 4O["rule-condition-row"]
4P["rule-condition-row.component.ts"]
end
subgraph 4Q["rule-filters"]
4R["rule-filters.component.ts"]
end
subgraph 4S["rule-form"]
4T["rule-form.component.ts"]
end
subgraph 4U["rule-share-bar"]
4V["rule-share-bar.component.ts"]
end
subgraph 4W["rules-overview"]
4X["rules-overview.component.ts"]
end
end
4Y["index.ts"]
4Z["rule-condition-editor.ts"]
50["rule-filters.ts"]
51["rule-labels.ts"]
52["rule-share.ts"]
53["rule-summary.ts"]
54["rules.store.ts"]
end
subgraph 55["feature-changelog"]
56["changelog.routes.ts"]
subgraph 57["components"]
subgraph 58["changelog-entry-row"]
59["changelog-entry-row.component.ts"]
end
subgraph 5A["changelog-page"]
5B["changelog-page.component.ts"]
end
5C["index.ts"]
end
subgraph 5D["data"]
5E["changelog-entries.ts"]
5F["roadmap-entries.ts"]
end
5G["group-changelog-entries.ts"]
5H["group-roadmap-entries.ts"]
5I["index.ts"]
end
subgraph 5J["feature-dashboard"]
5K["category-comparison-settings.store.ts"]
5L["category-comparison-vm.ts"]
subgraph 5M["components"]
subgraph 5N["account-balance-strip"]
5O["account-balance-strip.component.ts"]
end
subgraph 5P["action-queue-panel"]
5Q["action-queue-panel.component.ts"]
end
subgraph 5R["category-breakdown-panel"]
5S["category-breakdown-panel.component.ts"]
end
subgraph 5T["category-comparison-panel"]
5U["category-comparison-panel.component.ts"]
end
subgraph 5V["category-exclusion-dropdown"]
5W["category-exclusion-dropdown.component.ts"]
end
subgraph 5X["comparison-category-card"]
5Y["comparison-category-card.component.ts"]
end
subgraph 5Z["dashboard-customize-panel"]
60["dashboard-customize-panel.component.ts"]
end
subgraph 61["dashboard-overview"]
62["dashboard-overview.component.ts"]
end
63["index.ts"]
subgraph 64["spending-heatmap-panel"]
65["spending-heatmap-panel.component.ts"]
end
subgraph 66["top-transactions-panel"]
67["top-transactions-panel.component.ts"]
end
subgraph 68["trend-chart-panel"]
69["trend-chart-panel.component.ts"]
end
subgraph 6A["weekday-weekend-split-panel"]
6B["weekday-weekend-split-panel.component.ts"]
end
end
6C["dashboard-layout-settings.store.ts"]
6D["dashboard-row-order.ts"]
6E["dashboard.routes.ts"]
6F["index.ts"]
6G["stats.store.ts"]
end
subgraph 6H["feature-data-management"]
subgraph 6I["components"]
subgraph 6J["data-management-overview"]
6K["data-management-overview.component.ts"]
end
6L["index.ts"]
end
6M["index.ts"]
end
subgraph 6N["feature-explore"]
6O["bills-calendar-vm.ts"]
subgraph 6P["components"]
subgraph 6Q["bills-calendar"]
6R["bills-calendar.component.ts"]
end
subgraph 6S["bills-day-list"]
6T["bills-day-list.component.ts"]
end
subgraph 6U["bills-month-grid"]
6V["bills-month-grid.component.ts"]
end
subgraph 6W["explore-overview"]
6X["explore-overview.component.ts"]
end
6Y["index.ts"]
subgraph 6Z["money-flow-panel"]
70["money-flow-panel.component.ts"]
end
subgraph 71["recurring-payments-panel"]
72["recurring-payments-panel.component.ts"]
end
end
73["explore.routes.ts"]
74["index.ts"]
75["recurring-payments-row-vm.ts"]
76["recurring-series.store.ts"]
end
subgraph 77["feature-help"]
subgraph 78["components"]
subgraph 79["faq-page"]
7A["faq-page.component.ts"]
end
subgraph 7B["guide-detail"]
7C["guide-detail.component.ts"]
end
subgraph 7D["guide-steps"]
7E["guide-steps.component.ts"]
end
subgraph 7F["guides-index"]
7G["guides-index.component.ts"]
end
7H["index.ts"]
end
subgraph 7I["data"]
7J["faq.ts"]
7K["guides.ts"]
end
7L["help.routes.ts"]
7M["index.ts"]
end
subgraph 7N["feature-home"]
subgraph 7O["components"]
subgraph 7P["home-landing"]
7Q["home-landing.component.ts"]
end
7R["index.ts"]
end
7S["home.routes.ts"]
7T["index.ts"]
end
subgraph 7U["feature-import"]
7V["column-mapping.ts"]
subgraph 7W["components"]
subgraph 7X["account-draft-editor"]
7Y["account-draft-editor.component.ts"]
end
subgraph 7Z["batch-wait-card"]
80["batch-wait-card.component.ts"]
end
subgraph 81["column-map-amount-field"]
82["column-map-amount-field.component.ts"]
end
subgraph 83["column-map-counterparty-field"]
84["column-map-counterparty-field.component.ts"]
end
subgraph 85["column-map-sample-caption"]
86["column-map-sample-caption.component.ts"]
end
subgraph 87["column-map-simple-field"]
88["column-map-simple-field.component.ts"]
end
subgraph 89["column-map-stepper"]
8A["column-map-stepper.component.ts"]
end
subgraph 8B["column-map-summary-step"]
8C["column-map-summary-step.component.ts"]
end
subgraph 8D["import-map-step"]
8E["import-map-step.component.ts"]
end
subgraph 8F["import-preview-step"]
8G["import-preview-step.component.ts"]
end
subgraph 8H["import-select-step"]
8I["import-select-step.component.ts"]
end
subgraph 8J["import-summary-step"]
8K["import-summary-step.component.ts"]
end
subgraph 8L["import-wizard"]
8M["import-wizard.component.ts"]
end
8N["index.ts"]
subgraph 8O["queued-file-row"]
8P["queued-file-row.component.ts"]
end
end
8Q["import-batches.store.ts"]
8R["import-queue.ts"]
8S["import-wizard-session.ts"]
8T["import.routes.ts"]
8U["index.ts"]
8V["mapper-steps.ts"]
8W["mapping-profiles.store.ts"]
end
subgraph 8X["feature-income"]
8Y["career-start-date.ts"]
subgraph 8Z["components"]
subgraph 90["income-career-start"]
91["income-career-start.component.ts"]
end
subgraph 92["income-category-checklist"]
93["income-category-checklist.component.ts"]
end
subgraph 94["income-chart-cell"]
95["income-chart-cell.component.ts"]
end
subgraph 96["income-events-sidebar"]
97["income-events-sidebar.component.ts"]
end
subgraph 98["income-gross-color"]
99["income-gross-color.component.ts"]
end
subgraph 9A["income-gross-net-section"]
9B["income-gross-net-section.component.ts"]
end
subgraph 9C["income-growth-panel"]
9D["income-growth-panel.component.ts"]
end
subgraph 9E["income-intro"]
9F["income-intro.component.ts"]
end
subgraph 9G["income-main-category"]
9H["income-main-category.component.ts"]
end
subgraph 9I["income-overview"]
9J["income-overview.component.ts"]
end
subgraph 9K["income-settings-page"]
9L["income-settings-page.component.ts"]
end
subgraph 9M["income-yearly-panel"]
9N["income-yearly-panel.component.ts"]
end
9O["index.ts"]
subgraph 9P["salary-details-page"]
9Q["salary-details-page.component.ts"]
end
subgraph 9R["salary-metadata-table"]
9S["salary-metadata-table.component.ts"]
end
subgraph 9T["salary-month-modal"]
9U["salary-month-modal.component.ts"]
end
end
9V["gross-net-chart-options.ts"]
9W["income-category-vm.ts"]
9X["income-event-vm.ts"]
9Y["income-granularity.ts"]
9Z["income.routes.ts"]
A0["income.store.ts"]
A1["index.ts"]
A2["salary-metadata-edit.ts"]
A3["salary-metadata-rows.ts"]
end
subgraph A4["feature-learning"]
subgraph A5["components"]
A6["index.ts"]
subgraph A7["learning-overview"]
A8["learning-overview.component.ts"]
end
subgraph A9["model-status-badge"]
AA["model-status-badge.component.ts"]
end
subgraph AB["model-status"]
AC["model-status.component.ts"]
end
subgraph AD["rule-proposals"]
AE["rule-proposals.component.ts"]
end
subgraph AF["suggestions-table"]
AG["suggestions-table.component.ts"]
end
end
AH["index.ts"]
AI["learning.routes.ts"]
AJ["model-status-display.ts"]
end
subgraph AK["feature-settings"]
subgraph AL["components"]
AM["index.ts"]
subgraph AN["settings-about-section"]
AO["settings-about-section.component.ts"]
end
subgraph AP["settings-currency-locale-section"]
AQ["settings-currency-locale-section.component.ts"]
end
subgraph AR["settings-data-section"]
AS["settings-data-section.component.ts"]
end
subgraph AT["settings-overview"]
AU["settings-overview.component.ts"]
end
subgraph AV["settings-privacy-section"]
AW["settings-privacy-section.component.ts"]
end
subgraph AX["settings-theme-section"]
AY["settings-theme-section.component.ts"]
end
end
AZ["index.ts"]
B0["settings.routes.ts"]
end
subgraph B1["feature-transactions"]
B2["category-picker.ts"]
subgraph B3["components"]
subgraph B4["attribution-override-fieldset"]
B5["attribution-override-fieldset.component.ts"]
end
subgraph B6["category-select-cell"]
B7["category-select-cell.component.ts"]
end
B8["index.ts"]
subgraph B9["transaction-bulk-bar"]
BA["transaction-bulk-bar.component.ts"]
end
subgraph BB["transaction-edit-form"]
BC["transaction-edit-form.component.ts"]
end
subgraph BD["transaction-filters"]
BE["transaction-filters.component.ts"]
end
subgraph BF["transaction-row"]
BG["transaction-row.component.ts"]
end
subgraph BH["transactions-overview"]
BI["transactions-overview.component.ts"]
end
subgraph BJ["transfer-review"]
BK["transfer-review.component.ts"]
end
end
BL["index.ts"]
BM["transaction-filters.ts"]
BN["transaction-row-vm.ts"]
BO["transactions.routes.ts"]
end
subgraph BP["shared"]
subgraph BQ["echarts"]
BR["bucketed-axis-option.ts"]
BS["chart-theme.ts"]
BT["echarts-jsdom.testing.ts"]
BU["echarts-setup.ts"]
BV["index.ts"]
BW["legend-option.ts"]
BX["tooltip-formatter.ts"]
end
subgraph BY["ui"]
subgraph BZ["alert"]
C0["alert.component.ts"]
end
subgraph C1["badge"]
C2["badge.component.ts"]
end
subgraph C3["button"]
C4["button.component.ts"]
end
subgraph C5["collapse"]
C6["collapse.component.ts"]
end
subgraph C7["confirm-dialog"]
C8["confirm-dialog.component.ts"]
end
subgraph C9["cycle-picker"]
CA["cycle-picker.component.ts"]
end
subgraph CB["date-range-input"]
CC["date-range-input.component.ts"]
end
subgraph CD["divider"]
CE["divider.component.ts"]
end
subgraph CF["dropdown"]
CG["dropdown.component.ts"]
end
subgraph CH["empty-state"]
CI["empty-state.component.ts"]
end
subgraph CJ["fieldset"]
CK["fieldset.component.ts"]
end
subgraph CL["flex"]
CM["flex.component.ts"]
end
subgraph CN["granularity-picker"]
CO["granularity-picker.component.ts"]
end
CP["index.ts"]
subgraph CQ["input"]
CR["input.component.ts"]
end
subgraph CS["label"]
CT["label.component.ts"]
end
subgraph CU["loading-skeleton"]
CV["loading-skeleton.component.ts"]
end
subgraph CW["modal"]
CX["mm-modal.component.ts"]
end
subgraph CY["page-header"]
CZ["page-header.component.ts"]
end
subgraph D0["paginator"]
D1["paginator.component.ts"]
end
subgraph D2["paper"]
D3["paper.component.ts"]
end
subgraph D4["privacy-blur"]
D5["privacy-blur.component.ts"]
end
subgraph D6["range-grouping-switcher"]
D7["range-grouping-switcher.component.ts"]
end
subgraph D8["select"]
D9["select.component.ts"]
end
subgraph DA["stat-card"]
DB["stat-card.component.ts"]
end
subgraph DC["table"]
DD["table.component.ts"]
end
subgraph DE["tabs"]
DF["tabs.component.ts"]
end
subgraph DG["typography"]
DH["typography.component.ts"]
end
end
subgraph DI["utils"]
DJ["calendar-cycles.ts"]
DK["confidence-color.ts"]
DL["confirm-state.ts"]
DM["currency-format.ts"]
DN["currency-symbol-presets.ts"]
DO["daisy-classes.ts"]
DP["date-buckets.ts"]
DQ["date-format.pipe.ts"]
DR["date-format.ts"]
DS["debounced-text.ts"]
DT["download-json.ts"]
DU["fingerprint.ts"]
DV["format-settings.testing.ts"]
DW["format-settings.ts"]
DX["iban.ts"]
DY["index.ts"]
DZ["link-control-to-setting.ts"]
E0["locale-presets.ts"]
E1["number-format.ts"]
E2["pagination.ts"]
E3["percentage.ts"]
E4["search-params.ts"]
E5["selection-model.ts"]
E6["signed-amount.pipe.ts"]
E7["sortable.ts"]
E8["structural-filters.ts"]
E9["theme-hooks.ts"]
subgraph EA["validators"]
EB["iban.validator.ts"]
EC["percentage.validator.ts"]
end
ED["with-archivable.ts"]
EE["with-persisted-crud.ts"]
end
end
end
end
4-->O
4-->3I
5-->4
5-->6
6-->O
6-->DY
8-->O
9-->A
9-->O
A-->5
A-->O
B-->8
B-->A
B-->9
B-->C
B-->D
C-->O
D-->C
D-->O
F-->G
G-->1H
G-->38
G-->DU
G-->DW
H-->G
H-->39
H-->DY
I-->G
J-->G
K-->G
L-->G
M-->G
N-->G
O-->F
O-->G
O-->H
O-->I
O-->J
O-->K
O-->L
O-->M
O-->N
O-->P
O-->Q
O-->R
O-->S
O-->T
O-->U
P-->G
Q-->G
R-->G
S-->G
T-->G
U-->G
W-->O
W-->DY
X-->11
X-->O
Y-->10
Y-->11
Z-->Y
Z-->11
10-->O
11-->10
11-->O
13-->10
13-->O
13-->3I
13-->DY
14-->W
14-->X
14-->Y
14-->10
14-->11
14-->12
14-->13
17-->1V
17-->C4
17-->DH
18-->17
1B-->1A
1D-->1E
1D-->1F
1D-->1H
1E-->1H
1F-->1H
1G-->1E
1G-->1F
1G-->1H
1G-->1I
1G-->1J
1I-->B
1I-->O
1L-->1O
1M-->1L
1M-->1N
1M-->1O
1N-->1O
1Q-->1S
1Q-->1Y
1Q-->20
1Q-->5
1Q-->O
1Q-->2O
1Q-->3I
1Q-->DY
1R-->O
1R-->39
1R-->DY
1S-->1Y
1S-->O
1S-->DY
1T-->1U
1T-->BV
1T-->DY
1U-->BV
1U-->DY
1V-->1Q
1V-->1R
1V-->1S
1V-->1T
1V-->1U
1V-->1W
1V-->1X
1V-->1Y
1V-->1Z
1V-->20
1W-->1Q
1W-->1X
1W-->1Y
1W-->2O
1W-->DY
1X-->DY
1Y-->O
1Y-->3E
1Y-->3I
1Z-->O
20-->1Y
20-->1Z
20-->O
20-->3I
22-->O
22-->DY
23-->22
23-->O
23-->DY
24-->26
24-->2J
24-->DY
25-->2B
25-->O
26-->25
26-->O
26-->DY
27-->25
27-->2B
27-->O
27-->DY
28-->O
29-->25
29-->2V
29-->O
2A-->DY
2B-->28
2B-->2C
2B-->O
2B-->3I
2C-->5
2C-->O
2D-->O
2E-->26
2E-->2J
2E-->O
2E-->DY
2F-->O
2G-->DY
2H-->2I
2I-->2J
2I-->O
2J-->26
2J-->O
2J-->DY
2K-->2L
2K-->2N
2K-->30
2K-->O
2L-->26
2L-->2J
2L-->DY
2M-->2J
2M-->32
2M-->DY
2N-->26
2N-->2E
2N-->2J
2N-->DY
2O-->22
2O-->23
2O-->24
2O-->25
2O-->26
2O-->27
2O-->28
2O-->29
2O-->2A
2O-->2B
2O-->2C
2O-->2D
2O-->2E
2O-->2F
2O-->2G
2O-->2H
2O-->2I
2O-->2J
2O-->2K
2O-->2L
2O-->2M
2O-->2N
2O-->2P
2O-->2Q
2O-->2R
2O-->2S
2O-->2T
2O-->2U
2O-->2V
2O-->2W
2O-->2X
2O-->2Y
2O-->2Z
2O-->30
2O-->31
2O-->32
2O-->33
2P-->2C
2P-->O
2Q-->2C
2Q-->5
2Q-->O
2R-->O
2R-->DY
2S-->33
2U-->2B
2U-->O
2V-->DY
2W-->DY
2X-->2B
2X-->B
2X-->O
2X-->DY
2Y-->2X
2Y-->DY
2Z-->O
2Z-->3I
30-->2I
31-->2B
31-->O
31-->DY
32-->2U
32-->O
33-->26
33-->O
33-->DY
35-->36
38-->3A
39-->38
39-->3A
39-->3B
3B-->3A
3D-->O
3E-->3D
3E-->3F
3E-->3G
3F-->O
3G-->O
3G-->3I
3I-->3J
3I-->3K
3I-->3M
3I-->3L
3J-->O
3K-->3M
3K-->O
3L-->3K
3L-->3M
3L-->O
3M-->6
3M-->O
3M-->DY
3O-->O
3P-->O
3Q-->O
3R-->O
3S-->47
3S-->49
3S-->BV
3T-->2O
3T-->DY
3U-->O
3U-->1V
3U-->2O
3U-->DY
3X-->CP
3X-->DY
3Z-->3T
3Z-->3U
3Z-->O
3Z-->2O
3Z-->BV
3Z-->CP
3Z-->DY
41-->3T
41-->3U
41-->O
41-->1V
41-->2O
41-->BV
41-->CP
43-->3O
43-->3P
43-->3X
43-->CP
45-->3P
45-->3R
45-->O
45-->CP
45-->DY
45-->EB
45-->EC
47-->3X
47-->3Z
47-->45
47-->1V
47-->CP
47-->DY
49-->3O
49-->3P
49-->3Q
49-->41
49-->43
49-->45
49-->O
49-->1V
49-->CP
49-->DY
4A-->3X
4A-->3Z
4A-->41
4A-->43
4A-->45
4A-->47
4A-->49
4B-->3P
4B-->3R
4B-->3S
4B-->4A
4D-->4K
4D-->4X
4F-->1G
4G-->4F
4G-->54
4G-->O
4G-->1G
4G-->1V
4H-->O
4H-->CP
4K-->4E
4K-->4H
4K-->4M
4K-->B
4K-->O
4K-->1V
4K-->CP
4K-->DY
4M-->4E
4M-->O
4M-->CP
4N-->4K
4N-->4M
4N-->4R
4N-->4T
4N-->4V
4N-->4X
4P-->4Z
4P-->51
4P-->B
4P-->O
4P-->1V
4P-->CP
4R-->50
4R-->1V
4R-->CP
4R-->DY
4T-->4Z
4T-->4P
4T-->B
4T-->O
4T-->1V
4T-->CP
4V-->52
4V-->54
4V-->CP
4V-->DY
4X-->50
4X-->53
4X-->54
4X-->4R
4X-->4T
4X-->4V
4X-->O
4X-->1V
4X-->CP
4X-->DY
4Y-->4D
4Y-->4E
4Y-->4F
4Y-->4G
4Y-->4N
4Y-->50
4Y-->53
4Y-->54
4Z-->51
4Z-->O
50-->53
50-->O
51-->O
52-->O
53-->51
53-->O
54-->52
54-->B
54-->O
54-->1V
54-->DY
56-->5B
59-->CP
5B-->5E
5B-->5F
5B-->5G
5B-->5H
5B-->59
5B-->CP
5C-->5B
5G-->5E
5H-->5F
5I-->56
5I-->5C
5K-->O
5O-->1V
5O-->CP
5O-->DY
5Q-->1V
5Q-->3I
5Q-->CP
5Q-->DY
5S-->6G
5S-->1V
5S-->2O
5S-->BV
5S-->CP
5S-->DY
5U-->5K
5U-->5L
5U-->6G
5U-->5W
5U-->5Y
5U-->1V
5U-->CP
5U-->DY
5W-->1V
5W-->CP
5Y-->5L
5Y-->CP
60-->6C
60-->6D
60-->O
60-->CP
62-->6C
62-->6D
62-->6G
62-->5O
62-->5Q
62-->5S
62-->5U
62-->60
62-->65
62-->67
62-->69
62-->6B
62-->1V
62-->2O
62-->CP
62-->DY
63-->5O
63-->5Q
63-->5S
63-->5U
63-->5W
63-->60
63-->62
63-->65
63-->67
63-->69
63-->6B
65-->5W
65-->1V
65-->2O
65-->3I
65-->BV
65-->CP
65-->DY
67-->6G
67-->1V
67-->CP
67-->DY
69-->1V
69-->2O
69-->3I
69-->BV
69-->CP
69-->DY
6B-->6G
6B-->1V
6B-->CP
6B-->DY
6C-->6D
6C-->O
6D-->O
6E-->62
6E-->BV
6F-->63
6F-->6E
6G-->5K
6G-->1V
6G-->2O
6G-->3I
6K-->O
6K-->35
6K-->CP
6K-->DY
6L-->6K
6M-->6L
6O-->CP
6R-->6O
6R-->76
6R-->6T
6R-->6V
6R-->1V
6R-->2O
6R-->CP
6R-->DY
6T-->6O
6T-->CP
6V-->6O
6V-->CP
6X-->6R
6X-->70
6X-->72
6X-->1V
6X-->CP
6Y-->6R
6Y-->6T
6Y-->6V
6Y-->6X
6Y-->70
6Y-->72
70-->1V
70-->2O
70-->BV
70-->CP
70-->DY
72-->75
72-->76
72-->1V
72-->2O
72-->BV
72-->CP
72-->DY
73-->6X
73-->BV
74-->6Y
74-->73
76-->1V
76-->2O
76-->3I
7A-->7J
7A-->CP
7C-->7K
7C-->7E
7C-->CP
7E-->7K
7E-->CP
7G-->7K
7G-->CP
7H-->7A
7H-->7C
7H-->7E
7H-->7G
7L-->7A
7L-->7C
7L-->7G
7M-->7H
7M-->7K
7M-->7L
7Q-->1B
7Q-->CP
7R-->7Q
7S-->7Q
7T-->7R
7T-->7S
7V-->O
7Y-->8R
7Y-->4B
7Y-->CP
80-->CP
82-->86
82-->CP
84-->86
84-->CP
86-->CP
88-->7V
88-->86
88-->CP
8A-->8V
8A-->CP
8C-->7V
8C-->CP
8E-->7V
8E-->8V
8E-->8W
8E-->82
8E-->84
8E-->88
8E-->8A
8E-->8C
8E-->8G
8E-->O
8E-->14
8E-->CP
8G-->14
8G-->CP
8G-->DY
8I-->8R
8I-->8W
8I-->8P
8I-->O
8I-->14
8I-->CP
8K-->O
8K-->CP
8M-->8S
8M-->80
8M-->8E
8M-->8I
8M-->8K
8M-->1V
8M-->CP
8N-->8E
8N-->8G
8N-->8I
8N-->8K
8N-->8M
8P-->8R
8P-->7Y
8P-->O
8P-->CP
8Q-->B
8Q-->O
8Q-->14
8Q-->1V
8R-->O
8S-->7V
8S-->8Q
8S-->8R
8S-->8W
8S-->O
8S-->14
8S-->1V
8S-->4B
8T-->8M
8U-->7V
8U-->8N
8U-->8Q
8U-->8R
8U-->8T
8U-->8V
8U-->8W
8V-->7V
8W-->O
8Y-->2O
91-->8Y
91-->A0
91-->CP
91-->DY
93-->9W
93-->CP
95-->CP
97-->9X
97-->9Y
97-->A0
97-->1V
97-->2O
97-->CP
99-->A0
99-->39
99-->BV
99-->CP
9B-->9V
9B-->A0
9B-->95
9B-->2O
9B-->BV
9B-->CP
9B-->DY
9D-->9Y
9D-->A0
9D-->2O
9D-->CP
9D-->DY
9F-->1V
9F-->7M
9F-->CP
9H-->9W
9H-->A0
9H-->CP
9J-->A0
9J-->A3
9J-->97
9J-->9B
9J-->9D
9J-->9F
9J-->9N
9J-->9U
9J-->1V
9J-->2O
9J-->7M
9J-->BV
9J-->CP
9J-->DY
9L-->9W
9L-->A0
9L-->91
9L-->93
9L-->99
9L-->9H
9L-->CP
9N-->A0
9N-->1V
9N-->2O
9N-->3I
9N-->BV
9N-->CP
9N-->DY
9O-->91
9O-->93
9O-->95
9O-->97
9O-->9B
9O-->9D
9O-->9F
9O-->9H
9O-->9J
9O-->9L
9O-->9N
9O-->9Q
9O-->9S
9O-->9U
9Q-->9S
9Q-->CP
9S-->9Y
9S-->A0
9S-->A2
9S-->A3
9S-->CP
9S-->DY
9U-->A0
9U-->A2
9U-->A3
9U-->9S
9U-->O
9U-->CP
9V-->2O
9V-->BV
9V-->DY
9W-->O
9X-->9Y
9X-->O
9X-->2O
9X-->CP
9X-->DY
9Y-->DY
9Z-->9J
9Z-->9L
9Z-->9Q
9Z-->BV
A0-->8Y
A0-->9Y
A0-->O
A0-->1V
A0-->2O
A0-->39
A0-->3I
A1-->9O
A1-->9W
A1-->9Y
A1-->9Z
A1-->A0
A1-->A2
A1-->A3
A2-->O
A3-->O
A6-->A8
A6-->AC
A6-->AE
A6-->AG
A8-->AA
A8-->AC
A8-->AE
A8-->AG
A8-->CP
AA-->AJ
AA-->4Y
AA-->CP
AC-->AJ
AC-->1G
AC-->1V
AC-->4Y
AC-->CP
AE-->O
AE-->1G
AE-->1V
AE-->4Y
AE-->CP
AE-->DY
AG-->O
AG-->1V
AG-->4Y
AG-->CP
AG-->DY
AH-->A6
AH-->AI
AI-->A8
AJ-->4Y
AJ-->CP
AM-->AU
AO-->1B
AO-->CP
AQ-->1V
AQ-->CP
AQ-->DY
AS-->6M
AS-->CP
AU-->AO
AU-->AQ
AU-->AS
AU-->AW
AU-->AY
AU-->CP
AW-->1V
AW-->CP
AW-->DY
AY-->1V
AY-->39
AY-->CP
AZ-->AM
AZ-->B0
B0-->AU
B2-->O
B5-->O
B5-->1V
B5-->3E
B5-->CP
B5-->DY
B7-->B2
B8-->BA
B8-->BC
B8-->BE
B8-->BI
B8-->BK
BA-->B2
BA-->B
BA-->1V
BA-->CP
BC-->B2
BC-->B5
BC-->B
BC-->O
BC-->1V
BC-->3E
BC-->4Y
BC-->CP
BE-->B2
BE-->BM
BE-->B
BE-->1V
BE-->CP
BE-->DY
BG-->B2
BG-->BN
BG-->B7
BG-->CP
BG-->DY
BI-->B2
BI-->BM
BI-->BN
BI-->BA
BI-->BC
BI-->BE
BI-->BG
BI-->BK
BI-->B
BI-->O
BI-->1V
BI-->3I
BI-->4Y
BI-->CP
BI-->DY
BK-->1V
BK-->3I
BK-->CP
BK-->DY
BL-->BO
BM-->O
BM-->3I
BN-->O
BO-->BI
BV-->BR
BV-->BS
BV-->BU
BV-->BW
BV-->BX
BX-->DY
C0-->DY
C2-->DY
C4-->DY
C6-->DY
C8-->C4
C8-->CT
C8-->CX
C8-->DH
CA-->DY
CC-->CG
CC-->DY
CE-->DY
CG-->DY
CI-->CM
CI-->DH
CK-->DY
CM-->DY
CP-->C0
CP-->C2
CP-->C4
CP-->C6
CP-->C8
CP-->CA
CP-->CC
CP-->CE
CP-->CG
CP-->CI
CP-->CK
CP-->CM
CP-->CO
CP-->CR
CP-->CT
CP-->CV
CP-->CX
CP-->CZ
CP-->D1
CP-->D3
CP-->D5
CP-->D7
CP-->D9
CP-->DB
CP-->DD
CP-->DF
CP-->DH
CR-->DY
CT-->DY
CV-->CM
CX-->DY
CZ-->CM
CZ-->DH
D1-->C4
D1-->CM
D1-->DH
D1-->DY
D3-->DY
D5-->DY
D7-->C4
D7-->CC
D7-->CM
D9-->DY
DB-->D5
DB-->DH
DB-->DY
DD-->DY
DF-->DY
DH-->DY
DJ-->DR
DM-->DW
DP-->DW
DQ-->DR
DR-->DW
DV-->DW
DY-->DJ
DY-->DK
DY-->DL
DY-->DM
DY-->DN
DY-->DO
DY-->DP
DY-->DR
DY-->DQ
DY-->DS
DY-->DT
DY-->DU
DY-->DW
DY-->DX
DY-->DZ
DY-->E0
DY-->E1
DY-->E2
DY-->E3
DY-->E4
DY-->E5
DY-->E6
DY-->E7
DY-->E8
DY-->E9
DY-->ED
DY-->EE
E1-->DW
E6-->DM
```
