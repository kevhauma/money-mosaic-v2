...
BK["index.ts"]
subgraph BL["loan-card"]
BM["loan-card.component.ts"]
end
subgraph BN["loan-detail"]
BO["loan-detail.component.ts"]
end
subgraph BP["loan-form"]
BQ["loan-form.component.ts"]
end
subgraph BR["loans-overview"]
BS["loans-overview.component.ts"]
end
end
BT["index.ts"]
BU["loan-card-vm.ts"]
...
subgraph BY["feature-recurring"]
BZ["bills-calendar-vm.ts"]
subgraph C0["components"]
subgraph C1["bills-calendar"]
C2["bills-calendar.component.ts"]
end
subgraph C3["bills-day-list"]
C4["bills-day-list.component.ts"]
end
subgraph C5["bills-month-grid"]
C6["bills-month-grid.component.ts"]
end
C7["index.ts"]
subgraph C8["recurring-overview"]
C9["recurring-overview.component.ts"]
end
subgraph CA["recurring-payments-panel"]
CB["recurring-payments-panel.component.ts"]
end
end
CC["index.ts"]
CD["recurring-payments-row-vm.ts"]
CE["recurring-series.store.ts"]
CF["recurring.routes.ts"]
end
subgraph CG["feature-settings"]
subgraph CH["components"]
CI["index.ts"]
subgraph CJ["settings-about-section"]
CK["settings-about-section.component.ts"]
end
subgraph CL["settings-currency-locale-section"]
CM["settings-currency-locale-section.component.ts"]
end
subgraph CN["settings-data-section"]
CO["settings-data-section.component.ts"]
end
subgraph CP["settings-overview"]
CQ["settings-overview.component.ts"]
end
subgraph CR["settings-privacy-section"]
CS["settings-privacy-section.component.ts"]
end
subgraph CT["settings-reporting-section"]
CU["settings-reporting-section.component.ts"]
end
subgraph CV["settings-theme-section"]
CW["settings-theme-section.component.ts"]
end
end
CX["index.ts"]
CY["settings.routes.ts"]
end
subgraph CZ["feature-transactions"]
D0["category-picker.ts"]
subgraph D1["components"]
subgraph D2["attribution-override-fieldset"]
D3["attribution-override-fieldset.component.ts"]
end
subgraph D4["category-select-cell"]
D5["category-select-cell.component.ts"]
end
D6["index.ts"]
subgraph D7["transaction-bulk-bar"]
D8["transaction-bulk-bar.component.ts"]
end
subgraph D9["transaction-edit-form"]
DA["transaction-edit-form.component.ts"]
end
subgraph DB["transaction-filters"]
DC["transaction-filters.component.ts"]
end
subgraph DD["transaction-row"]
DE["transaction-row.component.ts"]
end
subgraph DF["transactions-overview"]
DG["transactions-overview.component.ts"]
end
subgraph DH["transfer-review"]
DI["transfer-review.component.ts"]
end
end
DJ["index.ts"]
DK["transaction-filters.ts"]
DL["transaction-row-vm.ts"]
DM["transactions.routes.ts"]
end
subgraph DN["shared"]
subgraph DO["echarts"]
DP["bucketed-axis-option.ts"]
DQ["chart-theme.ts"]
DR["echarts-jsdom.testing.ts"]
DS["echarts-setup.ts"]
DT["index.ts"]
DU["legend-option.ts"]
DV["tooltip-formatter.ts"]
end
subgraph DW["ui"]
subgraph DX["absolute-range-panel"]
DY["absolute-range-panel.component.ts"]
end
subgraph DZ["alert"]
E0["alert.component.ts"]
end
subgraph E1["badge"]
E2["badge.component.ts"]
end
subgraph E3["button"]
E4["button.component.ts"]
end
subgraph E5["collapse"]
E6["collapse.component.ts"]
end
subgraph E7["confirm-dialog"]
E8["confirm-dialog.component.ts"]
end
subgraph E9["cycle-picker"]
EA["cycle-picker.component.ts"]
end
subgraph EB["date-range-input"]
EC["date-range-input.component.ts"]
end
subgraph ED["divider"]
EE["divider.component.ts"]
end
subgraph EF["dropdown"]
EG["dropdown.component.ts"]
end
subgraph EH["empty-state"]
EI["empty-state.component.ts"]
end
subgraph EJ["fieldset"]
EK["fieldset.component.ts"]
end
subgraph EL["flex"]
EM["flex.component.ts"]
end
subgraph EN["granularity-picker"]
EO["granularity-picker.component.ts"]
end
EP["index.ts"]
subgraph EQ["input"]
ER["input.component.ts"]
end
subgraph ES["label"]
ET["label.component.ts"]
end
subgraph EU["loading-skeleton"]
EV["loading-skeleton.component.ts"]
end
subgraph EW["modal"]
EX["mm-modal.component.ts"]
end
subgraph EY["page-header"]
EZ["page-header.component.ts"]
end
subgraph F0["paginator"]
F1["paginator.component.ts"]
end
subgraph F2["paper"]
F3["paper.component.ts"]
end
subgraph F4["privacy-blur"]
F5["privacy-blur.component.ts"]
end
subgraph F6["privacy-toggle"]
F7["privacy-toggle.component.ts"]
end
subgraph F8["range-picker"]
F9["range-picker.component.ts"]
end
subgraph FA["select"]
FB["select.component.ts"]
end
subgraph FC["stat-card"]
FD["stat-card.component.ts"]
end
subgraph FE["table"]
FF["table.component.ts"]
end
subgraph FG["tabs"]
FH["tabs.component.ts"]
end
subgraph FI["typography"]
FJ["typography.component.ts"]
end
end
subgraph FK["utils"]
FL["calendar-cycles.ts"]
FM["confidence-color.ts"]
FN["confirm-state.ts"]
FO["currency-format.ts"]
FP["currency-symbol-presets.ts"]
FQ["daisy-classes.ts"]
FR["date-buckets.ts"]
FS["date-format.pipe.ts"]
FT["date-format.ts"]
FU["debounced-text.ts"]
FV["download-json.ts"]
FW["fingerprint.ts"]
FX["format-settings.testing.ts"]
FY["format-settings.ts"]
FZ["hidden-amount.ts"]
G0["iban.ts"]
G1["index.ts"]
G2["link-control-to-setting.ts"]
G3["locale-presets.ts"]
G4["number-format.ts"]
G5["pagination.ts"]
G6["percentage.ts"]
G7["quick-ranges.ts"]
G8["range-expression.ts"]
G9["search-params.ts"]
GA["selection-model.ts"]
GB["signed-amount.pipe.ts"]
GC["sortable.ts"]
GD["structural-filters.ts"]
GE["theme-hooks.ts"]
subgraph GF["validators"]
GG["iban.validator.ts"]
GH["percentage.validator.ts"]
end
GI["with-archivable.ts"]
GJ["with-persisted-crud.ts"]
end
end
end
end
