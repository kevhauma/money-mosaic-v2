...
FG["pagination.ts"]
FH["percentage.ts"]
FI["range-expression.ts"]
FJ["search-params.ts"]
FK["selection-model.ts"]
FL["signed-amount.pipe.ts"]
FM["sortable.ts"]
FN["structural-filters.ts"]
FO["theme-hooks.ts"]
subgraph FP["validators"]
FQ["iban.validator.ts"]
FR["percentage.validator.ts"]
end
FS["with-archivable.ts"]
FT["with-persisted-crud.ts"]
end
end
end
end
