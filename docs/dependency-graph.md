...
subgraph C4["feature-recurring"]
C5["bills-calendar-vm.ts"]
subgraph C6["components"]
subgraph C7["bills-calendar"]
C8["bills-calendar.component.ts"]
end
subgraph C9["bills-day-list"]
CA["bills-day-list.component.ts"]
end
subgraph CB["bills-month-grid"]
CC["bills-month-grid.component.ts"]
end
CD["index.ts"]
subgraph CE["recurring-overview"]
CF["recurring-overview.component.ts"]
end
subgraph CG["recurring-payments-panel"]
CH["recurring-payments-panel.component.ts"]
end
end
CI["index.ts"]
CJ["recurring-payments-row-vm.ts"]
CK["recurring-series.store.ts"]
CL["recurring.routes.ts"]
end
subgraph CM["feature-settings"]
subgraph CN["components"]
CO["index.ts"]
subgraph CP["settings-about-section"]
CQ["settings-about-section.component.ts"]
end
subgraph CR["settings-currency-locale-section"]
CS["settings-currency-locale-section.component.ts"]
end
subgraph CT["settings-data-section"]
CU["settings-data-section.component.ts"]
end
subgraph CV["settings-overview"]
CW["settings-overview.component.ts"]
end
subgraph CX["settings-privacy-section"]
CY["settings-privacy-section.component.ts"]
end
subgraph CZ["settings-reporting-section"]
D0["settings-reporting-section.component.ts"]
end
subgraph D1["settings-theme-section"]
D2["settings-theme-section.component.ts"]
end
end
D3["index.ts"]
D4["settings.routes.ts"]
end
subgraph D5["feature-transactions"]
D6["category-picker.ts"]
subgraph D7["components"]
subgraph D8["attribution-override-fieldset"]
D9["attribution-override-fieldset.component.ts"]
end
subgraph DA["category-select-cell"]
DB["category-select-cell.component.ts"]
end
DC["index.ts"]
subgraph DD["transaction-bulk-bar"]
DE["transaction-bulk-bar.component.ts"]
end
subgraph DF["transaction-edit-form"]
DG["transaction-edit-form.component.ts"]
end
subgraph DH["transaction-filters"]
DI["transaction-filters.component.ts"]
end
subgraph DJ["transaction-row"]
DK["transaction-row.component.ts"]
end
subgraph DL["transactions-overview"]
DM["transactions-overview.component.ts"]
end
subgraph DN["transfer-review"]
DO["transfer-review.component.ts"]
end
end
DP["index.ts"]
DQ["transaction-filters.ts"]
DR["transaction-row-vm.ts"]
DS["transactions.routes.ts"]
end
subgraph DT["shared"]
subgraph DU["echarts"]
DV["bucketed-axis-option.ts"]
DW["chart-theme.ts"]
DX["echarts-jsdom.testing.ts"]
DY["echarts-setup.ts"]
DZ["index.ts"]
E0["legend-option.ts"]
E1["tooltip-formatter.ts"]
end
subgraph E2["ui"]
subgraph E3["absolute-range-panel"]
E4["absolute-range-panel.component.ts"]
end
subgraph E5["alert"]
E6["alert.component.ts"]
end
subgraph E7["badge"]
E8["badge.component.ts"]
end
subgraph E9["button"]
EA["button.component.ts"]
end
subgraph EB["collapse"]
EC["collapse.component.ts"]
end
subgraph ED["confirm-dialog"]
EE["confirm-dialog.component.ts"]
end
subgraph EF["cycle-picker"]
EG["cycle-picker.component.ts"]
end
subgraph EH["date-range-input"]
EI["date-range-input.component.ts"]
end
subgraph EJ["divider"]
EK["divider.component.ts"]
end
subgraph EL["dropdown"]
EM["dropdown.component.ts"]
end
subgraph EN["empty-state"]
EO["empty-state.component.ts"]
end
subgraph EP["fieldset"]
EQ["fieldset.component.ts"]
end
subgraph ER["flex"]
ES["flex.component.ts"]
end
subgraph ET["granularity-picker"]
EU["granularity-picker.component.ts"]
end
EV["index.ts"]
subgraph EW["input"]
EX["input.component.ts"]
end
subgraph EY["label"]
EZ["label.component.ts"]
end
subgraph F0["loading-skeleton"]
F1["loading-skeleton.component.ts"]
end
subgraph F2["modal"]
F3["mm-modal.component.ts"]
end
subgraph F4["page-header"]
F5["page-header.component.ts"]
end
subgraph F6["paginator"]
F7["paginator.component.ts"]
end
subgraph F8["paper"]
F9["paper.component.ts"]
end
subgraph FA["privacy-blur"]
FB["privacy-blur.component.ts"]
end
subgraph FC["privacy-toggle"]
FD["privacy-toggle.component.ts"]
end
subgraph FE["range-picker"]
FF["range-picker.component.ts"]
end
subgraph FG["select"]
FH["select.component.ts"]
end
subgraph FI["stat-card"]
FJ["stat-card.component.ts"]
end
subgraph FK["table"]
FL["table.component.ts"]
end
subgraph FM["tabs"]
FN["tabs.component.ts"]
end
subgraph FO["typography"]
FP["typography.component.ts"]
end
end
subgraph FQ["utils"]
FR["calendar-cycles.ts"]
FS["confidence-color.ts"]
FT["confirm-state.ts"]
FU["currency-format.ts"]
FV["currency-symbol-presets.ts"]
FW["daisy-classes.ts"]
FX["date-buckets.ts"]
FY["date-format.pipe.ts"]
FZ["date-format.ts"]
G0["debounced-text.ts"]
G1["download-json.ts"]
G2["fingerprint.ts"]
G3["format-settings.testing.ts"]
G4["format-settings.ts"]
G5["hidden-amount.ts"]
G6["iban.ts"]
G7["index.ts"]
G8["link-control-to-setting.ts"]
G9["locale-presets.ts"]
GA["number-format.ts"]
GB["pagination.ts"]
GC["percentage.ts"]
GD["quick-ranges.ts"]
GE["range-expression.ts"]
GF["search-params.ts"]
GG["selection-model.ts"]
GH["signed-amount.pipe.ts"]
GI["sortable.ts"]
GJ["structural-filters.ts"]
GK["theme-hooks.ts"]
subgraph GL["validators"]
GM["iban.validator.ts"]
GN["percentage.validator.ts"]
end
GO["with-archivable.ts"]
GP["with-persisted-crud.ts"]
end
end
end
end
