...
A6["percentage.ts"]
A7["search-params.ts"]
A8["selection-model.ts"]
A9["signed-amount.pipe.ts"]
AA["sortable.ts"]
AB["structural-filters.ts"]
AC["theme-hooks.ts"]
subgraph AD["validators"]
AE["iban.validator.ts"]
AF["percentage.validator.ts"]
end
AG["with-archivable.ts"]
AH["with-persisted-crud.ts"]
end
end
end
end
