# Dependency graph reference

Full import graph of `src/app`, generated with `dependency-cruiser`, for Claude to consult
when tracing what imports what without re-grepping the tree by hand.

## Scope

Unlike the human-facing overview in [`reports/dependency-graph.html`](../reports/dependency-graph.html)
(which collapses each `feature-*` folder to a single node and stops one folder into
`core`/`shared`), this graph goes three folder levels deeper on both tiers — up to 4 folders
below `src/app` — which is enough to reach individual files in most feature/core/shared
subtrees.

<!-- GENERATED:STATS:START -->
- 317 leaf nodes, 902 edges.
<!-- GENERATED:STATS:END -->

- `*.spec.ts` and `node_modules` are excluded.
- Node IDs are dependency-cruiser's internal short hashes (`4`, `2K`, `6N`, ...) — read them
  via the `subgraph`/node label text, not the ID itself.
- An edge collapsed at a folder boundary (rare here, since most subtrees fit within 4 levels)
  means _something_ inside that folder imports the target, not necessarily its `index.ts`.

## Regenerate

```bash
depcruise src/app \
  --config .dependency-cruiser.cjs \
  --collapse '^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+|^src/app/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+|^src/app/[^/]+' \
  --output-type mermaid \
  --output-to reports/dependency-graph-deep.mmd
```

## Graph

```mermaid
flowchart LR

subgraph 0["src"]
subgraph 1["app"]
subgraph 2["core"]
subgraph 3["accounts"]
4["account-deletion.service.ts"]
5["index.ts"]
6["joint-owner-lookup.ts"]
end
subgraph 7["categorisation"]
8["co-owner-contribution.service.ts"]
9["co-owner-contribution.ts"]
A["index.ts"]
B["rule-matching.ts"]
C["rules-engine.service.ts"]
end
subgraph D["data-access"]
E["accounts.repository.ts"]
F["app-db.ts"]
G["app-settings.repository.ts"]
H["categories.repository.ts"]
I["category-comparison-settings.repository.ts"]
J["category-model.repository.ts"]
K["dashboard-layout-settings.repository.ts"]
L["data-management.repository.ts"]
M["import-batches.repository.ts"]
N["index.ts"]
O["mapping-profiles.repository.ts"]
P["rules.repository.ts"]
Q["salary-metadata.repository.ts"]
R["transactions.repository.ts"]
S["transfer-settings.repository.ts"]
T["transfers.repository.ts"]
end
subgraph U["import"]
V["account-detection.ts"]
W["csv-import.service.ts"]
X["csv-parse.ts"]
Y["csv-parse.worker.ts"]
Z["csv-row-mapper.ts"]
10["csv-worker.types.ts"]
11["delimiter-guess.ts"]
12["import.service.ts"]
13["index.ts"]
end
subgraph 14["layout"]
subgraph 15["app-shell"]
16["app-shell.component.ts"]
end
17["index.ts"]
end
subgraph 18["links"]
19["external-links.ts"]
1A["index.ts"]
end
subgraph 1B["ml"]
1C["category-model.worker.ts"]
1D["category-model.worker.types.ts"]
1E["feature-hashing.ts"]
1F["index.ts"]
1G["model-config.ts"]
1H["rule-proposal-mining.ts"]
1I["training-window.ts"]
end
subgraph 1J["onboarding"]
1K["home-redirect.guard.ts"]
1L["index.ts"]
1M["mark-visited.guard.ts"]
1N["visited.service.ts"]
end
subgraph 1O["state"]
1P["accounts.store.ts"]
1Q["app-settings.store.ts"]
1R["categories.store.ts"]
1S["index.ts"]
1T["range-state.store.ts"]
1U["transactions.store.ts"]
1V["transfer-settings.store.ts"]
1W["transfers.store.ts"]
end
subgraph 1X["stats"]
1Y["account-balance-trend.ts"]
1Z["annual-lump-sum-smoothing.ts"]
20["category-breakdown.ts"]
21["category-composition-trend.ts"]
22["category-kind-contribution.ts"]
23["category-period-comparison.ts"]
24["chart-zoom-window.ts"]
25["classify-for-stats.ts"]
26["classify-joint-leg.ts"]
27["embedded-bonus-smoothing.ts"]
28["full-history-range.ts"]
29["granularity-for-span.ts"]
2A["gross-net-growth.ts"]
2B["gross-net-ratio.ts"]
2C["income-category-series.ts"]
2D["income-events.ts"]
2E["income-gap-detection.ts"]
2F["income-growth.ts"]
2G["income-step-change-detection.ts"]
2H["index.ts"]
2I["joint-account-stake.ts"]
2J["joint-contributor-breakdown.ts"]
2K["multi-year-income-comparison.ts"]
2L["net-margin.ts"]
2M["net-worth-trend.ts"]
2N["period-stats.ts"]
2O["period-window.ts"]
2P["periodized-rate.ts"]
2Q["top-transactions.ts"]
2R["weekday-weekend-split.ts"]
2S["year-over-year.ts"]
2T["yearly-income-summary.ts"]
end
subgraph 2U["storage"]
2V["index.ts"]
2W["storage-status.service.ts"]
end
subgraph 2X["theme"]
2Y["accent-colors.ts"]
2Z["index.ts"]
30["theme-styles.ts"]
31["theme.service.ts"]
end
subgraph 32["transactions"]
33["attribution-override.ts"]
34["index.ts"]
35["nullify-transaction.ts"]
36["transaction-deletion.service.ts"]
end
subgraph 37["transfers"]
38["index.ts"]
39["transfer-cleanup.service.ts"]
3A["transfer-linking.service.ts"]
3B["transfer-matching.service.ts"]
3C["transfer-matching.ts"]
end
end
subgraph 3D["feature-accounts"]
3E["account-card-vm.ts"]
3F["account-icons.ts"]
3G["account-types.ts"]
3H["accounts.routes.ts"]
3I["balance-trend-signals.ts"]
subgraph 3J["components"]
subgraph 3K["account-balance-block"]
3L["account-balance-block.component.ts"]
end
subgraph 3M["account-balance-chart"]
3N["account-balance-chart.component.ts"]
end
subgraph 3O["account-card"]
3P["account-card.component.ts"]
end
subgraph 3Q["account-form"]
3R["account-form.component.ts"]
end
subgraph 3S["accounts-detail"]
3T["accounts-detail.component.ts"]
end
subgraph 3U["accounts-overview"]
3V["accounts-overview.component.ts"]
end
3W["index.ts"]
subgraph 3X["net-worth-history-chart"]
3Y["net-worth-history-chart.component.ts"]
end
end
3Z["index.ts"]
end
subgraph 40["feature-categories"]
41["categories.routes.ts"]
42["category-icons.ts"]
43["category-model.service.ts"]
44["category-model.store.ts"]
subgraph 45["components"]
subgraph 46["categories-overview"]
47["categories-overview.component.ts"]
end
subgraph 48["category-form"]
49["category-form.component.ts"]
end
4A["index.ts"]
subgraph 4B["rule-condition-row"]
4C["rule-condition-row.component.ts"]
end
subgraph 4D["rule-filters"]
4E["rule-filters.component.ts"]
end
subgraph 4F["rule-form"]
4G["rule-form.component.ts"]
end
subgraph 4H["rule-share-bar"]
4I["rule-share-bar.component.ts"]
end
subgraph 4J["rules-overview"]
4K["rules-overview.component.ts"]
end
end
4L["index.ts"]
4M["rule-condition-editor.ts"]
4N["rule-filters.ts"]
4O["rule-labels.ts"]
4P["rule-share.ts"]
4Q["rule-summary.ts"]
4R["rules.store.ts"]
end
subgraph 4S["feature-changelog"]
4T["changelog.routes.ts"]
subgraph 4U["components"]
subgraph 4V["changelog-entry-row"]
4W["changelog-entry-row.component.ts"]
end
subgraph 4X["changelog-page"]
4Y["changelog-page.component.ts"]
end
4Z["index.ts"]
end
subgraph 50["data"]
51["changelog-entries.ts"]
52["roadmap-entries.ts"]
end
53["group-changelog-entries.ts"]
54["group-roadmap-entries.ts"]
55["index.ts"]
end
subgraph 56["feature-dashboard"]
57["category-comparison-settings.store.ts"]
58["category-comparison-vm.ts"]
subgraph 59["components"]
subgraph 5A["account-balance-strip"]
5B["account-balance-strip.component.ts"]
end
subgraph 5C["action-queue-panel"]
5D["action-queue-panel.component.ts"]
end
subgraph 5E["category-breakdown-panel"]
5F["category-breakdown-panel.component.ts"]
end
subgraph 5G["category-comparison-panel"]
5H["category-comparison-panel.component.ts"]
end
subgraph 5I["comparison-category-card"]
5J["comparison-category-card.component.ts"]
end
subgraph 5K["dashboard-customize-panel"]
5L["dashboard-customize-panel.component.ts"]
end
subgraph 5M["dashboard-overview"]
5N["dashboard-overview.component.ts"]
end
5O["index.ts"]
subgraph 5P["net-worth-header"]
5Q["net-worth-header.component.ts"]
end
subgraph 5R["top-transactions-panel"]
5S["top-transactions-panel.component.ts"]
end
subgraph 5T["trend-chart-panel"]
5U["trend-chart-panel.component.ts"]
end
subgraph 5V["weekday-weekend-split-panel"]
5W["weekday-weekend-split-panel.component.ts"]
end
end
5X["dashboard-layout-settings.store.ts"]
5Y["dashboard-row-order.ts"]
5Z["dashboard.routes.ts"]
60["index.ts"]
61["stats.store.ts"]
end
subgraph 62["feature-data-management"]
subgraph 63["components"]
subgraph 64["data-management-overview"]
65["data-management-overview.component.ts"]
end
66["index.ts"]
end
67["index.ts"]
end
subgraph 68["feature-help"]
subgraph 69["components"]
subgraph 6A["faq-page"]
6B["faq-page.component.ts"]
end
subgraph 6C["guide-detail"]
6D["guide-detail.component.ts"]
end
subgraph 6E["guide-steps"]
6F["guide-steps.component.ts"]
end
subgraph 6G["guides-index"]
6H["guides-index.component.ts"]
end
6I["index.ts"]
end
subgraph 6J["data"]
6K["faq.ts"]
6L["guides.ts"]
end
6M["help.routes.ts"]
6N["index.ts"]
end
subgraph 6O["feature-home"]
subgraph 6P["components"]
subgraph 6Q["home-landing"]
6R["home-landing.component.ts"]
end
6S["index.ts"]
end
6T["home.routes.ts"]
6U["index.ts"]
end
subgraph 6V["feature-import"]
6W["column-mapping.ts"]
subgraph 6X["components"]
subgraph 6Y["account-draft-editor"]
6Z["account-draft-editor.component.ts"]
end
subgraph 70["batch-wait-card"]
71["batch-wait-card.component.ts"]
end
subgraph 72["column-map-amount-field"]
73["column-map-amount-field.component.ts"]
end
subgraph 74["column-map-counterparty-field"]
75["column-map-counterparty-field.component.ts"]
end
subgraph 76["column-map-sample-caption"]
77["column-map-sample-caption.component.ts"]
end
subgraph 78["column-map-simple-field"]
79["column-map-simple-field.component.ts"]
end
subgraph 7A["column-map-stepper"]
7B["column-map-stepper.component.ts"]
end
subgraph 7C["column-map-summary-step"]
7D["column-map-summary-step.component.ts"]
end
subgraph 7E["import-map-step"]
7F["import-map-step.component.ts"]
end
subgraph 7G["import-preview-step"]
7H["import-preview-step.component.ts"]
end
subgraph 7I["import-select-step"]
7J["import-select-step.component.ts"]
end
subgraph 7K["import-summary-step"]
7L["import-summary-step.component.ts"]
end
subgraph 7M["import-wizard"]
7N["import-wizard.component.ts"]
end
7O["index.ts"]
subgraph 7P["queued-file-row"]
7Q["queued-file-row.component.ts"]
end
end
7R["import-batches.store.ts"]
7S["import-queue.ts"]
7T["import-wizard-session.ts"]
7U["import.routes.ts"]
7V["index.ts"]
7W["mapper-steps.ts"]
7X["mapping-profiles.store.ts"]
end
subgraph 7Y["feature-income"]
7Z["career-start-date.ts"]
subgraph 80["components"]
subgraph 81["income-career-start"]
82["income-career-start.component.ts"]
end
subgraph 83["income-category-checklist"]
84["income-category-checklist.component.ts"]
end
subgraph 85["income-chart-cell"]
86["income-chart-cell.component.ts"]
end
subgraph 87["income-events-sidebar"]
88["income-events-sidebar.component.ts"]
end
subgraph 89["income-gross-color"]
8A["income-gross-color.component.ts"]
end
subgraph 8B["income-gross-net-section"]
8C["income-gross-net-section.component.ts"]
end
subgraph 8D["income-growth-panel"]
8E["income-growth-panel.component.ts"]
end
subgraph 8F["income-intro"]
8G["income-intro.component.ts"]
end
subgraph 8H["income-overview"]
8I["income-overview.component.ts"]
end
subgraph 8J["income-settings-page"]
8K["income-settings-page.component.ts"]
end
subgraph 8L["income-yearly-panel"]
8M["income-yearly-panel.component.ts"]
end
8N["index.ts"]
subgraph 8O["salary-details-page"]
8P["salary-details-page.component.ts"]
end
subgraph 8Q["salary-metadata-table"]
8R["salary-metadata-table.component.ts"]
end
subgraph 8S["salary-month-modal"]
8T["salary-month-modal.component.ts"]
end
end
8U["gross-net-chart-options.ts"]
8V["income-category-vm.ts"]
8W["income-event-vm.ts"]
8X["income-granularity.ts"]
8Y["income.routes.ts"]
8Z["income.store.ts"]
90["index.ts"]
91["salary-metadata-edit.ts"]
92["salary-metadata-rows.ts"]
end
subgraph 93["feature-learning"]
subgraph 94["components"]
95["index.ts"]
subgraph 96["learning-overview"]
97["learning-overview.component.ts"]
end
subgraph 98["model-status"]
99["model-status.component.ts"]
end
subgraph 9A["rule-proposals"]
9B["rule-proposals.component.ts"]
end
subgraph 9C["suggestions-table"]
9D["suggestions-table.component.ts"]
end
end
9E["index.ts"]
9F["learning.routes.ts"]
end
subgraph 9G["feature-settings"]
subgraph 9H["components"]
9I["index.ts"]
subgraph 9J["settings-about-section"]
9K["settings-about-section.component.ts"]
end
subgraph 9L["settings-currency-locale-section"]
9M["settings-currency-locale-section.component.ts"]
end
subgraph 9N["settings-data-section"]
9O["settings-data-section.component.ts"]
end
subgraph 9P["settings-overview"]
9Q["settings-overview.component.ts"]
end
subgraph 9R["settings-theme-section"]
9S["settings-theme-section.component.ts"]
end
end
9T["index.ts"]
9U["settings.routes.ts"]
end
subgraph 9V["feature-transactions"]
subgraph 9W["components"]
subgraph 9X["attribution-override-fieldset"]
9Y["attribution-override-fieldset.component.ts"]
end
subgraph 9Z["category-select-cell"]
A0["category-select-cell.component.ts"]
end
A1["index.ts"]
subgraph A2["transaction-bulk-bar"]
A3["transaction-bulk-bar.component.ts"]
end
subgraph A4["transaction-edit-form"]
A5["transaction-edit-form.component.ts"]
end
subgraph A6["transaction-filters"]
A7["transaction-filters.component.ts"]
end
subgraph A8["transaction-row"]
A9["transaction-row.component.ts"]
end
subgraph AA["transactions-overview"]
AB["transactions-overview.component.ts"]
end
subgraph AC["transfer-review"]
AD["transfer-review.component.ts"]
end
end
AE["index.ts"]
AF["transaction-filters.ts"]
AG["transaction-row-vm.ts"]
AH["transactions.routes.ts"]
end
subgraph AI["shared"]
subgraph AJ["echarts"]
AK["bucketed-axis-option.ts"]
AL["chart-theme.ts"]
AM["echarts-setup.ts"]
AN["index.ts"]
AO["tooltip-formatter.ts"]
end
subgraph AP["ui"]
subgraph AQ["alert"]
AR["alert.component.ts"]
end
subgraph AS["badge"]
AT["badge.component.ts"]
end
subgraph AU["button"]
AV["button.component.ts"]
end
subgraph AW["collapse"]
AX["collapse.component.ts"]
end
subgraph AY["confirm-dialog"]
AZ["confirm-dialog.component.ts"]
end
subgraph B0["date-range-input"]
B1["date-range-input.component.ts"]
end
subgraph B2["divider"]
B3["divider.component.ts"]
end
subgraph B4["dropdown"]
B5["dropdown.component.ts"]
end
subgraph B6["empty-state"]
B7["empty-state.component.ts"]
end
subgraph B8["fieldset"]
B9["fieldset.component.ts"]
end
subgraph BA["flex"]
BB["flex.component.ts"]
end
subgraph BC["granularity-picker"]
BD["granularity-picker.component.ts"]
end
BE["index.ts"]
subgraph BF["input"]
BG["input.component.ts"]
end
subgraph BH["label"]
BI["label.component.ts"]
end
subgraph BJ["loading-skeleton"]
BK["loading-skeleton.component.ts"]
end
subgraph BL["modal"]
BM["mm-modal.component.ts"]
end
subgraph BN["page-header"]
BO["page-header.component.ts"]
end
subgraph BP["paginator"]
BQ["paginator.component.ts"]
end
subgraph BR["paper"]
BS["paper.component.ts"]
end
subgraph BT["range-grouping-switcher"]
BU["range-grouping-switcher.component.ts"]
end
subgraph BV["select"]
BW["select.component.ts"]
end
subgraph BX["stat-card"]
BY["stat-card.component.ts"]
end
subgraph BZ["table"]
C0["table.component.ts"]
end
subgraph C1["tabs"]
C2["tabs.component.ts"]
end
subgraph C3["typography"]
C4["typography.component.ts"]
end
end
subgraph C5["utils"]
C6["confidence-color.ts"]
C7["confirm-state.ts"]
C8["currency-format.ts"]
C9["currency-symbol-presets.ts"]
CA["daisy-classes.ts"]
CB["date-buckets.ts"]
CC["date-format.pipe.ts"]
CD["date-format.ts"]
CE["debounced-text.ts"]
CF["download-json.ts"]
CG["fingerprint.ts"]
CH["format-settings.testing.ts"]
CI["format-settings.ts"]
CJ["iban.ts"]
CK["index.ts"]
CL["link-control-to-setting.ts"]
CM["locale-presets.ts"]
CN["number-format.ts"]
CO["pagination.ts"]
CP["percentage.ts"]
CQ["search-params.ts"]
CR["selection-model.ts"]
CS["signed-amount.pipe.ts"]
CT["sortable.ts"]
CU["structural-filters.ts"]
CV["theme-hooks.ts"]
subgraph CW["validators"]
CX["iban.validator.ts"]
CY["percentage.validator.ts"]
end
CZ["with-archivable.ts"]
D0["with-persisted-crud.ts"]
end
end
end
end
4-->N
4-->38
5-->4
5-->6
6-->N
6-->CK
8-->9
8-->N
9-->5
9-->N
A-->9
A-->8
A-->B
A-->C
B-->N
C-->B
C-->N
E-->F
F-->1G
F-->2Y
F-->CG
F-->CI
G-->F
G-->2Z
G-->CK
H-->F
I-->F
J-->F
K-->F
L-->F
M-->F
N-->E
N-->F
N-->G
N-->H
N-->I
N-->J
N-->K
N-->L
N-->M
N-->O
N-->P
N-->Q
N-->R
N-->S
N-->T
O-->F
P-->F
Q-->F
R-->F
S-->F
T-->F
V-->N
V-->CK
W-->10
W-->N
X-->Z
X-->10
Y-->X
Y-->10
Z-->N
10-->Z
10-->N
12-->Z
12-->N
12-->38
12-->CK
13-->V
13-->W
13-->X
13-->Z
13-->10
13-->11
13-->12
16-->1S
16-->2H
16-->AV
16-->BU
16-->C4
16-->CK
17-->16
1A-->19
1C-->1D
1C-->1E
1C-->1G
1D-->1G
1E-->1G
1F-->1D
1F-->1E
1F-->1G
1F-->1H
1F-->1I
1H-->A
1H-->N
1K-->1N
1L-->1K
1L-->1M
1L-->1N
1M-->1N
1P-->1R
1P-->1U
1P-->1W
1P-->5
1P-->N
1P-->2H
1P-->38
1P-->CK
1Q-->N
1Q-->2Z
1Q-->CK
1R-->1U
1R-->N
1R-->CK
1S-->1P
1S-->1Q
1S-->1R
1S-->1T
1S-->1U
1S-->1V
1S-->1W
1T-->CK
1U-->N
1U-->34
1U-->38
1V-->N
1W-->1U
1W-->1V
1W-->N
1W-->38
1Y-->26
1Y-->2M
1Y-->N
1Y-->CK
1Z-->21
1Z-->2C
1Z-->CK
20-->25
20-->N
21-->20
21-->N
21-->CK
22-->N
23-->20
23-->2O
23-->N
24-->CK
25-->22
25-->26
25-->N
25-->38
26-->5
26-->N
27-->2C
27-->N
27-->CK
28-->N
29-->CK
2A-->2B
2B-->2C
2B-->N
2C-->21
2C-->N
2C-->CK
2D-->2E
2D-->2G
2D-->N
2E-->21
2E-->2C
2E-->CK
2F-->2C
2F-->2S
2F-->CK
2G-->21
2G-->2C
2G-->CK
2H-->1Y
2H-->1Z
2H-->20
2H-->21
2H-->22
2H-->23
2H-->24
2H-->25
2H-->26
2H-->27
2H-->28
2H-->29
2H-->2A
2H-->2B
2H-->2C
2H-->2D
2H-->2E
2H-->2F
2H-->2G
2H-->2I
2H-->2J
2H-->2K
2H-->2L
2H-->2M
2H-->2N
2H-->2O
2H-->2P
2H-->2Q
2H-->2R
2H-->2S
2H-->2T
2I-->26
2I-->N
2J-->26
2J-->5
2J-->N
2K-->2T
2M-->26
2M-->N
2M-->CK
2N-->25
2N-->N
2O-->CK
2P-->CK
2Q-->N
2Q-->38
2R-->25
2R-->N
2R-->CK
2S-->2N
2S-->N
2T-->21
2T-->N
2T-->CK
2V-->2W
2Y-->30
2Z-->2Y
2Z-->30
2Z-->31
31-->30
33-->N
34-->33
34-->35
34-->36
35-->N
36-->N
36-->38
38-->39
38-->3A
38-->3C
38-->3B
39-->N
3A-->3C
3A-->N
3B-->3A
3B-->3C
3B-->N
3C-->6
3C-->N
3C-->CK
3E-->N
3F-->N
3G-->N
3H-->3T
3H-->3V
3H-->AN
3I-->N
3I-->1S
3I-->2H
3I-->CK
3L-->BE
3L-->CK
3N-->3I
3N-->N
3N-->2H
3N-->AN
3N-->BE
3N-->CK
3P-->3E
3P-->3F
3P-->3L
3P-->BE
3R-->3F
3R-->3G
3R-->N
3R-->BE
3R-->CK
3R-->CX
3R-->CY
3T-->3L
3T-->3N
3T-->3R
3T-->1S
3T-->BE
3T-->CK
3V-->3E
3V-->3F
3V-->3P
3V-->3R
3V-->3Y
3V-->N
3V-->1S
3V-->BE
3V-->CK
3W-->3L
3W-->3N
3W-->3P
3W-->3R
3W-->3T
3W-->3V
3W-->3Y
3Y-->3I
3Y-->N
3Y-->1S
3Y-->2H
3Y-->AN
3Y-->BE
3Z-->3F
3Z-->3G
3Z-->3H
3Z-->3W
41-->47
41-->4K
43-->1F
44-->43
44-->4R
44-->N
44-->1F
44-->1S
47-->42
47-->49
47-->N
47-->1S
47-->BE
47-->CK
49-->42
49-->N
49-->BE
4A-->47
4A-->49
4A-->4E
4A-->4G
4A-->4I
4A-->4K
4C-->4M
4C-->4O
4C-->A
4C-->N
4C-->1S
4C-->BE
4E-->4N
4E-->1S
4E-->BE
4E-->CK
4G-->4M
4G-->4C
4G-->N
4G-->1S
4G-->BE
4I-->4P
4I-->4R
4I-->BE
4I-->CK
4K-->4N
4K-->4Q
4K-->4R
4K-->4E
4K-->4G
4K-->4I
4K-->N
4K-->1S
4K-->BE
4K-->CK
4L-->41
4L-->42
4L-->43
4L-->44
4L-->4A
4L-->4N
4L-->4Q
4L-->4R
4M-->4O
4M-->N
4N-->4Q
4N-->N
4O-->N
4P-->N
4Q-->4O
4Q-->N
4R-->4P
4R-->A
4R-->N
4R-->1S
4R-->CK
4T-->4Y
4W-->BE
4Y-->51
4Y-->52
4Y-->53
4Y-->54
4Y-->4W
4Y-->BE
4Z-->4Y
53-->51
54-->52
55-->4T
55-->4Z
57-->N
5B-->1S
5B-->BE
5B-->CK
5D-->1S
5D-->38
5D-->BE
5D-->CK
5F-->61
5F-->1S
5F-->2H
5F-->AN
5F-->BE
5F-->CK
5H-->57
5H-->58
5H-->61
5H-->5J
5H-->1S
5H-->BE
5H-->CK
5J-->58
5J-->BE
5L-->5X
5L-->5Y
5L-->N
5L-->BE
5N-->5X
5N-->5Y
5N-->61
5N-->5B
5N-->5D
5N-->5F
5N-->5H
5N-->5L
5N-->5Q
5N-->5S
5N-->5U
5N-->5W
5N-->1S
5N-->2H
5N-->BE
5N-->CK
5O-->5B
5O-->5D
5O-->5F
5O-->5H
5O-->5L
5O-->5N
5O-->5Q
5O-->5S
5O-->5U
5O-->5W
5Q-->1S
5Q-->BE
5Q-->CK
5S-->61
5S-->1S
5S-->BE
5S-->CK
5U-->1S
5U-->2H
5U-->38
5U-->AN
5U-->BE
5U-->CK
5W-->61
5W-->1S
5W-->BE
5W-->CK
5X-->5Y
5X-->N
5Y-->N
5Z-->5N
5Z-->AN
60-->5O
60-->5Z
61-->57
61-->1S
61-->2H
61-->38
65-->N
65-->2V
65-->BE
65-->CK
66-->65
67-->66
6B-->6K
6B-->BE
6D-->6L
6D-->6F
6D-->BE
6F-->6L
6F-->BE
6H-->6L
6H-->BE
6I-->6B
6I-->6D
6I-->6F
6I-->6H
6M-->6B
6M-->6D
6M-->6H
6N-->6I
6N-->6L
6N-->6M
6R-->1A
6R-->BE
6S-->6R
6T-->6R
6U-->6S
6U-->6T
6W-->N
6Z-->7S
6Z-->3Z
6Z-->BE
71-->BE
73-->77
73-->BE
75-->77
75-->BE
77-->BE
79-->6W
79-->77
79-->BE
7B-->7W
7B-->BE
7D-->6W
7D-->BE
7F-->6W
7F-->7W
7F-->7X
7F-->73
7F-->75
7F-->79
7F-->7B
7F-->7D
7F-->7H
7F-->N
7F-->13
7F-->BE
7H-->13
7H-->BE
7H-->CK
7J-->7S
7J-->7X
7J-->7Q
7J-->N
7J-->13
7J-->BE
7L-->N
7L-->BE
7N-->7T
7N-->71
7N-->7F
7N-->7J
7N-->7L
7N-->1S
7N-->BE
7O-->7F
7O-->7H
7O-->7J
7O-->7L
7O-->7N
7Q-->7S
7Q-->6Z
7Q-->N
7Q-->BE
7R-->A
7R-->N
7R-->13
7R-->1S
7S-->N
7T-->6W
7T-->7R
7T-->7S
7T-->7X
7T-->N
7T-->13
7T-->1S
7T-->3Z
7U-->7N
7V-->6W
7V-->7O
7V-->7R
7V-->7S
7V-->7U
7V-->7W
7V-->7X
7W-->6W
7X-->N
7Z-->2H
82-->7Z
82-->8Z
82-->BE
82-->CK
84-->8V
84-->BE
86-->BE
88-->8W
88-->8X
88-->8Z
88-->1S
88-->2H
88-->BE
8A-->8Z
8A-->2Z
8A-->AN
8A-->BE
8C-->8U
8C-->8Z
8C-->86
8C-->2H
8C-->AN
8C-->BE
8C-->CK
8E-->8X
8E-->8Z
8E-->2H
8E-->BE
8E-->CK
8G-->1S
8G-->6N
8G-->BE
8I-->8Z
8I-->92
8I-->88
8I-->8C
8I-->8E
8I-->8G
8I-->8M
8I-->8T
8I-->1S
8I-->2H
8I-->6N
8I-->AN
8I-->BE
8I-->CK
8K-->8V
8K-->8Z
8K-->82
8K-->84
8K-->8A
8K-->BE
8M-->8Z
8M-->1S
8M-->2H
8M-->38
8M-->AN
8M-->BE
8M-->CK
8N-->82
8N-->84
8N-->86
8N-->88
8N-->8C
8N-->8E
8N-->8G
8N-->8I
8N-->8K
8N-->8M
8N-->8P
8N-->8R
8N-->8T
8P-->8R
8P-->BE
8R-->8X
8R-->8Z
8R-->91
8R-->92
8R-->BE
8R-->CK
8T-->8Z
8T-->91
8T-->92
8T-->8R
8T-->N
8T-->BE
8U-->2H
8U-->AN
8U-->CK
8V-->N
8W-->8X
8W-->N
8W-->2H
8W-->CK
8X-->CK
8Y-->8I
8Y-->8K
8Y-->8P
8Y-->AN
8Z-->7Z
8Z-->8X
8Z-->N
8Z-->1S
8Z-->2H
8Z-->2Z
8Z-->38
90-->8N
90-->8V
90-->8X
90-->8Y
90-->8Z
90-->91
90-->92
91-->N
92-->N
95-->97
95-->99
95-->9B
95-->9D
97-->99
97-->9B
97-->9D
97-->BE
99-->1F
99-->1S
99-->4L
99-->BE
9B-->N
9B-->1F
9B-->1S
9B-->4L
9B-->BE
9B-->CK
9D-->N
9D-->1S
9D-->4L
9D-->BE
9D-->CK
9E-->95
9E-->9F
9F-->97
9I-->9Q
9K-->1A
9K-->BE
9M-->1S
9M-->BE
9M-->CK
9O-->67
9O-->BE
9Q-->9K
9Q-->9M
9Q-->9O
9Q-->9S
9Q-->BE
9S-->1S
9S-->2Z
9S-->BE
9T-->9I
9T-->9U
9U-->9Q
9Y-->N
9Y-->1S
9Y-->34
9Y-->BE
9Y-->CK
A1-->A3
A1-->A5
A1-->A7
A1-->AB
A1-->AD
A3-->1S
A3-->BE
A5-->9Y
A5-->N
A5-->1S
A5-->34
A5-->4L
A5-->BE
A7-->AF
A7-->1S
A7-->BE
A7-->CK
A9-->AG
A9-->A0
A9-->BE
A9-->CK
AB-->AF
AB-->AG
AB-->A0
AB-->A3
AB-->A5
AB-->A7
AB-->A9
AB-->AD
AB-->N
AB-->1S
AB-->38
AB-->4L
AB-->BE
AB-->CK
AD-->1S
AD-->38
AD-->BE
AD-->CK
AE-->AH
AF-->N
AF-->38
AG-->N
AH-->AB
AN-->AK
AN-->AL
AN-->AM
AN-->AO
AO-->CK
AR-->CK
AT-->CK
AV-->CK
AX-->CK
AZ-->AV
AZ-->BI
AZ-->BM
AZ-->C4
B1-->B5
B1-->CK
B3-->CK
B5-->CK
B7-->BB
B7-->C4
B9-->CK
BB-->CK
BE-->AR
BE-->AT
BE-->AV
BE-->AX
BE-->AZ
BE-->B1
BE-->B3
BE-->B5
BE-->B7
BE-->B9
BE-->BB
BE-->BD
BE-->BG
BE-->BI
BE-->BK
BE-->BM
BE-->BO
BE-->BQ
BE-->BS
BE-->BU
BE-->BW
BE-->BY
BE-->C0
BE-->C2
BE-->C4
BG-->CK
BI-->CK
BK-->BB
BM-->CK
BO-->BB
BO-->C4
BQ-->AV
BQ-->BB
BQ-->C4
BQ-->CK
BS-->CK
BU-->AV
BU-->B1
BU-->BB
BW-->CK
BY-->C4
BY-->CK
C0-->CK
C2-->CK
C4-->CK
C8-->CI
CB-->CI
CC-->CD
CD-->CI
CH-->CI
CK-->C6
CK-->C7
CK-->C8
CK-->C9
CK-->CA
CK-->CB
CK-->CD
CK-->CC
CK-->CE
CK-->CF
CK-->CG
CK-->CI
CK-->CJ
CK-->CL
CK-->CM
CK-->CN
CK-->CO
CK-->CP
CK-->CQ
CK-->CR
CK-->CS
CK-->CT
CK-->CU
CK-->CV
CK-->CZ
CK-->D0
CN-->CI
CS-->C8
```
