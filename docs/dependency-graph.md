...
subgraph DM["transaction-card"]
DN["transaction-card.component.ts"]
end
subgraph DO["transaction-edit-form"]
DP["transaction-edit-form.component.ts"]
end
subgraph DQ["transaction-filters"]
DR["transaction-filters.component.ts"]
end
subgraph DS["transaction-row"]
DT["transaction-row.component.ts"]
end
subgraph DU["transactions-overview"]
DV["transactions-overview.component.ts"]
end
subgraph DW["transfer-review"]
DX["transfer-review.component.ts"]
end
end
DY["index.ts"]
DZ["transaction-filters.ts"]
E0["transaction-row-vm.ts"]
E1["transactions.routes.ts"]
E2["transfer-label.ts"]
end
subgraph E3["shared"]
subgraph E4["echarts"]
E5["bucketed-axis-option.ts"]
E6["chart-theme.ts"]
E7["echarts-jsdom.testing.ts"]
E8["echarts-setup.ts"]
E9["index.ts"]
EA["legend-option.ts"]
EB["tooltip-formatter.ts"]
end
subgraph EC["ui"]
subgraph ED["absolute-range-panel"]
EE["absolute-range-panel.component.ts"]
end
subgraph EF["alert"]
EG["alert.component.ts"]
end
subgraph EH["badge"]
EI["badge.component.ts"]
end
subgraph EJ["button"]
EK["button.component.ts"]
end
subgraph EL["collapse"]
EM["collapse.component.ts"]
end
subgraph EN["confirm-dialog"]
EO["confirm-dialog.component.ts"]
end
subgraph EP["cycle-picker"]
EQ["cycle-picker.component.ts"]
end
subgraph ER["date-range-input"]
ES["date-range-input.component.ts"]
end
subgraph ET["divider"]
EU["divider.component.ts"]
end
subgraph EV["dropdown"]
EW["dropdown.component.ts"]
end
subgraph EX["empty-state"]
EY["empty-state.component.ts"]
end
subgraph EZ["fieldset"]
F0["fieldset.component.ts"]
end
subgraph F1["flex"]
F2["flex.component.ts"]
end
subgraph F3["granularity-picker"]
F4["granularity-picker.component.ts"]
end
F5["index.ts"]
subgraph F6["input"]
F7["input.component.ts"]
end
subgraph F8["label"]
F9["label.component.ts"]
end
subgraph FA["loading-skeleton"]
FB["loading-skeleton.component.ts"]
end
subgraph FC["modal"]
FD["mm-modal.component.ts"]
end
subgraph FE["page-header"]
FF["page-header.component.ts"]
end
subgraph FG["paginator"]
FH["paginator.component.ts"]
end
subgraph FI["paper"]
FJ["paper.component.ts"]
end
subgraph FK["privacy-blur"]
FL["privacy-blur.component.ts"]
end
subgraph FM["privacy-toggle"]
FN["privacy-toggle.component.ts"]
end
subgraph FO["range-picker"]
FP["range-picker.component.ts"]
end
subgraph FQ["select"]
FR["select.component.ts"]
end
subgraph FS["stat-card"]
FT["stat-card.component.ts"]
end
subgraph FU["table"]
FV["table.component.ts"]
end
subgraph FW["tabs"]
FX["tabs.component.ts"]
end
subgraph FY["typography"]
FZ["typography.component.ts"]
end
end
subgraph G0["utils"]
G1["calendar-cycles.ts"]
G2["compact-viewport.ts"]
G3["confidence-color.ts"]
G4["confirm-state.ts"]
G5["currency-format.ts"]
G6["currency-symbol-presets.ts"]
G7["daisy-classes.ts"]
G8["date-buckets.ts"]
G9["date-format.pipe.ts"]
GA["date-format.ts"]
GB["debounced-text.ts"]
GC["download-json.ts"]
GD["fingerprint.ts"]
GE["format-settings.testing.ts"]
GF["format-settings.ts"]
GG["hidden-amount.ts"]
GH["iban.ts"]
GI["index.ts"]
GJ["link-control-to-setting.ts"]
GK["locale-presets.ts"]
GL["money-color.ts"]
GM["number-format.ts"]
GN["pagination.ts"]
GO["percentage.ts"]
GP["quick-ranges.ts"]
GQ["range-expression.ts"]
GR["search-params.ts"]
GS["selection-model.ts"]
GT["signed-amount.pipe.ts"]
GU["sortable.ts"]
GV["structural-filters.ts"]
GW["theme-hooks.ts"]
GX["unrendered-markup.testing.ts"]
subgraph GY["validators"]
GZ["iban.validator.ts"]
H0["percentage.validator.ts"]
end
H1["with-archivable.ts"]
H2["with-persisted-crud.ts"]
end
end
end
end
