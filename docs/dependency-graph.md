...
E0["transfer-label.ts"]
end
subgraph E1["shared"]
subgraph E2["echarts"]
E3["bucketed-axis-option.ts"]
E4["chart-theme.ts"]
E5["echarts-jsdom.testing.ts"]
E6["echarts-setup.ts"]
E7["index.ts"]
E8["legend-option.ts"]
E9["tooltip-formatter.ts"]
end
subgraph EA["ui"]
subgraph EB["absolute-range-panel"]
EC["absolute-range-panel.component.ts"]
end
subgraph ED["alert"]
EE["alert.component.ts"]
end
subgraph EF["badge"]
EG["badge.component.ts"]
end
subgraph EH["button"]
EI["button.component.ts"]
end
subgraph EJ["collapse"]
EK["collapse.component.ts"]
end
subgraph EL["confirm-dialog"]
EM["confirm-dialog.component.ts"]
end
subgraph EN["cycle-picker"]
EO["cycle-picker.component.ts"]
end
subgraph EP["date-range-input"]
EQ["date-range-input.component.ts"]
end
subgraph ER["divider"]
ES["divider.component.ts"]
end
subgraph ET["dropdown"]
EU["dropdown.component.ts"]
end
subgraph EV["empty-state"]
EW["empty-state.component.ts"]
end
subgraph EX["fieldset"]
EY["fieldset.component.ts"]
end
subgraph EZ["flex"]
F0["flex.component.ts"]
end
subgraph F1["granularity-picker"]
F2["granularity-picker.component.ts"]
end
F3["index.ts"]
subgraph F4["input"]
F5["input.component.ts"]
end
subgraph F6["label"]
F7["label.component.ts"]
end
subgraph F8["loading-skeleton"]
F9["loading-skeleton.component.ts"]
end
subgraph FA["modal"]
FB["mm-modal.component.ts"]
end
subgraph FC["page-header"]
FD["page-header.component.ts"]
end
subgraph FE["paginator"]
FF["paginator.component.ts"]
end
subgraph FG["paper"]
FH["paper.component.ts"]
end
subgraph FI["privacy-blur"]
FJ["privacy-blur.component.ts"]
end
subgraph FK["privacy-toggle"]
FL["privacy-toggle.component.ts"]
end
subgraph FM["range-picker"]
FN["range-picker.component.ts"]
end
subgraph FO["select"]
FP["select.component.ts"]
end
subgraph FQ["stat-card"]
FR["stat-card.component.ts"]
end
subgraph FS["table"]
FT["table.component.ts"]
end
subgraph FU["tabs"]
FV["tabs.component.ts"]
end
subgraph FW["typography"]
FX["typography.component.ts"]
end
end
subgraph FY["utils"]
FZ["calendar-cycles.ts"]
G0["confidence-color.ts"]
G1["confirm-state.ts"]
G2["currency-format.ts"]
G3["currency-symbol-presets.ts"]
G4["daisy-classes.ts"]
G5["date-buckets.ts"]
G6["date-format.pipe.ts"]
G7["date-format.ts"]
G8["debounced-text.ts"]
G9["download-json.ts"]
GA["fingerprint.ts"]
GB["format-settings.testing.ts"]
GC["format-settings.ts"]
GD["hidden-amount.ts"]
GE["iban.ts"]
GF["index.ts"]
GG["link-control-to-setting.ts"]
GH["locale-presets.ts"]
GI["money-color.ts"]
GJ["number-format.ts"]
GK["pagination.ts"]
GL["percentage.ts"]
GM["quick-ranges.ts"]
GN["range-expression.ts"]
GO["search-params.ts"]
GP["selection-model.ts"]
GQ["signed-amount.pipe.ts"]
GR["sortable.ts"]
GS["structural-filters.ts"]
GT["theme-hooks.ts"]
GU["unrendered-markup.testing.ts"]
subgraph GV["validators"]
GW["iban.validator.ts"]
GX["percentage.validator.ts"]
end
GY["with-archivable.ts"]
GZ["with-persisted-crud.ts"]
end
end
end
end
