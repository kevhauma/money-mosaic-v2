# Dependency graph reference

Full import graph of `src/app`, generated with `dependency-cruiser`, for Claude to consult
when tracing what imports what without re-grepping the tree by hand.

## Scope

Unlike the human-facing overview in [`reports/dependency-graph.html`](../reports/dependency-graph.html)
(which collapses each `feature-*` folder to a single node and stops one folder into
`core`/`shared`), this graph goes three folder levels deeper on both tiers — up to 4 folders
below `src/app` — which is enough to reach individual files in most feature/core/shared
subtrees.

<!-- GENERATED:STATS:START -->
- 375 leaf nodes, 1100 edges.
<!-- GENERATED:STATS:END -->

- `*.spec.ts` and `node_modules` are excluded.
- Node IDs are dependency-cruiser's internal short hashes (`4`, `2K`, `6N`, ...) — read them
  via the `subgraph`/node label text, not the ID itself.
- An edge collapsed at a folder boundary (rare here, since most subtrees fit within 4 levels)
  means _something_ inside that folder imports the target, not necessarily its `index.ts`.

## Regenerate

```bash
depcruise src/app \
  --config .dependency-cruiser.cjs \
  --collapse '^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+/[^/]+|^src/app/(?:core|shared)/[^/]+|^src/app/[^/]+/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+/[^/]+|^src/app/[^/]+/[^/]+|^src/app/[^/]+' \
  --output-type mermaid \
  --output-to reports/dependency-graph-deep.mmd
```

## Graph

```mermaid
flowchart LR

subgraph 0["src"]
subgraph 1["app"]
subgraph 2["core"]
subgraph 3["accounts"]
4["account-deletion.service.ts"]
5["index.ts"]
6["joint-owner-lookup.ts"]
end
subgraph 7["categorisation"]
8["category-applicability.ts"]
9["co-owner-contribution.service.ts"]
A["co-owner-contribution.ts"]
B["index.ts"]
C["rule-matching.ts"]
D["rules-engine.service.ts"]
end
subgraph E["data-access"]
F["accounts.repository.ts"]
G["app-db.ts"]
H["app-settings.repository.ts"]
I["categories.repository.ts"]
J["category-comparison-settings.repository.ts"]
K["category-model.repository.ts"]
L["dashboard-layout-settings.repository.ts"]
M["data-management.repository.ts"]
N["forecast-settings.repository.ts"]
O["goals.repository.ts"]
P["import-batches.repository.ts"]
Q["index.ts"]
R["mapping-profiles.repository.ts"]
S["rules.repository.ts"]
T["salary-metadata.repository.ts"]
U["transactions.repository.ts"]
V["transfer-settings.repository.ts"]
W["transfers.repository.ts"]
end
subgraph X["import"]
Y["account-detection.ts"]
Z["csv-import.service.ts"]
10["csv-parse.ts"]
11["csv-parse.worker.ts"]
12["csv-row-mapper.ts"]
13["csv-worker.types.ts"]
14["delimiter-guess.ts"]
15["import.service.ts"]
16["index.ts"]
end
subgraph 17["layout"]
subgraph 18["app-shell"]
19["app-shell.component.ts"]
end
1A["index.ts"]
end
subgraph 1B["links"]
1C["external-links.ts"]
1D["index.ts"]
end
subgraph 1E["ml"]
1F["category-model.worker.ts"]
1G["category-model.worker.types.ts"]
1H["feature-hashing.ts"]
1I["index.ts"]
1J["model-config.ts"]
1K["rule-proposal-mining.ts"]
1L["training-window.ts"]
end
subgraph 1M["onboarding"]
1N["home-redirect.guard.ts"]
1O["index.ts"]
1P["mark-visited.guard.ts"]
1Q["visited.service.ts"]
end
subgraph 1R["state"]
1S["accounts.store.ts"]
1T["app-settings.store.ts"]
1U["categories.store.ts"]
1V["chart-options-control.ts"]
1W["chart-options.store.ts"]
1X["forecast-settings.store.ts"]
1Y["goals.store.ts"]
1Z["index.ts"]
20["page-range-control.ts"]
21["range-state.store.ts"]
22["transactions.store.ts"]
23["transfer-settings.store.ts"]
24["transfers.store.ts"]
end
subgraph 25["stats"]
26["account-balance-history.ts"]
27["account-balance-trend.ts"]
28["annual-lump-sum-smoothing.ts"]
29["category-breakdown.ts"]
2A["category-composition-trend.ts"]
2B["category-cycle-heatmap.ts"]
2C["category-expense-transactions.ts"]
2D["category-kind-contribution.ts"]
2E["category-period-comparison.ts"]
2F["chart-zoom-window.ts"]
2G["classify-for-stats.ts"]
2H["classify-joint-leg.ts"]
2I["day-transactions.ts"]
2J["embedded-bonus-smoothing.ts"]
2K["full-history-range.ts"]
2L["granularity-for-span.ts"]
2M["gross-net-growth.ts"]
2N["gross-net-ratio.ts"]
2O["income-category-series.ts"]
2P["income-events.ts"]
2Q["income-gap-detection.ts"]
2R["income-growth.ts"]
2S["income-step-change-detection.ts"]
2T["index.ts"]
2U["joint-account-stake.ts"]
2V["joint-contributor-breakdown.ts"]
2W["money-flow-graph.ts"]
2X["multi-year-income-comparison.ts"]
2Y["net-margin.ts"]
2Z["period-stats.ts"]
30["period-window.ts"]
31["periodized-rate.ts"]
32["recurring-payments.ts"]
33["recurring-projection.ts"]
34["saving-velocity.ts"]
35["spending-mosaic.ts"]
36["top-transactions.ts"]
37["wage-change-detection.ts"]
38["weekday-weekend-split.ts"]
39["year-over-year.ts"]
3A["yearly-income-summary.ts"]
end
subgraph 3B["storage"]
3C["index.ts"]
3D["storage-status.service.ts"]
end
subgraph 3E["theme"]
3F["accent-colors.ts"]
3G["index.ts"]
3H["theme-styles.ts"]
3I["theme.service.ts"]
end
subgraph 3J["transactions"]
3K["attribution-override.ts"]
3L["index.ts"]
3M["nullify-transaction.ts"]
3N["transaction-deletion.service.ts"]
end
subgraph 3O["transfers"]
3P["index.ts"]
3Q["transfer-cleanup.service.ts"]
3R["transfer-linking.service.ts"]
3S["transfer-matching.service.ts"]
3T["transfer-matching.ts"]
end
end
subgraph 3U["feature-accounts"]
3V["account-card-vm.ts"]
3W["account-icons.ts"]
3X["account-list-order.ts"]
3Y["account-types.ts"]
3Z["accounts.routes.ts"]
40["balance-day-tooltip.ts"]
41["balance-trend-signals.ts"]
subgraph 42["components"]
subgraph 43["account-balance-block"]
44["account-balance-block.component.ts"]
end
subgraph 45["account-balance-chart"]
46["account-balance-chart.component.ts"]
end
subgraph 47["account-balance-history-chart"]
48["account-balance-history-chart.component.ts"]
end
subgraph 49["account-card"]
4A["account-card.component.ts"]
end
subgraph 4B["account-form"]
4C["account-form.component.ts"]
end
subgraph 4D["accounts-detail"]
4E["accounts-detail.component.ts"]
end
subgraph 4F["accounts-overview"]
4G["accounts-overview.component.ts"]
end
4H["index.ts"]
end
4I["index.ts"]
end
subgraph 4J["feature-categories"]
4K["categories.routes.ts"]
4L["category-icons.ts"]
4M["category-model.service.ts"]
4N["category-model.store.ts"]
4O["category-row-vm.ts"]
subgraph 4P["components"]
subgraph 4Q["categories-overview"]
4R["categories-overview.component.ts"]
end
subgraph 4S["category-form"]
4T["category-form.component.ts"]
end
4U["index.ts"]
subgraph 4V["rule-condition-row"]
4W["rule-condition-row.component.ts"]
end
subgraph 4X["rule-filters"]
4Y["rule-filters.component.ts"]
end
subgraph 4Z["rule-form"]
50["rule-form.component.ts"]
end
subgraph 51["rule-share-bar"]
52["rule-share-bar.component.ts"]
end
subgraph 53["rules-overview"]
54["rules-overview.component.ts"]
end
end
55["index.ts"]
56["rule-condition-editor.ts"]
57["rule-filters.ts"]
58["rule-labels.ts"]
59["rule-share.ts"]
5A["rule-summary.ts"]
5B["rules.store.ts"]
end
subgraph 5C["feature-changelog"]
5D["changelog.routes.ts"]
subgraph 5E["components"]
subgraph 5F["changelog-entry-row"]
5G["changelog-entry-row.component.ts"]
end
subgraph 5H["changelog-page"]
5I["changelog-page.component.ts"]
end
5J["index.ts"]
end
subgraph 5K["data"]
5L["changelog-entries.ts"]
5M["roadmap-entries.ts"]
end
5N["group-changelog-entries.ts"]
5O["group-roadmap-entries.ts"]
5P["index.ts"]
end
subgraph 5Q["feature-dashboard"]
5R["category-comparison-settings.store.ts"]
5S["category-comparison-vm.ts"]
subgraph 5T["components"]
subgraph 5U["account-balance-strip"]
5V["account-balance-strip.component.ts"]
end
subgraph 5W["action-queue-panel"]
5X["action-queue-panel.component.ts"]
end
subgraph 5Y["category-breakdown-panel"]
5Z["category-breakdown-panel.component.ts"]
end
subgraph 60["category-comparison-panel"]
61["category-comparison-panel.component.ts"]
end
subgraph 62["category-exclusion-dropdown"]
63["category-exclusion-dropdown.component.ts"]
end
subgraph 64["comparison-category-card"]
65["comparison-category-card.component.ts"]
end
subgraph 66["dashboard-customize-panel"]
67["dashboard-customize-panel.component.ts"]
end
subgraph 68["dashboard-overview"]
69["dashboard-overview.component.ts"]
end
6A["index.ts"]
subgraph 6B["spending-heatmap-panel"]
6C["spending-heatmap-panel.component.ts"]
end
subgraph 6D["top-transactions-panel"]
6E["top-transactions-panel.component.ts"]
end
subgraph 6F["trend-chart-panel"]
6G["trend-chart-panel.component.ts"]
end
subgraph 6H["weekday-weekend-split-panel"]
6I["weekday-weekend-split-panel.component.ts"]
end
end
6J["dashboard-layout-settings.store.ts"]
6K["dashboard-row-order.ts"]
6L["dashboard.routes.ts"]
6M["index.ts"]
6N["stats.store.ts"]
end
subgraph 6O["feature-data-management"]
subgraph 6P["components"]
subgraph 6Q["data-management-overview"]
6R["data-management-overview.component.ts"]
end
6S["index.ts"]
end
6T["index.ts"]
end
subgraph 6U["feature-explore"]
subgraph 6V["components"]
subgraph 6W["explore-overview"]
6X["explore-overview.component.ts"]
end
6Y["index.ts"]
subgraph 6Z["money-flow-panel"]
70["money-flow-panel.component.ts"]
end
subgraph 71["spending-mosaic-panel"]
72["spending-mosaic-panel.component.ts"]
end
end
73["explore.routes.ts"]
74["index.ts"]
end
subgraph 75["feature-future"]
subgraph 76["components"]
subgraph 77["future-overview"]
78["future-overview.component.ts"]
end
subgraph 79["goal-form"]
7A["goal-form.component.ts"]
end
subgraph 7B["goal-row"]
7C["goal-row.component.ts"]
end
subgraph 7D["goals-panel"]
7E["goals-panel.component.ts"]
end
7F["index.ts"]
end
7G["future.routes.ts"]
7H["goal-row-vm.ts"]
7I["index.ts"]
end
subgraph 7J["feature-help"]
subgraph 7K["components"]
subgraph 7L["faq-page"]
7M["faq-page.component.ts"]
end
subgraph 7N["guide-detail"]
7O["guide-detail.component.ts"]
end
subgraph 7P["guide-steps"]
7Q["guide-steps.component.ts"]
end
subgraph 7R["guides-index"]
7S["guides-index.component.ts"]
end
7T["index.ts"]
end
subgraph 7U["data"]
7V["faq.ts"]
7W["guides.ts"]
end
7X["help.routes.ts"]
7Y["index.ts"]
end
subgraph 7Z["feature-home"]
subgraph 80["components"]
subgraph 81["home-landing"]
82["home-landing.component.ts"]
end
83["index.ts"]
end
84["home.routes.ts"]
85["index.ts"]
end
subgraph 86["feature-import"]
87["column-mapping.ts"]
subgraph 88["components"]
subgraph 89["account-draft-editor"]
8A["account-draft-editor.component.ts"]
end
subgraph 8B["batch-wait-card"]
8C["batch-wait-card.component.ts"]
end
subgraph 8D["column-map-amount-field"]
8E["column-map-amount-field.component.ts"]
end
subgraph 8F["column-map-counterparty-field"]
8G["column-map-counterparty-field.component.ts"]
end
subgraph 8H["column-map-sample-caption"]
8I["column-map-sample-caption.component.ts"]
end
subgraph 8J["column-map-simple-field"]
8K["column-map-simple-field.component.ts"]
end
subgraph 8L["column-map-stepper"]
8M["column-map-stepper.component.ts"]
end
subgraph 8N["column-map-summary-step"]
8O["column-map-summary-step.component.ts"]
end
subgraph 8P["import-map-step"]
8Q["import-map-step.component.ts"]
end
subgraph 8R["import-preview-step"]
8S["import-preview-step.component.ts"]
end
subgraph 8T["import-select-step"]
8U["import-select-step.component.ts"]
end
subgraph 8V["import-summary-step"]
8W["import-summary-step.component.ts"]
end
subgraph 8X["import-wizard"]
8Y["import-wizard.component.ts"]
end
8Z["index.ts"]
subgraph 90["queued-file-row"]
91["queued-file-row.component.ts"]
end
end
92["import-batches.store.ts"]
93["import-queue.ts"]
94["import-wizard-session.ts"]
95["import.routes.ts"]
96["index.ts"]
97["mapper-steps.ts"]
98["mapping-profiles.store.ts"]
end
subgraph 99["feature-income"]
9A["career-start-date.ts"]
subgraph 9B["components"]
subgraph 9C["income-career-start"]
9D["income-career-start.component.ts"]
end
subgraph 9E["income-category-checklist"]
9F["income-category-checklist.component.ts"]
end
subgraph 9G["income-chart-cell"]
9H["income-chart-cell.component.ts"]
end
subgraph 9I["income-events-sidebar"]
9J["income-events-sidebar.component.ts"]
end
subgraph 9K["income-gross-color"]
9L["income-gross-color.component.ts"]
end
subgraph 9M["income-gross-net-section"]
9N["income-gross-net-section.component.ts"]
end
subgraph 9O["income-growth-panel"]
9P["income-growth-panel.component.ts"]
end
subgraph 9Q["income-intro"]
9R["income-intro.component.ts"]
end
subgraph 9S["income-main-category"]
9T["income-main-category.component.ts"]
end
subgraph 9U["income-overview"]
9V["income-overview.component.ts"]
end
subgraph 9W["income-settings-page"]
9X["income-settings-page.component.ts"]
end
subgraph 9Y["income-yearly-panel"]
9Z["income-yearly-panel.component.ts"]
end
A0["index.ts"]
subgraph A1["salary-details-page"]
A2["salary-details-page.component.ts"]
end
subgraph A3["salary-metadata-table"]
A4["salary-metadata-table.component.ts"]
end
subgraph A5["salary-month-modal"]
A6["salary-month-modal.component.ts"]
end
end
A7["gross-net-chart-options.ts"]
A8["income-category-vm.ts"]
A9["income-event-vm.ts"]
AA["income-granularity.ts"]
AB["income.routes.ts"]
AC["income.store.ts"]
AD["index.ts"]
AE["salary-metadata-edit.ts"]
AF["salary-metadata-rows.ts"]
end
subgraph AG["feature-learning"]
subgraph AH["components"]
AI["index.ts"]
subgraph AJ["learning-overview"]
AK["learning-overview.component.ts"]
end
subgraph AL["model-status-badge"]
AM["model-status-badge.component.ts"]
end
subgraph AN["model-status"]
AO["model-status.component.ts"]
end
subgraph AP["rule-proposals"]
AQ["rule-proposals.component.ts"]
end
subgraph AR["suggestions-table"]
AS["suggestions-table.component.ts"]
end
end
AT["index.ts"]
AU["learning.routes.ts"]
AV["model-status-display.ts"]
end
subgraph AW["feature-recurring"]
AX["bills-calendar-vm.ts"]
subgraph AY["components"]
subgraph AZ["bills-calendar"]
B0["bills-calendar.component.ts"]
end
subgraph B1["bills-day-list"]
B2["bills-day-list.component.ts"]
end
subgraph B3["bills-month-grid"]
B4["bills-month-grid.component.ts"]
end
B5["index.ts"]
subgraph B6["recurring-overview"]
B7["recurring-overview.component.ts"]
end
subgraph B8["recurring-payments-panel"]
B9["recurring-payments-panel.component.ts"]
end
end
BA["index.ts"]
BB["recurring-payments-row-vm.ts"]
BC["recurring-series.store.ts"]
BD["recurring.routes.ts"]
end
subgraph BE["feature-settings"]
subgraph BF["components"]
BG["index.ts"]
subgraph BH["settings-about-section"]
BI["settings-about-section.component.ts"]
end
subgraph BJ["settings-currency-locale-section"]
BK["settings-currency-locale-section.component.ts"]
end
subgraph BL["settings-data-section"]
BM["settings-data-section.component.ts"]
end
subgraph BN["settings-overview"]
BO["settings-overview.component.ts"]
end
subgraph BP["settings-privacy-section"]
BQ["settings-privacy-section.component.ts"]
end
subgraph BR["settings-theme-section"]
BS["settings-theme-section.component.ts"]
end
end
BT["index.ts"]
BU["settings.routes.ts"]
end
subgraph BV["feature-transactions"]
BW["category-picker.ts"]
subgraph BX["components"]
subgraph BY["attribution-override-fieldset"]
BZ["attribution-override-fieldset.component.ts"]
end
subgraph C0["category-select-cell"]
C1["category-select-cell.component.ts"]
end
C2["index.ts"]
subgraph C3["transaction-bulk-bar"]
C4["transaction-bulk-bar.component.ts"]
end
subgraph C5["transaction-edit-form"]
C6["transaction-edit-form.component.ts"]
end
subgraph C7["transaction-filters"]
C8["transaction-filters.component.ts"]
end
subgraph C9["transaction-row"]
CA["transaction-row.component.ts"]
end
subgraph CB["transactions-overview"]
CC["transactions-overview.component.ts"]
end
subgraph CD["transfer-review"]
CE["transfer-review.component.ts"]
end
end
CF["index.ts"]
CG["transaction-filters.ts"]
CH["transaction-row-vm.ts"]
CI["transactions.routes.ts"]
end
subgraph CJ["shared"]
subgraph CK["echarts"]
CL["bucketed-axis-option.ts"]
CM["chart-theme.ts"]
CN["echarts-jsdom.testing.ts"]
CO["echarts-setup.ts"]
CP["index.ts"]
CQ["legend-option.ts"]
CR["tooltip-formatter.ts"]
end
subgraph CS["ui"]
subgraph CT["alert"]
CU["alert.component.ts"]
end
subgraph CV["badge"]
CW["badge.component.ts"]
end
subgraph CX["button"]
CY["button.component.ts"]
end
subgraph CZ["collapse"]
D0["collapse.component.ts"]
end
subgraph D1["confirm-dialog"]
D2["confirm-dialog.component.ts"]
end
subgraph D3["cycle-picker"]
D4["cycle-picker.component.ts"]
end
subgraph D5["date-range-input"]
D6["date-range-input.component.ts"]
end
subgraph D7["divider"]
D8["divider.component.ts"]
end
subgraph D9["dropdown"]
DA["dropdown.component.ts"]
end
subgraph DB["empty-state"]
DC["empty-state.component.ts"]
end
subgraph DD["fieldset"]
DE["fieldset.component.ts"]
end
subgraph DF["flex"]
DG["flex.component.ts"]
end
subgraph DH["granularity-picker"]
DI["granularity-picker.component.ts"]
end
DJ["index.ts"]
subgraph DK["input"]
DL["input.component.ts"]
end
subgraph DM["label"]
DN["label.component.ts"]
end
subgraph DO["loading-skeleton"]
DP["loading-skeleton.component.ts"]
end
subgraph DQ["modal"]
DR["mm-modal.component.ts"]
end
subgraph DS["page-header"]
DT["page-header.component.ts"]
end
subgraph DU["paginator"]
DV["paginator.component.ts"]
end
subgraph DW["paper"]
DX["paper.component.ts"]
end
subgraph DY["privacy-blur"]
DZ["privacy-blur.component.ts"]
end
subgraph E0["privacy-toggle"]
E1["privacy-toggle.component.ts"]
end
subgraph E2["range-grouping-switcher"]
E3["range-grouping-switcher.component.ts"]
end
subgraph E4["select"]
E5["select.component.ts"]
end
subgraph E6["stat-card"]
E7["stat-card.component.ts"]
end
subgraph E8["table"]
E9["table.component.ts"]
end
subgraph EA["tabs"]
EB["tabs.component.ts"]
end
subgraph EC["typography"]
ED["typography.component.ts"]
end
end
subgraph EE["utils"]
EF["calendar-cycles.ts"]
EG["confidence-color.ts"]
EH["confirm-state.ts"]
EI["currency-format.ts"]
EJ["currency-symbol-presets.ts"]
EK["daisy-classes.ts"]
EL["date-buckets.ts"]
EM["date-format.pipe.ts"]
EN["date-format.ts"]
EO["debounced-text.ts"]
EP["download-json.ts"]
EQ["fingerprint.ts"]
ER["format-settings.testing.ts"]
ES["format-settings.ts"]
ET["hidden-amount.ts"]
EU["iban.ts"]
EV["index.ts"]
EW["link-control-to-setting.ts"]
EX["locale-presets.ts"]
EY["number-format.ts"]
EZ["pagination.ts"]
F0["percentage.ts"]
F1["search-params.ts"]
F2["selection-model.ts"]
F3["signed-amount.pipe.ts"]
F4["sortable.ts"]
F5["structural-filters.ts"]
F6["theme-hooks.ts"]
subgraph F7["validators"]
F8["iban.validator.ts"]
F9["percentage.validator.ts"]
end
FA["with-archivable.ts"]
FB["with-persisted-crud.ts"]
end
end
end
end
4-->Q
4-->3P
5-->4
5-->6
6-->Q
6-->EV
8-->Q
9-->A
9-->Q
A-->5
A-->Q
B-->8
B-->A
B-->9
B-->C
B-->D
C-->Q
D-->C
D-->Q
F-->G
G-->1J
G-->34
G-->3F
G-->EQ
G-->ES
H-->G
H-->3G
H-->EV
I-->G
J-->G
K-->G
L-->G
M-->G
N-->G
N-->2T
O-->G
P-->G
Q-->F
Q-->G
Q-->H
Q-->I
Q-->J
Q-->K
Q-->L
Q-->M
Q-->N
Q-->O
Q-->P
Q-->R
Q-->S
Q-->T
Q-->U
Q-->V
Q-->W
R-->G
S-->G
T-->G
U-->G
V-->G
W-->G
Y-->Q
Y-->EV
Z-->13
Z-->Q
10-->12
10-->13
11-->10
11-->13
12-->Q
13-->12
13-->Q
15-->12
15-->Q
15-->3P
15-->EV
16-->Y
16-->Z
16-->10
16-->12
16-->13
16-->14
16-->15
19-->1Z
19-->CY
19-->ED
1A-->19
1D-->1C
1F-->1G
1F-->1H
1F-->1J
1G-->1J
1H-->1J
1I-->1G
1I-->1H
1I-->1J
1I-->1K
1I-->1L
1K-->B
1K-->Q
1N-->1Q
1O-->1N
1O-->1P
1O-->1Q
1P-->1Q
1S-->1U
1S-->22
1S-->24
1S-->5
1S-->Q
1S-->2T
1S-->3P
1S-->EV
1T-->Q
1T-->3G
1T-->EV
1U-->22
1U-->Q
1U-->EV
1V-->1W
1V-->2T
1V-->CP
1V-->EV
1W-->2T
1W-->CP
1W-->EV
1X-->Q
1X-->2T
1Y-->Q
1Y-->EV
1Z-->1S
1Z-->1T
1Z-->1U
1Z-->1V
1Z-->1W
1Z-->1X
1Z-->1Y
1Z-->20
1Z-->21
1Z-->22
1Z-->23
1Z-->24
20-->1S
20-->21
20-->22
20-->2T
20-->EV
21-->EV
22-->Q
22-->3L
22-->3P
23-->Q
24-->22
24-->23
24-->Q
24-->3P
26-->Q
26-->EV
27-->26
27-->Q
27-->EV
28-->2A
28-->2O
28-->EV
29-->2G
29-->Q
2A-->29
2A-->Q
2A-->EV
2B-->29
2B-->2G
2B-->Q
2B-->EV
2C-->2G
2C-->Q
2D-->Q
2E-->29
2E-->30
2E-->Q
2F-->EV
2G-->2D
2G-->2H
2G-->Q
2G-->3P
2H-->5
2H-->Q
2I-->Q
2J-->2A
2J-->2O
2J-->Q
2J-->EV
2K-->Q
2L-->EV
2M-->2N
2N-->2O
2N-->Q
2O-->2A
2O-->Q
2O-->EV
2P-->2Q
2P-->2S
2P-->37
2P-->Q
2Q-->2A
2Q-->2O
2Q-->EV
2R-->2O
2R-->39
2R-->EV
2S-->2A
2S-->2J
2S-->2O
2S-->EV
2T-->26
2T-->27
2T-->28
2T-->29
2T-->2A
2T-->2B
2T-->2C
2T-->2D
2T-->2E
2T-->2F
2T-->2G
2T-->2H
2T-->2I
2T-->2J
2T-->2K
2T-->2L
2T-->2M
2T-->2N
2T-->2O
2T-->2P
2T-->2Q
2T-->2R
2T-->2S
2T-->2U
2T-->2V
2T-->2W
2T-->2X
2T-->2Y
2T-->2Z
2T-->30
2T-->31
2T-->32
2T-->33
2T-->34
2T-->35
2T-->36
2T-->37
2T-->38
2T-->39
2T-->3A
2U-->2H
2U-->Q
2V-->2H
2V-->5
2V-->Q
2W-->Q
2W-->EV
2X-->3A
2Z-->2G
2Z-->Q
30-->EV
31-->EV
32-->2G
32-->B
32-->Q
32-->EV
33-->32
33-->EV
34-->2Z
34-->Q
34-->EV
35-->29
35-->2C
35-->Q
36-->Q
36-->3P
37-->2N
38-->2G
38-->Q
38-->EV
39-->2Z
39-->Q
3A-->2A
3A-->Q
3A-->EV
3C-->3D
3F-->3H
3G-->3F
3G-->3H
3G-->3I
3I-->3H
3K-->Q
3L-->3K
3L-->3M
3L-->3N
3M-->Q
3N-->Q
3N-->3P
3P-->3Q
3P-->3R
3P-->3T
3P-->3S
3Q-->Q
3R-->3T
3R-->Q
3S-->3R
3S-->3T
3S-->Q
3T-->6
3T-->Q
3T-->EV
3V-->Q
3W-->Q
3X-->Q
3Y-->Q
3Z-->4E
3Z-->4G
3Z-->CP
40-->2T
40-->EV
41-->Q
41-->1Z
41-->2T
41-->EV
44-->DJ
44-->EV
46-->40
46-->41
46-->Q
46-->2T
46-->CP
46-->DJ
46-->EV
48-->40
48-->41
48-->Q
48-->1Z
48-->2T
48-->CP
48-->DJ
4A-->3V
4A-->3W
4A-->44
4A-->DJ
4C-->3W
4C-->3Y
4C-->Q
4C-->DJ
4C-->EV
4C-->F8
4C-->F9
4E-->44
4E-->46
4E-->4C
4E-->1Z
4E-->DJ
4E-->EV
4G-->3V
4G-->3W
4G-->3X
4G-->48
4G-->4A
4G-->4C
4G-->Q
4G-->1Z
4G-->DJ
4G-->EV
4H-->44
4H-->46
4H-->48
4H-->4A
4H-->4C
4H-->4E
4H-->4G
4I-->3W
4I-->3Y
4I-->3Z
4I-->4H
4K-->4R
4K-->54
4M-->1I
4N-->4M
4N-->5B
4N-->Q
4N-->1I
4N-->1Z
4O-->Q
4O-->DJ
4R-->4L
4R-->4O
4R-->4T
4R-->B
4R-->Q
4R-->1Z
4R-->DJ
4R-->EV
4T-->4L
4T-->Q
4T-->DJ
4U-->4R
4U-->4T
4U-->4Y
4U-->50
4U-->52
4U-->54
4W-->56
4W-->58
4W-->B
4W-->Q
4W-->1Z
4W-->DJ
4Y-->57
4Y-->1Z
4Y-->DJ
4Y-->EV
50-->56
50-->4W
50-->B
50-->Q
50-->1Z
50-->DJ
52-->59
52-->5B
52-->DJ
52-->EV
54-->57
54-->5A
54-->5B
54-->4Y
54-->50
54-->52
54-->Q
54-->1Z
54-->DJ
54-->EV
55-->4K
55-->4L
55-->4M
55-->4N
55-->4U
55-->57
55-->5A
55-->5B
56-->58
56-->Q
57-->5A
57-->Q
58-->Q
59-->Q
5A-->58
5A-->Q
5B-->59
5B-->B
5B-->Q
5B-->1Z
5B-->EV
5D-->5I
5G-->DJ
5I-->5L
5I-->5M
5I-->5N
5I-->5O
5I-->5G
5I-->DJ
5J-->5I
5N-->5L
5O-->5M
5P-->5D
5P-->5J
5R-->Q
5V-->1Z
5V-->DJ
5V-->EV
5X-->1Z
5X-->3P
5X-->DJ
5X-->EV
5Z-->6N
5Z-->1Z
5Z-->2T
5Z-->CP
5Z-->DJ
5Z-->EV
61-->5R
61-->5S
61-->6N
61-->63
61-->65
61-->1Z
61-->DJ
61-->EV
63-->1Z
63-->DJ
65-->5S
65-->DJ
67-->6J
67-->6K
67-->Q
67-->DJ
69-->6J
69-->6K
69-->6N
69-->5V
69-->5X
69-->5Z
69-->61
69-->67
69-->6C
69-->6E
69-->6G
69-->6I
69-->1Z
69-->2T
69-->DJ
69-->EV
6A-->5V
6A-->5X
6A-->5Z
6A-->61
6A-->63
6A-->67
6A-->69
6A-->6C
6A-->6E
6A-->6G
6A-->6I
6C-->63
6C-->1Z
6C-->2T
6C-->3P
6C-->CP
6C-->DJ
6C-->EV
6E-->6N
6E-->1Z
6E-->DJ
6E-->EV
6G-->1Z
6G-->2T
6G-->3P
6G-->CP
6G-->DJ
6G-->EV
6I-->6N
6I-->1Z
6I-->DJ
6I-->EV
6J-->6K
6J-->Q
6K-->Q
6L-->69
6L-->CP
6M-->6A
6M-->6L
6N-->5R
6N-->1Z
6N-->2T
6N-->3P
6R-->Q
6R-->3C
6R-->DJ
6R-->EV
6S-->6R
6T-->6S
6X-->70
6X-->72
6X-->1Z
6X-->DJ
6Y-->6X
6Y-->70
6Y-->72
70-->1Z
70-->2T
70-->CP
70-->DJ
70-->EV
72-->Q
72-->1Z
72-->2T
72-->3P
72-->CP
72-->DJ
72-->EV
73-->6X
73-->CP
74-->6Y
74-->73
78-->7E
78-->DJ
7A-->Q
7A-->DJ
7C-->7H
7C-->DJ
7E-->7H
7E-->7A
7E-->7C
7E-->Q
7E-->1Z
7E-->DJ
7E-->EV
7F-->78
7F-->7A
7F-->7C
7F-->7E
7G-->78
7G-->CP
7H-->Q
7I-->7F
7I-->7G
7M-->7V
7M-->DJ
7O-->7W
7O-->7Q
7O-->DJ
7Q-->7W
7Q-->DJ
7S-->7W
7S-->DJ
7T-->7M
7T-->7O
7T-->7Q
7T-->7S
7X-->7M
7X-->7O
7X-->7S
7Y-->7T
7Y-->7W
7Y-->7X
82-->1D
82-->DJ
83-->82
84-->82
85-->83
85-->84
87-->Q
8A-->93
8A-->4I
8A-->DJ
8C-->DJ
8E-->8I
8E-->DJ
8G-->8I
8G-->DJ
8I-->DJ
8K-->87
8K-->8I
8K-->DJ
8M-->97
8M-->DJ
8O-->87
8O-->DJ
8Q-->87
8Q-->97
8Q-->98
8Q-->8E
8Q-->8G
8Q-->8K
8Q-->8M
8Q-->8O
8Q-->8S
8Q-->Q
8Q-->16
8Q-->DJ
8S-->16
8S-->DJ
8S-->EV
8U-->93
8U-->98
8U-->91
8U-->Q
8U-->16
8U-->DJ
8W-->Q
8W-->DJ
8Y-->94
8Y-->8C
8Y-->8Q
8Y-->8U
8Y-->8W
8Y-->1Z
8Y-->DJ
8Z-->8Q
8Z-->8S
8Z-->8U
8Z-->8W
8Z-->8Y
91-->93
91-->8A
91-->Q
91-->DJ
92-->B
92-->Q
92-->16
92-->1Z
93-->Q
94-->87
94-->92
94-->93
94-->98
94-->Q
94-->16
94-->1Z
94-->4I
95-->8Y
96-->87
96-->8Z
96-->92
96-->93
96-->95
96-->97
96-->98
97-->87
98-->Q
9A-->2T
9D-->9A
9D-->AC
9D-->DJ
9D-->EV
9F-->A8
9F-->DJ
9H-->DJ
9J-->A9
9J-->AA
9J-->AC
9J-->1Z
9J-->2T
9J-->DJ
9L-->AC
9L-->3G
9L-->CP
9L-->DJ
9N-->A7
9N-->AC
9N-->9H
9N-->1Z
9N-->2T
9N-->CP
9N-->DJ
9N-->EV
9P-->AA
9P-->AC
9P-->1Z
9P-->2T
9P-->DJ
9P-->EV
9R-->1Z
9R-->7Y
9R-->DJ
9T-->A8
9T-->AC
9T-->DJ
9V-->AC
9V-->AF
9V-->9J
9V-->9N
9V-->9P
9V-->9R
9V-->9Z
9V-->A6
9V-->1Z
9V-->2T
9V-->7Y
9V-->CP
9V-->DJ
9V-->EV
9X-->A8
9X-->AC
9X-->9D
9X-->9F
9X-->9L
9X-->9T
9X-->DJ
9Z-->AC
9Z-->1Z
9Z-->2T
9Z-->3P
9Z-->CP
9Z-->DJ
9Z-->EV
A0-->9D
A0-->9F
A0-->9H
A0-->9J
A0-->9N
A0-->9P
A0-->9R
A0-->9T
A0-->9V
A0-->9X
A0-->9Z
A0-->A2
A0-->A4
A0-->A6
A2-->A4
A2-->DJ
A4-->AA
A4-->AC
A4-->AE
A4-->AF
A4-->DJ
A4-->EV
A6-->AC
A6-->AE
A6-->AF
A6-->A4
A6-->Q
A6-->DJ
A7-->2T
A7-->CP
A7-->EV
A8-->Q
A9-->AA
A9-->Q
A9-->2T
A9-->DJ
A9-->EV
AA-->EV
AB-->9V
AB-->9X
AB-->A2
AB-->CP
AC-->9A
AC-->AA
AC-->Q
AC-->1Z
AC-->2T
AC-->3G
AC-->3P
AD-->A0
AD-->A8
AD-->AA
AD-->AB
AD-->AC
AD-->AE
AD-->AF
AE-->Q
AF-->Q
AI-->AK
AI-->AO
AI-->AQ
AI-->AS
AK-->AM
AK-->AO
AK-->AQ
AK-->AS
AK-->DJ
AM-->AV
AM-->55
AM-->DJ
AO-->AV
AO-->1I
AO-->1Z
AO-->55
AO-->DJ
AQ-->Q
AQ-->1I
AQ-->1Z
AQ-->55
AQ-->DJ
AQ-->EV
AS-->Q
AS-->1Z
AS-->55
AS-->DJ
AS-->EV
AT-->AI
AT-->AU
AU-->AK
AV-->55
AV-->DJ
AX-->DJ
B0-->AX
B0-->BC
B0-->B2
B0-->B4
B0-->1Z
B0-->2T
B0-->DJ
B0-->EV
B2-->AX
B2-->DJ
B4-->AX
B4-->DJ
B5-->B0
B5-->B2
B5-->B4
B5-->B7
B5-->B9
B7-->B0
B7-->B9
B7-->1Z
B7-->DJ
B9-->BB
B9-->BC
B9-->1Z
B9-->2T
B9-->CP
B9-->DJ
B9-->EV
BA-->B5
BA-->BD
BC-->1Z
BC-->2T
BC-->3P
BD-->B7
BG-->BO
BI-->1D
BI-->DJ
BK-->1Z
BK-->DJ
BK-->EV
BM-->6T
BM-->DJ
BO-->BI
BO-->BK
BO-->BM
BO-->BQ
BO-->BS
BO-->DJ
BQ-->1Z
BQ-->DJ
BQ-->EV
BS-->1Z
BS-->3G
BS-->DJ
BT-->BG
BT-->BU
BU-->BO
BW-->Q
BZ-->Q
BZ-->1Z
BZ-->3L
BZ-->DJ
BZ-->EV
C1-->BW
C2-->C4
C2-->C6
C2-->C8
C2-->CC
C2-->CE
C4-->BW
C4-->B
C4-->1Z
C4-->DJ
C6-->BW
C6-->BZ
C6-->B
C6-->Q
C6-->1Z
C6-->3L
C6-->55
C6-->DJ
C8-->BW
C8-->CG
C8-->B
C8-->1Z
C8-->DJ
C8-->EV
CA-->BW
CA-->CH
CA-->C1
CA-->DJ
CA-->EV
CC-->BW
CC-->CG
CC-->CH
CC-->C4
CC-->C6
CC-->C8
CC-->CA
CC-->CE
CC-->B
CC-->Q
CC-->1Z
CC-->3P
CC-->55
CC-->DJ
CC-->EV
CE-->1Z
CE-->3P
CE-->DJ
CE-->EV
CF-->CI
CG-->Q
CG-->3P
CH-->Q
CI-->CC
CP-->CL
CP-->CM
CP-->CO
CP-->CQ
CP-->CR
CR-->EV
CU-->EV
CW-->EV
CY-->EV
D0-->EV
D2-->CY
D2-->DN
D2-->DR
D2-->ED
D4-->EV
D6-->DA
D6-->EV
D8-->EV
DA-->EV
DC-->DG
DC-->ED
DE-->EV
DG-->EV
DJ-->CU
DJ-->CW
DJ-->CY
DJ-->D0
DJ-->D2
DJ-->D4
DJ-->D6
DJ-->D8
DJ-->DA
DJ-->DC
DJ-->DE
DJ-->DG
DJ-->DI
DJ-->DL
DJ-->DN
DJ-->DP
DJ-->DR
DJ-->DT
DJ-->DV
DJ-->DX
DJ-->DZ
DJ-->E1
DJ-->E3
DJ-->E5
DJ-->E7
DJ-->E9
DJ-->EB
DJ-->ED
DL-->EV
DN-->EV
DP-->DG
DR-->EV
DT-->DG
DT-->ED
DV-->CY
DV-->DG
DV-->ED
DV-->EV
DX-->EV
DZ-->EV
E1-->CY
E1-->1Z
E3-->CY
E3-->D6
E3-->DG
E5-->EV
E7-->DZ
E7-->ED
E7-->EV
E9-->EV
EB-->EV
ED-->EV
EF-->EN
EI-->ES
EL-->ES
EM-->EN
EN-->ES
ER-->ES
EV-->EF
EV-->EG
EV-->EH
EV-->EI
EV-->EJ
EV-->EK
EV-->EL
EV-->EN
EV-->EM
EV-->EO
EV-->EP
EV-->EQ
EV-->ES
EV-->ET
EV-->EU
EV-->EW
EV-->EX
EV-->EY
EV-->EZ
EV-->F0
EV-->F1
EV-->F2
EV-->F3
EV-->F4
EV-->F5
EV-->F6
EV-->FA
EV-->FB
EY-->ES
F3-->EI
```
