...
subgraph A1["settings-privacy-section"]
A2["settings-privacy-section.component.ts"]
end
subgraph A3["settings-theme-section"]
A4["settings-theme-section.component.ts"]
end
end
A5["index.ts"]
A6["settings.routes.ts"]
end
subgraph A7["feature-transactions"]
subgraph A8["components"]
subgraph A9["attribution-override-fieldset"]
AA["attribution-override-fieldset.component.ts"]
end
subgraph AB["category-select-cell"]
AC["category-select-cell.component.ts"]
end
AD["index.ts"]
subgraph AE["transaction-bulk-bar"]
AF["transaction-bulk-bar.component.ts"]
end
subgraph AG["transaction-edit-form"]
AH["transaction-edit-form.component.ts"]
end
subgraph AI["transaction-filters"]
AJ["transaction-filters.component.ts"]
end
subgraph AK["transaction-row"]
AL["transaction-row.component.ts"]
end
subgraph AM["transactions-overview"]
AN["transactions-overview.component.ts"]
end
subgraph AO["transfer-review"]
AP["transfer-review.component.ts"]
end
end
AQ["index.ts"]
AR["transaction-filters.ts"]
AS["transaction-row-vm.ts"]
AT["transactions.routes.ts"]
end
subgraph AU["shared"]
subgraph AV["echarts"]
AW["bucketed-axis-option.ts"]
AX["chart-theme.ts"]
AY["echarts-jsdom.testing.ts"]
AZ["echarts-setup.ts"]
B0["index.ts"]
B1["legend-option.ts"]
B2["tooltip-formatter.ts"]
end
subgraph B3["ui"]
subgraph B4["alert"]
B5["alert.component.ts"]
end
subgraph B6["badge"]
B7["badge.component.ts"]
end
subgraph B8["button"]
B9["button.component.ts"]
end
subgraph BA["collapse"]
BB["collapse.component.ts"]
end
subgraph BC["confirm-dialog"]
BD["confirm-dialog.component.ts"]
end
subgraph BE["date-range-input"]
BF["date-range-input.component.ts"]
end
subgraph BG["divider"]
BH["divider.component.ts"]
end
subgraph BI["dropdown"]
BJ["dropdown.component.ts"]
end
subgraph BK["empty-state"]
BL["empty-state.component.ts"]
end
subgraph BM["fieldset"]
BN["fieldset.component.ts"]
end
subgraph BO["flex"]
BP["flex.component.ts"]
end
subgraph BQ["granularity-picker"]
BR["granularity-picker.component.ts"]
end
BS["index.ts"]
subgraph BT["input"]
BU["input.component.ts"]
end
subgraph BV["label"]
BW["label.component.ts"]
end
subgraph BX["loading-skeleton"]
BY["loading-skeleton.component.ts"]
end
subgraph BZ["modal"]
C0["mm-modal.component.ts"]
end
subgraph C1["page-header"]
C2["page-header.component.ts"]
end
subgraph C3["paginator"]
C4["paginator.component.ts"]
end
subgraph C5["paper"]
C6["paper.component.ts"]
end
subgraph C7["privacy-blur"]
C8["privacy-blur.component.ts"]
end
subgraph C9["range-grouping-switcher"]
CA["range-grouping-switcher.component.ts"]
end
subgraph CB["select"]
CC["select.component.ts"]
end
subgraph CD["stat-card"]
CE["stat-card.component.ts"]
end
subgraph CF["table"]
CG["table.component.ts"]
end
subgraph CH["tabs"]
CI["tabs.component.ts"]
end
subgraph CJ["typography"]
CK["typography.component.ts"]
end
end
subgraph CL["utils"]
CM["confidence-color.ts"]
CN["confirm-state.ts"]
CO["currency-format.ts"]
CP["currency-symbol-presets.ts"]
CQ["daisy-classes.ts"]
CR["date-buckets.ts"]
CS["date-format.pipe.ts"]
CT["date-format.ts"]
CU["debounced-text.ts"]
CV["download-json.ts"]
CW["fingerprint.ts"]
CX["format-settings.testing.ts"]
CY["format-settings.ts"]
CZ["iban.ts"]
D0["index.ts"]
D1["link-control-to-setting.ts"]
D2["locale-presets.ts"]
D3["number-format.ts"]
D4["pagination.ts"]
D5["percentage.ts"]
D6["search-params.ts"]
D7["selection-model.ts"]
D8["signed-amount.pipe.ts"]
D9["sortable.ts"]
DA["structural-filters.ts"]
DB["theme-hooks.ts"]
subgraph DC["validators"]
DD["iban.validator.ts"]
DE["percentage.validator.ts"]
end
DF["with-archivable.ts"]
DG["with-persisted-crud.ts"]
end
end
end
end
