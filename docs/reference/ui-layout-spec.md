...
- **Topbar** — ~~(top of `drawer-content`) hamburger (mobile/tablet only), current page title, a global **range & grouping switcher** (FR-STAT-7), and a page-specific primary action slot~~. **Revised by TICKET-UI-22/UI-23:** the shell's own bar is now mobile-only chrome (hamburger + brand, hidden from `lg:`), and the top bar every page shows is that page's `mm-page-header` — see §3. The range switcher is no longer global; each range-owning page renders its own, and the pages no longer share one range.
...
- **Page header** (revised by TICKET-UI-22, sectioned by TICKET-UI-24) — every page opens with exactly one `mm-page-header`, rendered as the app's top bar: daisyUI's `navbar`, full-bleed across the content area, carrying the page title on the left, that page's own page-level controls in its two action slots, and nothing else. It is the same bar the shell topbar used to be — TICKET-UI-23 handed it to the pages so each could own its date range. **There is no subtitle** — a page that needs an explanatory sentence puts it in the body, where it belongs to the content rather than to the chrome. "Page-level" means anything that reconfigures the whole page (date range, view switch, create-new, show-archived, page settings, guide links, re-run); a control scoped to one panel (a chart's bucket picker, say) stays on that panel. **Which slot** a page-level control goes in: one that changes *what the page is showing* — date range, view switch, back-to-parent — goes in `[actions-start]`, beside the title it qualifies; one that *acts on what is shown* — create, re-run, show-archived, page settings, a link elsewhere — goes in `[actions-end]`, right-aligned. Both groups wrap rather than overflowing, so a four-control header degrades to two rows on a phone. **The bar is `sticky top-0`** (TICKET-UI-25) at `z-10` — below the drawer's `z-20`, far below daisyUI's `z-999` modal/dropdown overlays — so a long page's controls stay reachable; the shell's mobile bar stays in flow and scrolls away rather than becoming a second pinned strip.
