...
- This is a QoL/interaction ticket, not part of the v1.5 styling rework — filed under `docs/releases/v1.5_redesign` at the user's explicit request as a separate "QoL addendum" section, mirroring how a similar QoL batch was previously appended to `docs/releases/v1.1_joint_accounts` rather than mixed into that version's original ticket set. It does not depend on and is not blocked by any `TICKET-UI-*` ticket in this folder.
