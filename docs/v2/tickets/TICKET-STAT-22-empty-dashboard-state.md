...
- [ ] `dashboard-overview.component.ts` computes a "no transactions" state from `TransactionsStore.hydrated()`/`transactions()`, not from `StatsStore`'s period-filtered stats (so it reflects the *entire* dataset being empty, not just the currently selected date range).
- [ ] When there are zero transactions, the Dashboard renders `mm-empty-state` (title + message + icon + CTA), following the same composition already used on [accounts-overview.component.html](../../../src/app/feature-accounts/components/accounts-overview/accounts-overview.component.html), instead of the stat-card/chart-panel rows.
- [ ] The CTA button navigates to `/import` (uses `routerLink`, not a manual navigation call).
- [ ] The empty state does not show while `statsStore.dataReady()`/`transactionsStore.hydrated()` is still loading — the existing loading-skeleton path is unaffected; the empty-state check only applies once hydration has completed.
- [ ] Once transactions exist, the normal row-based Dashboard renders as it does today — no regression to `visibleRows()`/TICKET-STAT-14 customize mode for non-empty accounts.
- [ ] Unit tests cover: the empty-state branch rendering when `transactions()` is empty and `hydrated()` is true; the normal row branch rendering once transactions exist; the CTA's `routerLink` pointing at `/import`.
