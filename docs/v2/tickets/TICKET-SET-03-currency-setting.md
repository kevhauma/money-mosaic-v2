...
- `AppSettings` (from TICKET-SET-05) gains an additive `currency?: string` field (ISO 4217 code, e.g. `'EUR'`, `'USD'`, `'GBP'`), defaulting to `'EUR'` in `DEFAULT_APP_SETTINGS` so existing behavior is unchanged for anyone who never opens Settings — no Dexie version bump.
