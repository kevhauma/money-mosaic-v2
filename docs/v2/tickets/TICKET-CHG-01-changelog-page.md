...
- TICKET-PUB-05 (Roadmap) builds on this ticket: it mirrors this data-file/route pattern for planned-but-unshipped work, and adds a further step to `work-ticket/SKILL.md`'s changelog step (right next to the one this ticket adds) that removes a ticket's Roadmap entry once its Changelog entry is added. Build this ticket first.
