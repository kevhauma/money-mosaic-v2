...
- [ ] As a developer, I want the transfer-cleanup cascade (collect transferIds → remove transfers → clear surviving side) extracted into one `core/transfers` helper used by both `ImportService.undoImport` and `AccountDeletionService`, so the subtle "surviving side may also be doomed" invariant lives in exactly one place ([TICKET-SOLID-03](./tickets/TICKET-SOLID-03-extract-transfer-cleanup-cascade.md), CR2-4.2; also deletes the unused `DeleteAccountResult` alias, CR2-4.3)
