...
- **Type:** Refactor
- **Traceability:** CR4-11 Options A + C-lite, CR4-10 Option C ([solution doc](../../releases/v2_code_review/solutions/CR4-11-project-map-skill.md), [CR4-10 doc](../../releases/v2_code_review/solutions/CR4-10-store-placement-rule.md))
...
- [.claude/skills/project-map/SKILL.md](../../../.claude/skills/project-map/SKILL.md): missing 4 of 11 routed features; describes bootstrap-time hydration (reversed by PERF-07 — stores self-hydrate on first injection); omits `AppSettingsStore` from the state inventory; stale `/data` and `docs/releases/v2` claims; half the `shared/ui` inventory absent; misplaced file claims (`date-buckets.ts` etc.); missing `core/` modules (`theme`, `onboarding`, `links`, `storage`, `layout`).
