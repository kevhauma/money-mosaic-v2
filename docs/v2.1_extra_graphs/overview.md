...
- [ ] [TICKET-REC-09](./tickets/TICKET-REC-09-recurring-includes-joint-accounts.md) — Recurring detection sees joint-account payments, at their full amount (bug fix, revises FR-REC-1 — `classifyForStats` excludes co-owner/`notMine` legs outright and scales the rest to `ownershipShare`, so a joint household bill is either invisible or banded on a fraction of its real amount) — **needs REC-01** (shipped); independent of REC-08/10; adds an explicit joint-handling mode to the shared classifier rather than forking it, and knowingly breaks reconciliation with the Dashboard's weighted figures (stated in the panel's caption)
