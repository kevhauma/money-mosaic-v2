...
- [x] [TICKET-STAT-31](./tickets/TICKET-STAT-31-heatmap-cycles-fit-the-range.md) — Only offer heatmap cycles the selected range can actually fill (bug fix, revises FR-STAT-15 — the picker offers all four cycles whatever the range, so a one-week range can be switched to a Month view whose columns are empty for calendar reasons) — **needs STAT-30**; from feedback on the shipped panel — every criterion met except the live browser check, which is blocked on the Browser pane and stays open on the ticket
- [x] [TICKET-STAT-32](./tickets/TICKET-STAT-32-heatmap-exclude-categories.md) — Exclude categories from the spending heatmap, like the category period comparison panel already allows (extends FR-STAT-15) — **needs STAT-29**; independent of STAT-31, also from feedback on the shipped panel — every criterion met except the live browser check, which is blocked on the Browser pane and stays open on the ticket
- [x] [TICKET-STAT-34](./tickets/TICKET-STAT-34-heatmap-per-category-colour-scales.md) — Each heatmap row shaded in its own category colour, anchored on that row's average and readable in light and dark (revises FR-STAT-15's colour encoding) — **needs STAT-29**; independent of STAT-31/32, from feedback on the shipped panel. Listed before STAT-33 because it replaces STAT-29's single `visualMap` with the per-row colour resolution STAT-33 also needs
- [x] [TICKET-STAT-33](./tickets/TICKET-STAT-33-heatmap-all-row.md) — An "All" row at the top of the spending heatmap, on its own scale and separated from the categories (extends FR-STAT-15) — **needs STAT-29**; **cheaper after STAT-34**, though it can be built alone at the cost of a two-scale arrangement STAT-34 then removes — every criterion met except the live browser check, which was confirmed on the pre-revision build and is blocked on the Browser pane for the rest; it stays open on the ticket
