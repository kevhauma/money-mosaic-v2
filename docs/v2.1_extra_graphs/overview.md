...
- [ ] [TICKET-REC-10](./tickets/TICKET-REC-10-recurring-table-fits-without-scrolling.md) — The recurring table fits its panel, with long payment names truncated (bug fix, revises FR-REC-2 — nothing constrains the Payment column's width, so `truncate` has no bounded box to work in and the table overflows, clipping "Per month" behind a horizontal scrollbar) — independent of everything, can ship any time; last of the three because REC-08 makes it more visible rather than the other way round
