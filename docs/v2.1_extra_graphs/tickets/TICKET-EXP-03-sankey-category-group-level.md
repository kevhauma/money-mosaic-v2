...
- [x] Links still span exactly one level, and the graph stays acyclic, with grouping on. (spec `still spans exactly one level per link, with grouping on`, over a fixture with grouped + ungrouped categories and a savings movement)
...
- [x] Savings and left-over nodes are unchanged by grouping. (spec `leaves savings and left-over untouched by grouping` — identical link objects and both still at level 2)
