...
- [x] Verified live in the browser on a populated range, including a range with one dominant category. (dev server :4210, 2026-08-25, once the browser pane was displayed — with it hidden the page composited no frames, so the panel's `@defer (on viewport)` never fired and nothing could be observed at all. On *This month*, which is exactly the dominant-category case: **Housing €950,00 against Groceries €131,55, Utilities €64,20, Other €28,79 and Eating Out €27,50** — a 7× outlier. The four smaller rows still carry visibly saturated cells rather than being flattened to the pale end, because the ramp is anchored on the pooled *average* (~€28,60 here) and moves each cell away from it in its own category's colour. The `All` strip renders above the grid with its own margin and its own scale, and the caption reads as shipped.)
