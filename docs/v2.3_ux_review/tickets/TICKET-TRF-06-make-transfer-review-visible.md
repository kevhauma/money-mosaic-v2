...
- [x] Unit tests cover: a linked row renders the transfer presentation and not "Uncategorised"; an unlinked uncategorised row is unaffected; the trigger's count reflects pending pairs; the zero-pending case renders the resolved state. (Eight cases in the new `transfer-label.spec.ts` — including both legs of a pair, an unlinked row, and the two half-deleted-pair fallbacks — three in `transaction-row.component.spec.ts`, and three in `transfer-review.component.spec.ts`, one of which asserts the count is non-zero *while `reviewExpanded()` is still false*, which is the assertion the old gate would fail. 3416 tests green.)
