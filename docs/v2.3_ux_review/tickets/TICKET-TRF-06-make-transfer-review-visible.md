...
- [ ] Verified live in the browser with at least one linked pair and one pending candidate. — **half done, and honestly so.** The *linked-pair* half is verified (dev server on :4210, 2026-08-24): eight rows — four pairs — render `Transfer · Rainy Day Savings` with **no** category picker and no "Uncategorised", while the 33 unlinked uncategorised rows keep theirs untouched; the trigger reads `Review possible transfers — 4 pairs linked, none to review.`, with the status reaching the button's accessible name through the `sr-only` span. The *pending-candidate* half is **not** live-verified: the seed's four pairs are all linked (so excluded from the matcher's `unlinked` set) and widening the match window to 30 days with medium auto-link off still yields zero candidates, so producing one means writing crafted transactions into the user's real database — which is not something to do unasked. It is covered by `transfer-review.component.spec.ts`'s ambiguous-set case, which asserts a non-zero count *while the panel is collapsed*. Re-open this box when a fixture exists.
