...
- [ ] Verified live in the browser, including keyboard-only operation. — **deferred, not skipped**: the user chose a single browser pass over the whole v2.3 batch rather than one per ticket; tick this when that pass runs. It matters more here than elsewhere — jsdom implements neither `showModal()` nor top-layer inertness, so the specs can only assert the component's two open models, and the inertness itself is the browser's half of the fix.
...
- **For the browser pass: watch the focus ring through Delete → Cancel → Delete → confirm.**
  `confirmDelete()` writes both open states in one tick, and `MmModalComponent` captures
  `document.activeElement` as its restore target when it opens. Since `mm-modal` leaves its
  `<dialog>` in the DOM, the confirm can capture the edit dialog's own "Delete" button and, on close,
  focus an element inside a closed dialog. jsdom implements neither `showModal()` nor top-layer
  inertness, so no spec here can see it.
