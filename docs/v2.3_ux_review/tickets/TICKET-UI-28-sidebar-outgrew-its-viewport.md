...
- [x] At a 700px viewport, either every nav item is visible, or the sidebar shows a visible indication that it scrolls. (Measured at 1280×700 on the dev server, 2026-08-24: the scroll frame's content is 492px in a 433px box, and **"Auto-categoriser" is cut mid-item** at the frame's edge — 17 of its 38px visible, with "Import" below it. A row sliced in half under a heading is the affordance; the list can no longer appear to simply end. At 1280×1000 nothing scrolls at all and all 15 links are in view.)
...
- [x] Verified live in the browser at 700px and at a taller viewport. (Dev server on :4210, 2026-08-24. **At 700px** — the exact viewport the as-is section measured — `.drawer-side` now reports `scrollHeight` 700 against `height` 700, so the whole-sidebar scroll that had no affordance is gone (it was 758 vs 700); `nav#app-nav` is `height 700, overflow hidden, flex-wrap nowrap`; and Settings sits at bottom **684**, Changelog at **642**, FAQ at **601** — all fully on screen, where Settings used to sit at 741.9 and Changelog at 700.3. **At 1000px** nothing scrolls: not the drawer, not the nav, not the frame, and all 15 links are visible. Also checked at **450px**, well below anything the ticket asked for: Settings is still at bottom 434, on screen. Both group headings still resolve their `aria-labelledby` lists. No console or server errors.)
