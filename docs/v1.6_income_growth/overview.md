...
- [ ] [TICKET-INC-12](./tickets/TICKET-INC-12-career-start-date.md) — Career start date filters the Income page from where your working life started, not where the data does (adds FR-INC-12) — independent, but cheapest right after INC-06 so the yearly view stops rendering pre-career zero bars and INC-07 inherits the clamped span
