...
- [x] [TICKET-TRF-03](./tickets/TICKET-TRF-03-guard-partner-inflow-matching.md) — Guard transfer auto-matching against one-sided partner inflows (extends FR-TRF-3 / FR-TRF-5, needs ACC-03; protects the matching CAT-02 depends on) — every criterion met except the live browser check, which stays open on the ticket
- [x] [TICKET-TRF-05](./tickets/TICKET-TRF-05-mutual-iban-corroboration.md) — High-confidence matching links my own transfer leg, not a co-owner's same-amount contribution (bug fix, fixes FR-TRF-3 / FR-TRF-5 — `ibanConfirms` is satisfied by one side alone, so an outflow naming the joint account confirms against *any* same-amount credit on it, and closest-by-date then picks the contribution) — **follow-on to TRF-03**, reported from real data after it shipped; every criterion met except the live browser check, which the reporter is running against their own data and stays open on the ticket
...
- [x] [TICKET-TXN-05](./tickets/TICKET-TXN-05-inline-category-quickset.md) — Inline category quick-set on transaction rows (extends FR-TXN-2) — highest daily-friction reduction, reuses the existing bulk-assign write path
...
- [x] [TICKET-IMP-08](./tickets/TICKET-IMP-08-create-account-from-import.md) — Create a new account directly from the CSV import step (extends FR-IMP-1) — independent of the rest of this track; builds on the existing per-file IBAN auto-detect from TICKET-IMP-05, can ship any time
