...
fix the rate and solve for the date ([FUT-05](../../future/tickets/TICKET-FUT-05-goal-affordability-projection.md)),
or fix the date and solve for the rate ([FUT-09](../../future/tickets/TICKET-FUT-09-required-saving-rate-mode.md)).
...
[TICKET-REC-03](../../recurring/tickets/TICKET-REC-03-upcoming-bills-calendar.md)'s bill
...
history ([FUT-01](../../future/tickets/TICKET-FUT-01-saving-velocity-aggregate.md)). Take today's net worth,
...
([FUT-05](../../future/tickets/TICKET-FUT-05-goal-affordability-projection.md)). That single decision is what
...
([FUT-07](../../future/tickets/TICKET-FUT-07-projected-net-worth-chart.md)) as a sawtooth — rising at the saving
...
sets and you get the second mode ([FUT-09](../../future/tickets/TICKET-FUT-09-required-saving-rate-mode.md)): the
...
([FUT-06](../../future/tickets/TICKET-FUT-06-forecast-controls.md)) is *how much* has to stay untouched — a floor
...
([FUT-08](../../future/tickets/TICKET-FUT-08-account-scope.md)) is *which* money is in the conversation at all —
...
[FUT-02](../../future/tickets/TICKET-FUT-02-goals-persistence.md) adds **one additive `.version(14)` block**
...
`.version(14)`'s type from the start and is non-indexed, so [FUT-09](../../future/tickets/TICKET-FUT-09-required-saving-rate-mode.md)
...
([FUT-06](../../future/tickets/TICKET-FUT-06-forecast-controls.md)). Every one of those is an acceptance
...
- [x] [TICKET-FUT-01](../../future/tickets/TICKET-FUT-01-saving-velocity-aggregate.md) — Saving velocity: how much I actually saved per month, over a configurable lookback (adds FR-FUT-1) — first: the pure aggregate every projection consumes, no UI of its own; independent of FUT-02/03, can be built in parallel with them
- [x] [TICKET-FUT-02](../../future/tickets/TICKET-FUT-02-goals-persistence.md) — Goals persistence: `savingsGoals` + `forecastSettings`, their repositories and stores (adds FR-FUT-2) — the version's only Dexie change, `.version(14)`, additive; independent of FUT-01
- [x] [TICKET-FUT-03](../../future/tickets/TICKET-FUT-03-future-page-scaffold.md) — Future page: route, shell, nav item, no date range (adds FR-FUT-3) — the stage every UI ticket below renders into, and where the route-level ECharts provider lives; independent of FUT-01/02
- [x] [TICKET-FUT-04](../../future/tickets/TICKET-FUT-04-goals-list-crud-reorder.md) — Goals list: add, edit, delete, drag into priority order (extends FR-FUT-2) — **needs FUT-02 + FUT-03**; the order it sets is what FUT-05 funds top-down, so it ships before the projection that depends on it
- [x] [TICKET-FUT-05](../../future/tickets/TICKET-FUT-05-goal-affordability-projection.md) — When can I afford it: an ETA per goal, funded in the order I set (adds FR-FUT-4) — **needs FUT-01 + FUT-02 + FUT-04**; the headline ticket, and the one the whole version exists for
- [x] [TICKET-FUT-06](../../future/tickets/TICKET-FUT-06-forecast-controls.md) — Forecast controls: lookback window, what counts as saving, safety net, and the spread behind the number (extends FR-FUT-1) — **needs FUT-05**; until it ships the forecast runs on FUT-02's defaults with no way to change them, which is why it lands early rather than as polish
- [x] [TICKET-FUT-07](../../future/tickets/TICKET-FUT-07-projected-net-worth-chart.md) — Projected net worth: the sawtooth, stepping down as each goal gets bought (adds FR-FUT-5) — **needs FUT-05** + FUT-03's provider scope; independent of FUT-06, reads the same settings
- [x] [TICKET-FUT-09](../../future/tickets/TICKET-FUT-09-required-saving-rate-mode.md) — What do I need to save: hit a goal by a date I choose, and see the gap against what I actually save (adds FR-FUT-6, revises FR-FUT-5) — **needs FUT-05 + FUT-07**; the second mode and its page-level toggle, reusing FUT-05's cumulative targets and making FUT-07's chart mode-aware; no Dexie change of its own
- [x] [TICKET-FUT-08](../../future/tickets/TICKET-FUT-08-account-scope.md) — Only count these accounts: scope the starting balance *and* the measured rate to the accounts I pick (revises FR-FUT-1/FR-FUT-4) — **needs FUT-05 + FUT-06**; last on purpose, and the only ticket here that refactors shipped code (`AccountsStore.netWorth` into a per-account contribution map, so a scoped total can't drift from the Dashboard's card)
...
- **Solving for the date instead of the rate** — "you can't find another €120/month, but push the
  ...
  [FUT-09](../../future/tickets/TICKET-FUT-09-required-saving-rate-mode.md) answers "what would it take" with a
...
- **Per-goal account scopes** ("fund the holiday from savings, the laptop from checking").
  [FUT-08](../../future/tickets/TICKET-FUT-08-account-scope.md) gives the *whole forecast* one scope, which is
...
the visually-hidden figure table [TICKET-STAT-20](../../dashboard/tickets/TICKET-STAT-20-trend-chart-accessible-numbers.md)
