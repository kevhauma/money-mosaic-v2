...
- [x] [TICKET-NG-01](../../engineering/tickets/TICKET-NG-01-shared-mm-modal.md) — Extract a shared `mm-modal` component for the repeated dialog open/close effect (CR-7.1; Fallow `dup:871c39b0`/`dup:5bb3d5e1`)
- [x] [TICKET-NG-02](../../engineering/tickets/TICKET-NG-02-overview-input-binding.md) — Bind transactions filters via `input()` instead of route snapshot (CR-7.2)
- [x] [TICKET-NG-03](../../engineering/tickets/TICKET-NG-03-url-mirror-skip-noop.md) — Skip redundant navigations in the URL-mirroring effect (CR-7.3)
- [x] [TICKET-NG-04](../../engineering/tickets/TICKET-NG-04-finish-with-archivable.md) — Finish `withArchivable` — drop dead `setArchived` or fold the flow in (CR-7.4)
...
- [ ] Sort rules and compile regexes once per "run rules" pass, not per transaction (CR-3.2, [rule-matching.ts:63](../../../src/app/core/categorisation/rule-matching.ts)) — **ticketed as [TICKET-PERF-02](../../engineering/tickets/TICKET-PERF-02-rules-compile-once.md)**
- [ ] Index `importBatchId` on transactions so undo-import isn't a full-table scan (CR-3.3, [transactions.repository.ts:24](../../../src/app/core/data-access/transactions.repository.ts)) — **ticketed as [TICKET-PERF-03](../../engineering/tickets/TICKET-PERF-03-import-batch-id-index.md)**
...
- [ ] Dexie version declarations only re-declare the tables they change (CR-6.4, [app-db.ts](../../../src/app/core/data-access/app-db.ts)) — **ticketed as [TICKET-CLEANUP-04](../../engineering/tickets/TICKET-CLEANUP-04-dexie-minimal-version-declarations.md)** (convention-forward; shipped blocks stay untouched)
- [ ] Transaction row-selection checkboxes need an accessible name (CR-8, [transactions-overview.component.html:72](../../../src/app/feature-transactions/components/transactions-overview/transactions-overview.component.html)) — header checkbox fixed; per-row remainder **ticketed as [TICKET-TXN-07](../../transactions/tickets/TICKET-TXN-07-row-checkbox-accessible-name.md)**
- [ ] Batch hot write paths into single bulk transactions — rules persistence, transfer auto-linking, category removal (CR-3.1, [rules-engine.service.ts:36](../../../src/app/core/categorisation/rules-engine.service.ts), [transfer-matching.service.ts:36](../../../src/app/core/transfers/transfer-matching.service.ts), [categories.store.ts:71](../../../src/app/feature-categories/categories.store.ts)) — **ticketed as [TICKET-PERF-04](../../engineering/tickets/TICKET-PERF-04-batch-hot-write-paths.md)**
- [ ] Decode the file once — slice to the first ~64KB for header/preview, cache per `(File, encoding)` (CR-4.1, [csv-import.service.ts:39](../../../src/app/core/import/csv-import.service.ts)) — **ticketed as [TICKET-IMP-06](../../import/tickets/TICKET-IMP-06-csv-decode-pipeline.md)** (together with CR-4.2/4.3)
- [ ] Spec the `ImportService.undoImport` cross-import transfer-cleanup branch (CR-9, [import.service.spec.ts](../../../src/app/core/import/import.service.spec.ts)) — **ticketed as [TICKET-TEST-02](../../engineering/tickets/TICKET-TEST-02-first-review-spec-gaps.md)** (all four CR-9 bullets)
...
- [ ] Hydrate only what the landing route needs; stream transactions/transfers behind a `hydrated` signal (CR-3.4, [app.config.ts:44](../../../src/app/app.config.ts)) — **ticketed as [TICKET-PERF-05](../../engineering/tickets/TICKET-PERF-05-hydrate-on-demand.md)** (together with CR-5.1)
...
- [ ] Wrap below-fold dashboard panels in `@defer (on viewport)` (CR-5.3, minor) — customize panel deferred meanwhile (TICKET-STAT-14); content panels **ticketed as [TICKET-PERF-06](../../engineering/tickets/TICKET-PERF-06-defer-dashboard-panels.md)**
- [ ] Promote app-wide entity stores to `core/state/` (or a `shared/data-access` tier); `feature-*` holds only components/routes (CR-6.1, [app.routes.ts:14](../../../src/app/app.routes.ts)) — **ticketed as [TICKET-SOLID-05](../../engineering/tickets/TICKET-SOLID-05-entity-stores-to-core.md)**, upgraded from "do last" to structural priority: fallow (2026-07-14) traced all 20 circular dependencies to this placement
- [ ] Extracted `mm-modal` wires `aria-labelledby` to the dialog title and restores focus to the trigger on close (CR-8, depends on CR-7.1 which has shipped) — `aria-labelledby` done; focus restore **ticketed as [TICKET-NG-09](../../engineering/tickets/TICKET-NG-09-mm-modal-focus-restore.md)**
- [ ] Trend chart's key numbers exposed in DOM text (even a visually-hidden table) (CR-8) — **ticketed as [TICKET-STAT-20](../../dashboard/tickets/TICKET-STAT-20-trend-chart-accessible-numbers.md)**
