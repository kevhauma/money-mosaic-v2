...
**What:** [`classify-for-stats.ts:43`](../../../src/app/core/stats/classify-for-stats.ts) — the shared per-transaction classifier CR3 deliberately created (STAT-47) — is one 54-line function at cyclomatic 26 / cognitive 26, and its joint-account path (`classify-joint-leg.ts`, 13/18) adds a second layer. It now encodes: range/nullified/zero/savings/transfer exclusion ordering, three attribution-override modes, a personal-flagged-leg special case, an untagged-refund-on-joint special case, and signed category-kind netting.
...
**How it shows:** Ticket lines 34–38 vs. `settings.routes.ts` (single `path: ''` route) and `settings-overview.component.html:168`; `docs/releases/v2/overview.md:16` still `[ ]`.
