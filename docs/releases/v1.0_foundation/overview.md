...
- [x] Persistent sidebar nav + topbar app shell (added once Accounts needed real navigation; [ui-layout-spec.md](../../reference/ui-layout-spec.md) §1)
- [x] [TICKET-DEV-01](../../tooling/tickets/TICKET-DEV-01-seed-dev-data.md) — Seed sample dev data on a fresh browser in dev mode (dev-only, supports bootstrap hydration, no production FR)
...
- [x] [TICKET-ACC-01](../../accounts/tickets/TICKET-ACC-01-clear-account-transactions.md) — Clear an account's transactions without deleting the account (extends FR-ACC-2)
...
- [x] [TICKET-IMP-02](../../import/tickets/TICKET-IMP-02-batch-multi-file-mapping.md) — Map a multi-file import batch once instead of re-mapping every file (extends FR-IMP-1/FR-IMP-3)
- [x] [TICKET-IMP-03](../../import/tickets/TICKET-IMP-03-header-mismatch-error.md) — Surface a header/mapping mismatch per file (extends FR-IMP-8)
- [x] [TICKET-IMP-04](../../import/tickets/TICKET-IMP-04-combined-map-preview-step.md) — Combine map + preview into one screen with the confirm bar pinned at the top (FR-IMP-4)
- [ ] [TICKET-IMP-01](../../import/tickets/TICKET-IMP-01-bank-presets.md) — Bank presets for KBC, Belfius, BNP Paribas Fortis, ING, and Argenta (FR-IMP §"v1 preset targets", Open Decision #5) — **partial:** KBC + Belfius shipped (unverified against a real export); BNP Paribas Fortis/ING/Argenta remain — **the only ticket still open**, independent of everything else, incremental as real sample exports become available
...
- [x] [TICKET-TXN-01](../../transactions/tickets/TICKET-TXN-01-bulk-category-assign.md) — Bulk-assign a category to selected rows via a bulk-action bar (ui-layout-spec.md §4.3)
- [x] [TICKET-TXN-02](../../transactions/tickets/TICKET-TXN-02-virtualized-table.md) — Virtualize the transactions table (NFR-PERF-1)
...
- [x] [TICKET-CAT-01](../../categories/tickets/TICKET-CAT-01-and-or-rule-conditions.md) — AND/OR combinators for rule conditions instead of an implicit AND (extends FR-CAT-2)
...
- [x] [TICKET-TRF-01](../../transfers/tickets/TICKET-TRF-01-clear-category-on-link.md) — Clear category when a transaction is linked as a transfer (extends FR-TRF-1)
- [x] [TICKET-TRF-02](../../transfers/tickets/TICKET-TRF-02-classify-savings-movements.md) — Classify money moved into savings as "savings", not expense (extends FR-TRF-1)
...
- [x] ~~[TICKET-STAT-01](../../dashboard/tickets/TICKET-STAT-01-custom-range-enable.md)~~ — superseded by TICKET-STAT-03 below, kept for traceability
- [x] [TICKET-STAT-02](../../dashboard/tickets/TICKET-STAT-02-per-account-networth.md) — Full-history balance chart per account, plus a stacked all-accounts net-worth chart (extends FR-STAT-4/FR-ACC-3)
- [x] [TICKET-STAT-03](../../dashboard/tickets/TICKET-STAT-03-expanded-range-presets-default-grouping.md) — Expanded date-range presets with a linked default grouping (extends FR-STAT-7)
