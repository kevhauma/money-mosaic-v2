...
Item IDs are `CR2-<section>.<n>` for traceability from [overview.md](./overview.md), which links each item to the ticket it produced (now filed under its feature area).
...
- `80af6be` — *"feat: Add in-browser neural-net transaction categorizer with TensorFlow.js integration"* — the diff is one markdown file (`docs/releases/v1.2_auto_categorise/auto-categorise.md`). There is no TensorFlow.js anywhere in `package.json` or `src/`.
...
Coordinate with [TICKET-NG-02](../../engineering/tickets/TICKET-NG-02-overview-input-binding.md) (query-param intake via `input()`), which shrinks the same file — do NG-02 first or together. → [TICKET-SOLID-01](../../engineering/tickets/TICKET-SOLID-01-split-transactions-overview.md)
...
3.1 + 3.2 + the ISP fix are one small ticket. → [TICKET-SOLID-02](../../engineering/tickets/TICKET-SOLID-02-type-import-domain.md)
...
These are precisely the files SOLID tickets in this review will touch — specs first makes every other item here safer. → [TICKET-TEST-01](../../engineering/tickets/TICKET-TEST-01-orchestrator-store-specs.md)
...
- **`dup:bf6204a7`** — the transfer-cleanup cascade clone, [account-deletion.service.ts:61-84](../../../src/app/core/accounts/account-deletion.service.ts) ↔ [import.service.ts:134-160](../../../src/app/core/import/import.service.ts) = CR2-4.2, exactly the extraction scoped in [TICKET-SOLID-03](../../engineering/tickets/TICKET-SOLID-03-extract-transfer-cleanup-cascade.md).
- **`dup:871c39b0` / `dup:5bb3d5e1` / `dup:9a2fd13a`** — the `<dialog>` open/close effect across the four form components = [TICKET-NG-01](../../engineering/tickets/TICKET-NG-01-shared-mm-modal.md) (fingerprints already cited there).
- **Hotspots** (complexity × churn): [transactions-overview.component.ts](../../../src/app/feature-transactions/components/transactions-overview/transactions-overview.component.ts) is the #1 hotspot at nearly double the runner-up (score 63.6, 10 commits; next: transactions.store 44.3, import-wizard 34.5) — hard churn evidence behind CR2-2.1 / [TICKET-SOLID-01](../../engineering/tickets/TICKET-SOLID-01-split-transactions-overview.md)'s priority.
...
| high (cyc 23 / cog 35) | [import-map-step.component.ts:144](../../../src/app/feature-import/components/import-map-step/import-map-step.component.ts) | `updateResult`'s `\|\| undefined` column-mapping chains — becomes data-driven under [TICKET-SOLID-02](../../engineering/tickets/TICKET-SOLID-02-type-import-domain.md) |
...
→ [TICKET-CLEANUP-01](../../engineering/tickets/TICKET-CLEANUP-01-fallow-verified-dead-code.md)
