...
* Previous/next buttons step the current range backward/forward by its own length (e.g. "This month" steps a calendar month at a time, a 7-day custom range steps 7 days at a time). Stepping away from a preset switches the selection to **Custom**, since the shifted range no longer matches that preset's definition. Delivered early against the current lightweight switcher by [TICKET-STAT-16](../../dashboard/tickets/TICKET-STAT-16-date-range-prev-next-navigation.md); this rebuild should preserve that behaviour rather than reinvent it.
