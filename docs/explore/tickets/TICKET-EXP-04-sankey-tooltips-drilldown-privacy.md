...
- **Type:** Feature
- **Traceability:** extends **FR-EXP-2** ([TICKET-EXP-02](./TICKET-EXP-02-money-flow-sankey.md)); applies FR-STAT drill-down conventions (`buildTransactionDrilldownParams`) and the privacy-mode rule from [TICKET-PRIV-01](../../settings/tickets/TICKET-PRIV-01-privacy-mode-dashboard.md).
...
- **Click-through**, using the Explore range's `from`/`to` plus whatever the clicked element identifies:
  ...
  - `savings`, `existing-balance`, `left-over` and (with EXP-03) `group` nodes → range only, since no single transaction filter expresses them. These are visibly non-interactive (no pointer cursor, no click handler) rather than clickable-but-inert.
...
- [x] ~~`savings`,~~ `existing-balance`, `left-over` (and `group`, if EXP-03 has landed) are non-interactive and visibly so. — **`savings` removed by [TICKET-EXP-06](./TICKET-EXP-06-money-flow-account-balance-tiers.md)**: a savings account is an ordinary account node now, so it drills down by `accountId` like any other, and an account-to-account ribbon filters by the account the money left. The rest is unchanged. (spec `makes savings, existing-balance and left-over non-interactive, in params and in cursor` — asserts both `undefined` params and `cursor: 'default'` on the emitted nodes *and* links, against `'pointer'` for the interactive ones; `group` covered by `still reaches the category once a group level sits in between`, and `does not navigate for a non-interactive element` proves the click handler returns without navigating)
