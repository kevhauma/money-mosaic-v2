...
- **Node ids are namespaced by kind** — `income:<categoryId|none>`, `account:<id>`, `category:<categoryId|none>`, `savings:<accountId|other>`, plus the two synthetic singletons `existing-balance` and `left-over`. A category and an account sharing a name must not collapse into one node.
- **Levels are strict**: level 0 = income sources + `existing-balance`; level 1 = accounts; level 2 = expense categories + savings + `left-over`. Every link goes from level *n* to level *n+1*, which is what makes the graph acyclic by construction (ECharts' Sankey cannot render a cycle).
...
- **The graph balances per account**, which is the correctness property that makes the picture honest:
  ...
  - `outflow > inflow` → add a `existing-balance` → `account` link for the difference (money the account already held before the range, or received outside it).
...
- Degenerate ranges render honestly: no income in range → level 0 is just `existing-balance`; no transactions at all → the section renders nothing (the page's own empty state, from EXP-01, covers the no-data-anywhere case).
- A visually-hidden table lists every link as source → target = amount, per [TICKET-STAT-20](../../dashboard/tickets/TICKET-STAT-20-trend-chart-accessible-numbers.md)'s convention, with `role="img"` and a summary label on the canvas host.

## Acceptance criteria

**Superseded in part, 2026-08-07 — see [TICKET-EXP-06](./TICKET-EXP-06-money-flow-account-balance-tiers.md).** Feedback on the shipped diagram reframed it as an *account-balance* view rather than a net-worth one, and added a second account tier so own-account transfers are visible. Five criteria below are struck through where EXP-06 reversed them; each keeps its original wording so the change is legible, and each names what replaced it. The rest still hold and were re-run green under the new model.
...
would also make a pure aggregate read `document`. `existing-balance`/`left-over` therefore carry the
...
- [x] ~~Every per-transaction decision routes through `classifyForStats()`; the aggregate re-checks none of `transferId`, `nullified`, savings-IBAN membership or joint weighting itself.~~ **Reversed by EXP-06**: the aggregate now classifies by amount sign and excludes only non-movements, because `classifyForStats` answers a net-worth question and this diagram asks a balance one. (`accumulateFlows` switches only on `classifyForStats`'s returned `kind`/`amount`; the words `transferId` and `nullified` do not appear in the file)
...
- [x] ~~Every emitted link goes from level *n* to level *n+1*~~ — **relaxed by EXP-06 to "strictly downhill"**, which is the property that actually guarantees acyclicity; with two account tiers, a purchase from a checking account runs level 1 → 3. Still asserted over a fixture containing accounts, categories, savings and joint legs. (spec `emits every link from level n to level n+1, so the graph is acyclic by construction`, over a 7-transaction fixture spanning 4 accounts incl. savings + joint)
...
- [x] ~~Savings movements land on a per-savings-account node, with an "other" fallback~~ — **replaced by EXP-06**: a savings account is an ordinary account node on tier 2, so the `savings` node kind and its `savings:other` fallback are gone. A withdrawal still nets the ribbon down instead of appearing as income, which is the half of this criterion that survived. (specs `lands a savings deposit on that savings account, and nets a withdrawal back out of it` and `falls back to a single "other" savings node …`)
...
- [x] Unit tests cover: a single-account single-income single-expense flow end to end; an account that overspends (carried-in) and one that underspends (left-over); a savings deposit and a withdrawal; an expense refund netting a link down and, at zero, dropping it; a linked non-savings transfer producing no links at all; a joint account's `ownershipShare` reaching the links through `classifyForStats` (**EXP-06 reversed this case**: the ribbon is now the face-value amount, and the spec asserts that instead); uncategorised income and expense nodes; an empty range returning empty `nodes`/`links`. (`money-flow-graph.spec.ts`, 13 cases — one per clause plus the acyclicity and multi-account balance cases. The joint case records the real contribution model: an untagged inflow is `mineIn` and counts whole, only shared *spending* is share-weighted.)
...
- [ ] Verified live in the browser: the Sankey renders on `/explore` with real imported data, ribbon widths visibly match the Dashboard's income/expense/savings totals for the same range (**EXP-06 narrowed this**: they still match on a solely-owned account, and deliberately do *not* on a joint one), and changing the range redraws it. — **left open at the user's instruction.** The *figures* were confirmed live against 41 real transactions: `/explore` for August 2026 lists Salary → Everyday Checking €2,800.00, six expense ribbons summing to €1,202.04, savings €300.00 and left-over €1,297.96 (in == out, exactly €2,800.00), which matches `/dashboard`'s Income €2,800.00 / Expense €1,202.04 / savings rate 10.7% / net cash flow €1,597.96 for the same range. The *visual* half was not checked: the Browser pane was closed, so `document.hidden` was true, `requestAnimationFrame` never fired and no chart in the app painted. `getDataURL()` on the live instance returned a 15 kB PNG (a blank canvas compresses to well under 1 kB), so the canvas is not empty — but no screenshot was taken.

## Notes

- **Why carried-in / left-over instead of just drawing what happened.** A Sankey is read as a conservation diagram: what enters a node leaves it. Feeding ECharts unbalanced data doesn't produce an error, it produces a *plausible-looking* picture with meaningless widths. Naming the imbalance ("existing balance", "left over") is both honest and useful — "left over" is the period's savings-in-place, and a large "existing balance" is the visual form of living off a buffer.
