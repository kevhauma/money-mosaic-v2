...
- [x] Dedicated money-positive / money-negative custom properties exist and are used by every money figure, including the Net worth tile. (`--mm-money-positive` / `--mm-money-negative`, declared in all ten daisyUI theme blocks and consumed by the un-layered `.mm-money-positive` / `.mm-money-negative` hooks in `styles.css`. Reached from markup through `MoneyTextColor` on `mm-text`/`mm-stat-card` and the `moneyColor` / `negativeMoneyColor` helpers. Converted: the Net cash flow tile (`dashboard-overview`), `account-balance-block`, `top-transactions-panel`, `transaction-row`, `suggestions-table`, `rule-proposals`, `transfer-review`, `import-preview-step`.)
- [x] In both default themes, a positive figure is visually distinguishable at a glance from a negative one, and neither is the brand red. (Sign-coloured money renders `--mm-money-positive` — the success green at hue 145 — against the brand coral at hue 25. `--color-error` moved from hue 20 to hue 0 in both defaults, 25° clear of the brand. **Revised 2026-08-25:** the Net worth tile is excluded — see the revision note below.)
- [x] The Net worth tile retains its existing hero styling hooks across all ten themes (no regression in `mm-net-worth` plate/blob rendering). (True by construction: the plate, wash and label colours come from the `.mm-net-worth.stat` rules in `styles.css` and the three per-theme overrides in `anti-polish` / `memphis` / `skeuomorphism`, all keyed on the class. `class="mm-net-worth"` is untouched, and since the 2026-08-25 revision the `color` input is back to `primary` too.)
...
- [x] Verified live in the browser across the default light theme, the default dark theme, and at least `retro-futurism` and `memphis`. (Dev server on :4210, 2026-08-24, and swept across **all ten** themes rather than the four asked for. Every theme reports both `--mm-money-*` tokens set and a primary↔error hue gap of at least 25° — `deformable` and `deformable-dark` 25°, `retro-futurism` 27°, `memphis` 35°, the rest 55–177°. On `deformable` the Net worth tile's `+€16.898,26` computes to `oklch(0.74 0.17 145)` — the money-positive green, matching the adjacent Net cash flow — where it used to take the brand coral at hue 25; nine of ten themes render it green. On `/transactions`, a `-€300,00` carries `.mm-money-negative` → `oklch(0.62 0.22 0)` while `+€300,00` stays in body ink, and **zero** amount cells still carry `text-error`. AC 3 checked on the same tile: plate background and border are still `--color-primary` at 10%/30%, the `.stat-title` is still coral, and the `::before` blob keeps its morph radius — only the figure's colour moved.)
...
## Revision, 2026-08-25 — the Net worth tile keeps `color="primary"`

Reverted on the user's instruction: the hero figure is meant to follow the theme's primary colour,
whatever that colour is, **including when it is red**. `netWorthColor` is gone and
`dashboard-overview.component.html` passes `color="primary"` again, as it did before this ticket.

Everything else the ticket shipped stands: the `--mm-money-*` tokens, the `.mm-money-*` hooks, the
`moneyColor` / `negativeMoneyColor` helpers, every other converted surface, the moved `--color-error`
hues and the `theme-palette.spec.ts` hue-distance guard. Those still keep a *sign-coloured* figure
clear of the brand; the Net worth tile is simply no longer a sign-coloured figure.
...
- Related: [TICKET-SET-10](../../settings/tickets/TICKET-SET-10-default-locale-to-belgian.md) — the other change that touches every screen at once.
...
- **Two figures deliberately left on the alert palette**, both checked and both out of scope by the
  ...
  which outranks `.mm-money-*`. The browser pass **confirmed** the second: under `anti-polish` the
  figure computes to `oklch(0.15 0.02 100)`, the plate ink, not the money token — that is the hero
  treatment AC 3 protects, and it is the one theme of ten where the figure is not green.
