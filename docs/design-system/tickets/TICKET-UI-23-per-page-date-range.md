...
- **Only the pages that use a range render one**, in their `[actions]` slot: `/dashboard`
  ([TICKET-STAT-25](../../dashboard/tickets/TICKET-STAT-25-dashboard-page-header.md)) and `/accounts`
  ([TICKET-ACC-08](../../accounts/tickets/TICKET-ACC-08-accounts-page-header.md)). `/transactions` does **not** — its filter
...
- **Ranges are session state, not persisted** — same as today, `RangeStore` is not Dexie-backed and
  ...
  session is [TICKET-STAT-27](../../dashboard/tickets/TICKET-STAT-27-session-persistent-chart-options.md)'s job.
...
3. **The switcher is placed on both pages here, not deferred.** This ticket's own browser criterion
   ...
   [TICKET-STAT-25](../../dashboard/tickets/TICKET-STAT-25-dashboard-page-header.md) and
   [TICKET-ACC-08](../../accounts/tickets/TICKET-ACC-08-accounts-page-header.md) still own their headers' ordering and
