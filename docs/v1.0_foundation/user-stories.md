...
- [ ] As a developer, I want bank presets for KBC, Belfius, BNP Paribas Fortis, ING, and Argenta, prioritised by what the user actually uses ([TICKET-IMP-01](./tickets/TICKET-IMP-01-bank-presets.md), FR-IMP §"v1 preset targets", Open Decision #5) — **partial:** KBC + Belfius shipped (column signatures unverified against a real export, best-effort only); BNP Paribas Fortis/ING/Argenta deferred to a follow-up
- [x] As a user, I want to select multiple CSV files, optionally link each to an account, and map the whole batch once instead of re-mapping every file, so importing several months across accounts isn't tedious ([TICKET-IMP-02](./tickets/TICKET-IMP-02-batch-multi-file-mapping.md), extends FR-IMP-1/FR-IMP-3 — today ImportWizardComponent loops the mapping step per file)
- [x] As a user, I want a visible error when a file's headers don't match the batch's chosen mapping, so I immediately know which file needs handling instead of it silently parsing wrong ([TICKET-IMP-03](./tickets/TICKET-IMP-03-header-mismatch-error.md), extends FR-IMP-8)
...
- [x] As a user, I want a transaction linked as a transfer to have no category and be excluded from income/expense on the dashboard, while still counting normally toward its account's balance, so a transfer never gets miscategorised as spending or income ([TICKET-TRF-01](./tickets/TICKET-TRF-01-clear-category-on-link.md), extends FR-TRF-1 — income/expense exclusion already works via the `transferId` checks in `period-stats.ts`/`category-breakdown.ts`, and account balances already include transfers unexcluded; the gap is that `categoryId` isn't cleared when a transaction is linked, so a rule-assigned or manually-set category can survive on a linked transaction)
- [x] As a saver, I want money I move into a savings account reported as "savings" rather than "expense", and never requiring a category, so putting money aside doesn't look like spending or nag me for a category ([TICKET-TRF-02](./tickets/TICKET-TRF-02-classify-savings-movements.md), extends FR-TRF-1 — today a savings movement is only kept out of expenses when it happens to be linked; an unlinked one-sided movement to a savings account still counts as expense and shows up as uncategorised)
...
- [x] As a user, I want more date-range presets (this/last week, this/last quarter, this/last year, year-to-date, all-time) plus a working "Custom" option, each applying a sensible default grouping the moment it's picked (e.g. a full year defaults to monthly buckets, a quarter to weekly), so I can view my finances at whatever timeframe I actually want without manually fixing the grouping every time ([TICKET-STAT-03](./tickets/TICKET-STAT-03-expanded-range-presets-default-grouping.md), extends FR-STAT-7 — supersedes [TICKET-STAT-01](./tickets/TICKET-STAT-01-custom-range-enable.md)'s narrower custom-range bug fix, folded in as part of this ticket)
- [x] As a saver, I want a full-history balance chart on each account's detail page, plus a stacked all-accounts net-worth-history chart (toggleable per account) on the accounts overview, so I can see how each account's saldo has trended over its whole life and how the accounts add up — while the dashboard trend stays focused on income/expense + combined net worth (extends FR-STAT-4 / FR-ACC-3 — [TICKET-STAT-02](./tickets/TICKET-STAT-02-per-account-networth.md); moved off the dashboard into the Accounts feature)
