...
- **Page header** (revised by TICKET-UI-22) — every page opens with exactly one `mm-page-header`, rendered as the app's top bar: daisyUI's `navbar`, full-bleed across the content area, carrying the page title on the left, that page's own page-level controls in its `[actions]` slot on the right, and nothing else. It is the same bar the shell topbar used to be — TICKET-UI-23 handed it to the pages so each could own its date range. **There is no subtitle** — a page that needs an explanatory sentence puts it in the body, where it belongs to the content rather than to the chrome. "Page-level" means anything that reconfigures the whole page (date range, view switch, create-new, show-archived, page settings, guide links, re-run); a control scoped to one panel (a chart's bucket picker, say) stays on that panel. The action group wraps rather than overflowing, so a four-control header degrades to two rows on a phone.
