...
| 6 | Trend & outliers | `trend-chart-panel` (Income \| Expense, each a same-scale stacked-by-category bar chart) \| `top-transactions-panel` (biggest individual transactions) | `grid grid-cols-1 lg:grid-cols-2` | v1.0 (trend chart) + [STAT-08](./tickets/TICKET-STAT-08-biggest-transactions.md) + [STAT-17](./tickets/TICKET-STAT-17-split-trend-chart-income-expense.md) | Trend chart moves out of row 4 (bumped by STAT-13 going full-width) into its own two-column row, paired with the new biggest-transactions list — both are "look past the totals" panels of similar visual weight. STAT-17 splits the trend chart *internally* into two same-scale, category-stacked bar charts (Income, Expense) — no change to this row's own column count. |
| 7 | Action queue | Uncategorised-count card, transfer-review-count card | full | v1.0 | Unchanged. Hidden per-card at zero, as today. |
| 8 | Account balance strip | Per-account name + balance | full | v1.0 | Unchanged, stays last. |
...
├───────────────────────────────┬──────────────────────────────┤
│ Trend chart (income/expense/  │ Biggest transactions list      │
│ net-worth over time)          │                                │
├────────────────────────────────────────────────────────────────┤
...
- **`<640px` (mobile) / `640–1023px` (tablet):** every multi-column row (stats, rate & rhythm, category breakdown's expense/income columns, trend & outliers) collapses to 1 column, stacked top-to-bottom in the same row order as the desktop table above. The stats row uses daisyUI's existing `stats-vertical` fallback.
- **`≥1024px` (desktop):** rows 3 and 6 use `lg:grid-cols-2`; row 4's expense/income columns use the same `lg:grid-cols-2` internally, within a full-width card.
...
Once STAT-13 ships, this splits into two rows — `category-breakdown-panel` full width, `trend-chart-panel` moved into the new row 6 alongside `top-transactions-panel` (STAT-08). Each new panel component (`category-comparison-panel`, `top-transactions-panel`, the rate/rhythm cards) is added to this template as its own ticket lands; this doc is the target arrangement each of those tickets' "placed on the dashboard grid" notes should converge on, so two tickets landing in either order don't fight over row/column placement.
