...
| 6 | Trend & outliers | `trend-chart-panel` (existing income/expense/net-worth trend) \| `top-transactions-panel` (biggest individual transactions) | `grid grid-cols-1 lg:grid-cols-2` | v1.0 (trend chart) + [STAT-08](./tickets/TICKET-STAT-08-biggest-transactions.md) | Trend chart moves out of row 4 (bumped by STAT-13 going full-width) into its own two-column row, paired with the new biggest-transactions list — both are "look past the totals" panels of similar visual weight. |
