...
- **Area:** Build/bundling
- **Released in:** [v1.2 Auto-categorise](../../releases/v1.2_auto_categorise/overview.md)
- **Type:** Bug fix
...
- Confirmed root cause is unrelated to any single feature's code: reproduced on a clean `main` with zero other changes applied (verified while working [TICKET-ML-05](../../learning/tickets/TICKET-ML-05-training-worker.md), by stashing an unrelated diff and rebuilding).
...
- Discovered while working [TICKET-ML-05](../../learning/tickets/TICKET-ML-05-training-worker.md) (the auto-categorisation training worker); it is unrelated to tfjs or auto-categorisation — tfjs was independently confirmed to isolate correctly into its own worker chunk.
