# TICKET-STAT-46 — Contribution-based net worth for joint accounts

- **Area:** Statistics & Dashboard
- **Released in:** [v1.1 Joint accounts](../../releases/v1.1_joint_accounts/overview.md)
- **Type:** Feature
- **Traceability:** changes FR-STAT-1 and FR-TRF-1 semantics for joint accounts; touches FR-STAT-2/3/4; depends on [ACC-02](../../accounts/tickets/TICKET-ACC-02-joint-ownership-share.md) + [ACC-03](../../accounts/tickets/TICKET-ACC-03-multi-owner-coowner-ibans.md) + [CAT-02](../../categories/tickets/TICKET-CAT-02-neutral-category-kind.md)
...
Make net worth count only *my* stake in a joint account under a **contribution model**: my deposits into the pot (from my own accounts) and my own income into it count at 100%; a partner's/external inflow counts at 0% (via the `neutral` kind from [CAT-02](../../categories/tickets/TICKET-CAT-02-neutral-category-kind.md)); joint spending reduces my stake by my `ownershipShare` (from [ACC-02](../../accounts/tickets/TICKET-ACC-02-joint-ownership-share.md)); money I move back out to my own accounts reduces it at 100%. The account's real bank balance is unchanged and still shown on the accounts page — only the *net-worth contribution* of a joint account is re-weighted. Income/expense stats are weighted consistently so savings rate stays coherent with the stake.
...
- **My net-worth stake** in `J`:
  ...
  - co-owner/external inflows contribute **0** — identified by `kind === 'neutral'` **or** a `counterpartyIban` matching a registered co-owner IBAN ([ACC-03](../../accounts/tickets/TICKET-ACC-03-multi-owner-coowner-ibans.md)), so a co-owner deposit counts as 0 to my stake even before it has been tagged.
