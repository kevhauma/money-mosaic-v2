...
- Because both columns now render at once, the panel card grows from half-width to full-width in the dashboard grid (see [dashboard-layout.md](../../releases/v1.3_dashboard_insights/dashboard-layout.md)) — `dashboard-overview.component.html`'s two-column row is adjusted so `app-category-breakdown-panel` spans the full row and `app-trend-chart-panel` moves to its own row.
