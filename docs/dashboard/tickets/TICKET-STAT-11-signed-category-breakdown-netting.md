...
- This does not change **where** a categorised transaction lands (still driven by `category.kind`, established by [TICKET-CAT-02](../../categories/tickets/TICKET-CAT-02-neutral-category-kind.md)) — only **how its magnitude nets** once it's in that bucket, and that the overall totals now agree with the per-category ones instead of falling back to raw sign for categorised transactions.
