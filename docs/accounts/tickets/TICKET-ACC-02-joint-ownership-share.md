...
- **Type:** Feature
- **Traceability:** extends FR-ACC-1; consumed by [STAT-46](../../dashboard/tickets/TICKET-STAT-46-contribution-net-worth.md)
...
- The input's **placeholder** defaults to `1 / (1 + number of registered co-owners)` — e.g. `50%` for 1 co-owner, `33%` for 2 — recalculated live as co-owners are added/removed on [ACC-03](./TICKET-ACC-03-multi-owner-coowner-ibans.md)'s sub-form (with no co-owners yet, it falls back to `50%` as today). It is only ever a **placeholder**: the stored value stays `undefined` until the user commits one, per the "undefined ⇒ 1 (whole)" convention below.
...
- No visible number changes yet: balances, net worth, and stats are untouched until STAT-46 lands.
...
- [x] **No Dexie version bump** is introduced for this field (it is non-indexed and optional); a version block is added only if STAT-46 or CAT-02 needs one, and if so it stays additive (next is version 6). The field is confirmed to round-trip through IndexedDB without a schema change.
...
- **Scope guard:** field + form only. If reviewers want the number to *do* something, that is STAT-46 — keep this ticket inert so it can merge independently.
...
- `ownershipShare` is a single scalar, so changing it rewrites the entire historical net-worth trend retroactively. Date-effective shares are the correct long-term fix but are out of scope for v1.5 — record the limitation in the helper text and in STAT-46.
- The placeholder default is `1 / (1 + co-owner count)`, not a fixed `0.5` — store `undefined` until the user commits a value, so the "undefined ⇒ 1" convention stays unambiguous for accounts created before this ticket. This placeholder depends on ACC-03's co-owner list existing on the in-progress form state, so build the two controls together even though they persist to separate fields.
