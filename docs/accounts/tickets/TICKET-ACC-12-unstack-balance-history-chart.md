...
- [x] Verified live in the browser: `/accounts` balance history read against the account cards' balances for the same date. (Dev server on :4210, 2026-08-24. Per-account default: the two series carry no `stack`, `areaStyle {opacity: 0.12}`, and their last plotted values are **7691.84** and **9206.42** — identical to the cards' `+€7.691,84` (Everyday Checking) and `+€9.206,42` (Rainy Day Savings), which are the very figures the UX review reported the chart contradicting. Clicking "Combined total" restores `stack: 'account-balance'` with solid fills, the two sum to **16898.26**, and the caption switches to "the top edge is the combined total across every account, not any one account's balance". No console or server errors.)
...
- Related: [TICKET-STAT-42](../../dashboard/tickets/TICKET-STAT-42-name-the-savings-measures.md), [TICKET-EXP-09](./TICKET-EXP-09-exclude-linked-transfers-from-sankey.md) — same theme of numbers the user cannot trust.
