...
- [ ] As a developer, I want `withArchivable` finished — either drop the unused `setArchived` or move the persist-then-patch archive flow into the feature — so the two consuming stores stop duplicating their `archiveX`/`unarchiveX` pairs ([TICKET-NG-04](./tickets/TICKET-NG-04-finish-with-archivable.md), CR-7.4, [with-archivable.ts](../../src/app/shared/utils/with-archivable.ts))
