...
- [ ] As a developer, I want the URL-mirroring effect to skip navigating when params already match the current snapshot, so it can't ping-pong or schedule redundant navigations right after the initial read-in ([TICKET-NG-03](./tickets/TICKET-NG-03-url-mirror-skip-noop.md), CR-7.3, [app.ts:80](../../src/app/app.ts))
