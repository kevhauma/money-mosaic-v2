...
| `AppSettings` | `appSettings` | `id` (singleton, id=1) | app-wide portable preferences (TICKET-SET-05): accent `primaryColor`, `currencySymbol`/`currencySymbolPosition`, `locale`, plus the `/income` page's `excludedIncomeCategoryIds`, `smoothedBonusCategoryIds` and `careerStartDate`. **This is the table to extend for a new user preference** — every field is non-indexed and optional, so adding one needs no version bump (`.stores()` declares indexes, not fields). Fields are typed required-but-possibly-`undefined` and default to `undefined`, never a concrete value; each reader applies its own `?? DEFAULT_*`, so one setter's read-merge-put can't leak another's default into the row |
