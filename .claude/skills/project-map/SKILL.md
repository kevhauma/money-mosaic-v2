...
| `IncomeStore` | `feature-income/` | single feature; derives only (no repository of its own yet) — reads `CategoriesStore` for `incomeCategories`, `AccountsStore`/`TransactionsStore` for the page-level `fullHistoryRange` every chart on `/income` shares, and `AppSettingsStore` for the FR-INC-3 selection, which it exposes as `selectedIncomeCategoryIds()`/`toggleIncomeCategory()` while `appSettings.excludedIncomeCategoryIds` is what actually persists |
