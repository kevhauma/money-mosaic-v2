...
| `IncomeStore` | `feature-income/` | single feature; derives only (no repository of its own yet) — reads `CategoriesStore` for `incomeCategories`, and `AppSettingsStore` for the FR-INC-3 selection, which it exposes as `selectedIncomeCategoryIds()`/`toggleIncomeCategory()` while `appSettings.excludedIncomeCategoryIds` is what actually persists |
