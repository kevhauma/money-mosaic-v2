...
- **A money figure never reads its colour from the daisyUI palette** (TICKET-UI-27). `primary`, `success` and `error` are the *alert/brand* vocabulary; money has its own — `--mm-money-positive` / `--mm-money-negative`, declared by every theme block and reached from markup as the `money-positive` / `money-negative` members of `TextColor` (`mm-text`, `mm-stat-card`) or as the `.mm-money-*` classes directly on a `<td>`. Decide the value with [`moneyColor` / `negativeMoneyColor`](../../../src/app/shared/ui/typography/money-color.ts), never an inline `amount < 0 ? 'error' : …`: `moneyColor` for a figure coloured by its sign, `negativeMoneyColor` for a surface that marks losses only and leaves positives in the body ink (a long table). This exists because both default themes shipped `--color-primary` and `--color-error` five hue degrees apart, so the Net worth tile's `color="primary"` painted a positive figure in the loss red; [`src/themes/theme-palette.spec.ts`](../../../src/themes/theme-palette.spec.ts) now fails any theme whose two hues come within 20°, so **a new theme must state both `--mm-money-*` tokens in its `@plugin 'daisyui/theme'` block** — the spec asserts one pair per block.
...
- **The app shell owns two landmarks and the two skip links that target them** (TICKET-UI-31): `<main id="main-content" tabindex="-1" aria-label="Page content">` and `<nav id="app-nav" tabindex="-1" aria-label="Main">` in [app-shell.component.html](../../../src/app/core/layout/app-shell/app-shell.component.html), with the skip links as the first two children of `.drawer-content` so nothing can precede them in the tab order. daisyUI's drawer puts content *before* the sidebar in the DOM — the sidebar only looks left-hand because the drawer is a grid — so "skip to navigation" is the one that saves a traversal here, not "skip to content". Work that restructures the sidebar keeps the `nav` element, its id and its `tabindex="-1"`; a skip link resolves its target through `viewChild.required`, never `document.getElementById`.
- **A figure is blurred under privacy mode; prose about the figure is not** (TICKET-STAT-42). `mm-stat-card` carries both kinds and keeps them on separate inputs: `tooltip` holds the *amounts* behind a sub-label and goes through `mm-privacy-blur`, while `definition` holds one sentence saying what the tile counts and stays legible — hiding "what this measures" along with "how much" makes a card less useful without making it more private. A definition affordance is a real `<button>` carrying the sentence as its accessible name (daisyUI's `.tooltip` opens on `:has(:focus-visible)` as well as `:hover`, so that is also what makes it reachable without a pointer), and the sentence goes on `data-tip` rather than in a `.tooltip-content` child so it does not land inside `.stat-title` and pollute every reading of the label.
