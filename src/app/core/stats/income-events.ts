 ...
 * would need adding.
 */
export type IncomeEventKind = 'raise' | 'pay-cut' | 'bonus' | 'stream-stopped';
 ...
 * quiet, and the months the user recorded a bonus against their salary (FR-INC-10), which until now
 * only ever existed as an adjustment inside the take-home ratio rather than as a moment.
 *
 ...
 * one (display smoothing must not paint over a genuine silence) — and this module stays a pure merge
 * with no opinion about detection.
 *
