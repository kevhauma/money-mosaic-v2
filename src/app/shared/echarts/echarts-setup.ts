...
//
// `HeatmapChart` + `VisualMapComponent` (TICKET-STAT-29), `SankeyChart` (TICKET-EXP-02) and
// `TreemapChart` (TICKET-EXP-07) are registered here rather than inside feature-dashboard /
// feature-explore for the same reason as everything above them: one registration point is what keeps
// a second feature from re-registering (and re-bundling) the same module later. All are small, and
// echarts is loaded only from lazy feature chunks either way — the production `initial` budget is
// untouched.
//
